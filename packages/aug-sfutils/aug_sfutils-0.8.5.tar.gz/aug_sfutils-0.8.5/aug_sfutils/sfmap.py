...
SFOtypeList = [\
    [ 1, 'Diagnostic' , '>4s2H2I4H' , ('diag_code', 'num_objs', 'diag_type', 'shot_nr', 'c_time', 'up_limit', 'exp', 'version', 's_type')],
...
calibLabel = {val: key for key, val in vars(CalibType).items()}
#timebaseLabel = {val: key for key, val in var(TimebaseType).items() if '__' not in key}
timebaseLabel = {val: key for key, val in vars(TimebaseType).items()}
