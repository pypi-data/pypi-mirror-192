...
def _check_probdist_args(freqdist, **kwargs):
    ...
    if unobs_prob:
        if unobs_prob < 0.0 or unobs_prob > 1.0:
            raise ValueError(
                "Reserved unobserved probability must be in " "range [0.0, 1.0]."
            )
...
def uniform_dist(
    freqdist: Dict[str, int], unobs_prob: float = _UNOBS
) -> Tuple[Dict, float]:
    """
...
def random_dist(
    freqdist: Dict[str, int], unobs_prob: float = _UNOBS, seed: Hashable = None
) -> Tuple[Dict, float]:
    """
    ...
    probdist = {
        sample: math.log((count / fake_sum) * (1.0 - unobs_prob))
...
def mle_dist(
    freqdist: Dict[str, float], unobs_prob: float = _UNOBS
) -> Tuple[Dict, float]:
    """
    ...
    probdist = {
        sample: math.log((count / value_sum) * (1.0 - unobs_prob))
...
def lidstone_dist(
    ...
    probdist = {
        sample: math.log((count + gamma) / (n + b * gamma))
        for sample, count in freqdist.items()
    }
...
def wittenbell_dist(
    freqdist: Dict[str, int], bins: Optional[int] = None
) -> Tuple[Dict, float]:
    """
