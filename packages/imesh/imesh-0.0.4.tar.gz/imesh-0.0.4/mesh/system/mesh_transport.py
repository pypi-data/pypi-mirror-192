...
class MeshTransport(Transport):
    ...
    def open(self, session_id: str, metadata: Dict[str, str]) -> Session:
        ...
        if not session:
            ...
            session = self.sessions[session_id] = MeshSession(session_id, metadata, attachments, addr, principal)
        return session
...
class MeshSession(Session):
    ...
    def release(self, timeout: int):
        return self.with_context(lambda: self.session.release(timeout), False, timeout)
