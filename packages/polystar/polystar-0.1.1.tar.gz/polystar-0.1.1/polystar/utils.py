...
.. code-block:: python
  ...
  import numpy as np
  from polystar.utils import create_bz, create_grid
...
Note
----
The ``rotateslike`` enumeration is given in :py:meth:`~polystar._polystar.BZMeshQdc.fill`
...
def create_bz(*args, is_reciprocal=False, use_primitive=True, search_length=1,
    ...
    """
    from polystar import Lattice, BrillouinZone
...
def create_grid(bz, complex_values=False, complex_vectors=False,
    ...
    """
    from polystar import BrillouinZone, _polystar
    ...
    def constructor(grid_type):
        if complex_values and complex_vectors:
            return getattr(_polystar, grid_type + 'cc')
        elif complex_vectors:
            return getattr(_polystar, grid_type + 'dc')
        else:
            return getattr(_polystar, grid_type + 'dd')
