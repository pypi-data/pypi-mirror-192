...
from aus.dsl.experimental.conditionals import conditional
from aus.dsl.placeholder import placeholder as ph
from aus.orchestrator import data_types
from aus.orchestrator import metadata
from aus.types import channel
from aus.utils import topsort
...
class Pipeline(base_node.BaseNode):
    """Logical AUS pipeline object.
  Pipeline object represents the DAG of AUS widgets, which can be run using
  one of the workflow orchestration systems that AUS supports. For details,
  please refer to the
  ...
  """
    ...
    @property
    def dsl_context_registry(
            self) -> dsl_context_registry.DslContextRegistry:  # pylint: disable=g-missing-from-attributes
