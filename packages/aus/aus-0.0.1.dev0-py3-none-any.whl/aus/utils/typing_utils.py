...
ArtifactMultiMap = MultiMap[str, aus.types.Artifact]
ArtifactMutableMultiMap = MutableMultiMap[str, aus.types.Artifact]
# Commonly used legacy artifact dict concrete type. Always prefer to use
# ArtifactMultiMap or ArtifactMutableMultiMap.
ArtifactMultiDict = Dict[str, List[aus.types.Artifact]]
...
def is_homogeneous_artifact_list(value: Any) -> bool:
  ...
  return (
      is_compatible(value, Sequence[aus.types.Artifact]) and
      all(isinstance(v, type(value[0])) for v in value[1:]))


def is_artifact_list(value: Any) -> bool:
  return is_compatible(value, Sequence[aus.types.Artifact])
