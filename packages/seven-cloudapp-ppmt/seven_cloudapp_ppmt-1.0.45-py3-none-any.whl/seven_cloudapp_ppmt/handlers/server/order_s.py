...
@LastEditTime: 2023-02-15 17:03:10
...
class PrizeOrderExportHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if export_type == 1:
            ...
            if len(prize_order_list_dict) > 0:
                ...
                for prize_order in prize_order_list_dict:
                    ...
                    for prize_roster in prize_roster_list_filter:
                        ...
                        data_row["open_id"] = prize_order["open_id"]
            ...
            data_columns = ["淘宝主订单号", "淘宝子订单号", "小程序订单号", "淘宝名","open_id", "盒子名称", "盒子价格", "奖品名称", "商家编码", "姓名", "手机号", "省份", "城市", "区县", "街道", "收货地址", "物流单号", "物流公司", "发货时间","下单时间", "订单状态", "奖品价值", "奖品规格", "备注"]
    ...
    def get_async(self):
        ...
        if export_type == 1:
            ...
            for prize_roster in prize_roster_list:
                ...
                data_row["open_id"] = prize_roster["open_id"]
                data_row["盒子名称"] = prize_roster["machine_name"]
                data_row["盒子价格"] = str(prize_roster["machine_price"])
                data_row["奖品名称"] = prize_roster["prize_name"]
                data_row["奖品价值"] = str(prize_roster["prize_price"])
            ...
            data_columns = ["行为编号", "淘宝子订单号", "淘宝名", "open_id", "盒子名称", "盒子价格", "奖品名称", "奖品价值", "SKU", "获得时间", "状态"]
