...
from domino_code_assist.util import nb_locals

from .transform_panel import TransformPanel


@reacton.component
def TransformDrawer(action_state, set_code, dfs, open, edit, on_close, on_load_own_data, overwrite_warning=None):
    ...
    with drawer.RightDrawer(
        ...
        with sol.Div(class_="dca-transform-header", style_="margin-left: -24px; margin-right: -24px; margin-top: -4px; padding: 8px 80px;"):
            DfSelect(v_model=df_var_name, label="DataFrame", on_v_model=set_df_var_name, items=extended_dfs or [])
