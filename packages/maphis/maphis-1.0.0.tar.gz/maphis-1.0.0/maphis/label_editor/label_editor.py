...
class LabelEditor(QObject):
    ...
    def _handle_primary_label_changed(self, label: int, old_label: int):
        if self.state.storage is None:
            return
        if label > 0:
            label_level = self.state.storage.get_label_hierarchy(self.state.current_label_name).get_level(label)
    ...
    def toggle_approval_for(self, level_idx: int, approved: bool):
        level_name = self.state.storage.get_label_hierarchy(self.state.current_label_name).mask_names[level_idx]
        if approved:
            self.approve_level(level_name)
        else:
