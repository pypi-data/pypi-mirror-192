...
def json_decode(w):
    if NODEJS:
        ...
        r = run("node {}".format(os.path.join(os.getcwd(), "tmp.js")), shell=True, stderr=PIPE, stdout=PIPE).stdout.decode()
        os.remove("tmp.js")
        return r
    elif JSONNET:
        import _jsonnet
...
def search_images(kw, page=1, raw=True, k=None, r=None, **kwargs):
    cat = "Images"
...
def submit_search_image(url, k=None, raw=True):
    url = "https://"+google_domain+"/searchbyimage?client=app&image_url="+quote(url)
    r = get(url, k)
    if raw:
        return r.content
    else:
...
def search_by_image(url, page=1, raw=True, k=None, **kwargs):
    if k is None:
        while True:
            k = random.SystemRandom().randint(0, len(mirrors)-1)
            if mirrors[k][1] in sbi_bl:
                continue
            break
    r = submit_search_image(url, k)
    r2 = html.fromstring(r.decode())
    url = str(r2.xpath("//title-with-lhs-icon//a/@href")[0])
    if url.startswith("/search"):
        url = "https://"+google_domain+url
    r = get(url, k)
    return search_images(None, page=page, raw=raw, k=k, r=r, **kwargs)
