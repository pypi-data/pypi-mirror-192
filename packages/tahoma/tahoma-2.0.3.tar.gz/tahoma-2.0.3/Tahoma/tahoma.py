...
#MIT Licence
version = 'tahoma-pzim - Version 2.0.3 - by @pzim-devdata'
        ...
        try :
            ...
            try:
                if notification.lower() == 'y'or notification.lower() == 'yes':
                    os.system("notify-send -i "+icon_app+" --expire-time=150000 Tahoma-pzim '"+str_info+"'")
            except:pass
            info()
            exit()
...
for i in range(-1,int(len(args.suite)/3)) :
    ...
    ##########################ALARMS
        ...
        if remove_accent(action).upper() == "ARM" or remove_accent(action).upper() == "ACTIVER" :
            fonction = Command(OverkizCommand.ARM)
        elif remove_accent(action).upper() == 'DISARM' or remove_accent(action).upper() == "DESACTIVER" :
