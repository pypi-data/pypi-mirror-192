...
class LucoApi:
    ...
    def __get_slot_sequence_id(self, name):
        """
        ...
        payload = urllib.parse.urlencode({'SlotSequenceDefinition': definitions}, doseq=True)
        ...
        if sequence_id:
            ...
            raise Exception(f'Could not find Id for Slot Sequence: {name}')

    def export_slot_sequences(self, ids=None, names=None):
        """
        ...
        if names:
            ...
            ids = [self.__get_slot_sequence_id(name) for name in names]
