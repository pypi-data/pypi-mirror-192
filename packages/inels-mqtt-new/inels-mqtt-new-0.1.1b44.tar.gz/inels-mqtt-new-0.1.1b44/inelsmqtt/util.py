...
class DeviceValue(object):
    ...
    def __find_ha_value(self) -> None:
        ...
        if self.__device_type is SWITCH:  # outlet switch
            if self.__inels_type is RFSC_61:
                if self.inels_status_value is None:
                    _LOGGER.info("inels_status_value was 'None' for %s", RFSC_61)
                ...
                self.__inels_set_value = f"02\n{(self.__ha_value.re[0] * 1):02X}\n"
            if self.__inels_type is RFSTI_11B:
                if self.inels_status_value is None:
                    _LOGGER.info("inels_status_value was 'None' for %s", RFSTI_11B)
                    self.__inels_set_value(DEVICE_TYPE_07_COMM_TEST)
                    self.ha_value = None
                else:
                    re = []
            ...
            if self.__inels_type is RFTI_10B:
                ...
                temp_in = self.__trim_inels_status_values(DEVICE_TYPE_12_DATA, TEMP_IN, "")
            ...
            if self.__inels_type is RFDAC_71B:
                if self.inels_status_value is None:
                    ...
                    brightness = int(self.__trim_inels_status_values(DEVICE_TYPE_05_DATA, RFDAC_71B, ""), 16)
    ...
    def __find_inels_value(self) -> None:
        ...
        if self.__device_type is SWITCH:
            if self.__inels_type in [SA3_01B, SA3_02B, SA3_02M, SA3_04M, SA3_06M, SA3_012M, SA3_022M]:
                value = ""
                ...
                self.__inels_set_value = SWITCH_WITH_TEMP_SET[self.__ha_value.re[0]]
            elif self.__inels_type == RFSC_61:
                self.__inels_set_value = f"02\n{(self.__ha_value.re[0] * 1):02X}\n"
            else:
