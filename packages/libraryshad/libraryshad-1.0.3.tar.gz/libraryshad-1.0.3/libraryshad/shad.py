...
class robo_shad:
	...
	def serch(self,object_guid, search_text):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robo_shad._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("message_ids")
	            break
	        except:continue
