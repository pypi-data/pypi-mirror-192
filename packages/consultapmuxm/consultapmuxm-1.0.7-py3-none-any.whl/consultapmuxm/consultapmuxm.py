            ...
            for vari in var:
                try:
                    if vari in lvar:
                        ...
                        dffinal [fname] = dfcol
                        #dfcol.name = fname
                        #dffinal = pd.concat(dffinal,dfcol,axis=1)
                    else:
            ...
            if vari in volvars:
                ...
                for i in dfinfopmu:
                    try:
                        ...
                        #lvoltagecol.append(v_m)
                        dfpmudata [v_m] = dfpmudata [v_m] * m.sqrt(3)
                    except: 
                        print("La magnitud de tensión no existe, de la PMU con ID:" + str(i))
        #lvolcol = list(set(lvoltagecol))
        #dfpmudataline = dfpmudata[lvolcol] *m.sqrt(3)
        #dfpmudataline['ts'] = dfpmudata['ts']
        #dfdata = pd.merge(dfpmudata, dfpmudataline, on='ts', how=  )
        return(dfpmudata)    
