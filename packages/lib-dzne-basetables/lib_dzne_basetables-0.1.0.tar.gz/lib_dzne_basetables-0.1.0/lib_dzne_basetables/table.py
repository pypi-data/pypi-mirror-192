...
import BASE.lib.tabling.common as comm
import BASE.lib.tabling.pattern
...
def identify_columns(table, *, patterns):
    ...
    return BASE.lib.tabling.pattern.select(list(table.columns), patterns)
...
def augment(table, *, aux, columns, demand):
    ...
    for i, row in table.iterrows():
        ...
        if demand and (0 == len(auxrowss[ID])):
            continue
        try:
            rows.append(comm.fuse(row, *auxrowss[ID]))
...
def unify(table, *, columns, purge=False):
    ...
    for ID, rds in rdss.items():
        try:
            rows.append(comm.fuse(*rds, reduce=True))
        except AssertionError as err:
            info = dict(zip(list(columns), list(ID)))
            print(
