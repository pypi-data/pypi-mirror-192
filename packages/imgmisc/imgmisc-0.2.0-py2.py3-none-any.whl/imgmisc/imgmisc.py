...
def _parse_input_files(inputs, include=None, exclude=None):
    """Parse input files or directory"""
    if len(inputs) == 1 and os.path.isdir(os.path.abspath(inputs)):
...
def _parse_resolution(fname, resolution=None, um_threshold=1e-3):
...
def _parse_image_input(fname, channel=None):
...
def regionprops3D(
    label_image, intensity_image=None, properties=("label"), resolution=[1, 1, 1]
):
