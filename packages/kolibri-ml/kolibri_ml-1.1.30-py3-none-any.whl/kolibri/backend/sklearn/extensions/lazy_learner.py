...
class LazyRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_neighbors:int=10, algorithm='auto',  distance='heom', leaf_size:int=10,weight_by_distance=True,
                 ...
                 learning_rate=0.03, l2_leaf_reg=3.0, iterations=100, verbose=False, linear_model=False, augment_samples=False, inddependent_var_name=None):
        "constructor"
...
if __name__ == '__main__':
    ...
    columns_to_remove = ["NUM","NUMDO","NUMDO_NOPAP"]
    # Load the dataset from sklearn
    data_origin = pd.read_csv("/Users/mohamedmentis/Downloads/Poidspapier- 3 ao_t 2022_V1.csv")

    data=data_origin.drop(columns=columns_to_remove, errors='ignore')

    lr=LazyRegression(weight_by_distance=True, linear_model=True, augment_samples=True)
    target=data["PoidsKG"]

    lr.fit(data.drop(columns=["PoidsKG"]), target)
    test_data=pd.read_csv("/Users/mohamedmentis/Downloads/Poidspapier-test.csv")

    print(lr.predict(test_data.drop(columns=["PoidsKG"])))
