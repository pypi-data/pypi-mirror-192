    ...
    def handle_request(self, msg):
        ...
        try:
            ...
            if message_type is MessageType.TRAIN.value:
                add_metrics = self.local_training_handler.should_incr_n_completed_trains(data.get('payload'))
                if add_metrics and self.metrics_recorder:
            ...
            if message_type is MessageType.TRAIN.value and add_metrics:
    ...
    def handle_async_request(self, msg):
        ...
        try:
            response_msg = ResponseMessage(
                ...
                id_request=-1, data={'ACK': True, 'id_request': msg.get_header()['id_request']}
            )
            logger.info("received a async request")
