...
class XMLFile():
    def read(self, path: str, mode: str = 'r', encoding = 'utf-8'):
        try:
            ...
            # print("> hvscripts.xml_file.py | XMLFile.read - path:: \n", path)
            # print("> hvscripts.xml_file.py | XMLFile.read - pathTarget:: \n", pathTarget)

            return xmltodict.parse(pathTarget, process_namespaces=True)
        except Exception as err:
            e = "[ERROR] hvscripts.xml_file.py | read - err:: {err} ".format(err = str(err))
            # print(e)
    ...
    def write(self, path: str, datas: dict, mode: str = 'wb', encoding = 'utf-8'):
        try:
            ...
            # print("> hvscripts.xml_file.py | XMLFile.write - out:: ", out)
            with open(path, mode) as file:
                file.write(out.encode(encoding))

            return True
        except Exception as err:
            e = "[ERROR] hvscripts.xml_file.py | XMLFile.write - err:: {err} ".format(err = str(err))
            # print(e)
            stack = str(traceback.format_exc())
            log.error(stack)
            return False