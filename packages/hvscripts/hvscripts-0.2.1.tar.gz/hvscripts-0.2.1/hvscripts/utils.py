...
def getPurIdBody(
    ...
    if type(body) == dict:
        ...
        slug = body['slug'] if 'slug' in body.keys() else id

        finalId = (_id if _id else slug)
...
def CRUDCleanFinalDatas(data: any, exclusion: 'list | tuple' = None, singleid: bool = False):
    ...
    if type(data) == dict :
        ...
        res: list = list(
            map(
                lambda value: CRUDCleanFinalDatas(value),
                data,
            )
        )
        return res
...
def CRUDCleanFinalDatas2(data: any, exclusion: 'list | tuple' = None, singleid: bool = False):
    ...
    if type(data) == dict :
        ...
        res: list = list(
            map(
                lambda value: CRUDCleanFinalDatas2(value),
                data,
            )
        )
        return res
...
def TranslateText(texte, translations):
    texte = texte if (
        type(texte) == str
        and len(texte) > 0
    ) else ''
