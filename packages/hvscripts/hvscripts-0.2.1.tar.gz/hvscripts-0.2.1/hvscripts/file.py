...
def dataURIGetFIleData(
    ...
    try:
        ...
        if(checkIfBASE64(data)):
            ...
            # if DEBUG :
                # print('--> hvscripts - fileData:: ', fileData)
...
def Upload(
    ...
    showMessage = True,
    lang = 'fr',
    map = lambda x: "{filename}{sub}".format(
        filename = x,
        sub = RandomStr(
            lengthStr=10
        ),
...
def Download(
    ...
    loseMessage = {
        'type': 'danger',
        'message' : 'Echec lors du telechargement du file'
    }
):
    result = JsonResponse(loseMessage)
    try:
        newFileName = newFileName if type(newFileName) == str and len(newFileName) > 0 else fileName
        #newFileName = CleanName(newFileName)
        file_path = os.os.path.join(src, fileName)
        contentType = mimetypes.guess_type(file_path)
        contentType = contentType[0] if contentType[0] else contentType
        if os.os.path.exists(file_path):
            with open(file_path, 'rb') as fh:
                response = HttpResponse(fh.read(), content_type=contentType)
                response['Content-Disposition'] = 'inline; filename=' + os.os.path.basename(newFileName)
                result = response
        else:
            result = JsonResponse(loseMessage)
    except:        
