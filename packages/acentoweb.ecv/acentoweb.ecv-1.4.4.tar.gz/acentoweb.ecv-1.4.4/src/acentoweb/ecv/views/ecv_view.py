...
<DESCRIPTION LANG_REF=\"eng\">The main dataset in the ASL Signbank</DESCRIPTION>\n""" % datetime.datetime.now().isoformat()
        ...
        for item in self.get_items():
            ...
            CVE = CVE + """<CV_ENTRY_ML CVE_ID=\"{id}\" EXT_REF=\"signbank-ecv\"><CVE_VALUE DESCRIPTION=\"{description}\" LANG_REF=\"eng\">{title}</CVE_VALUE>"""   .format(id=eco_id, description=eco_description, title=eco_title)

            #import pdb; pdb.set_trace()
            #for field in [ {'name': 'observations', 'desc': 'Observations' } ]:
            #    fieldvalue = getattr(obj, field["name"]) or ''
            #    CVE = CVE + """<CVE_VALUE DESCRIPTION=\"{description}\" LANG_REF=\"eng\">{fieldvalue}</CVE_VALUE>"""  .format( description=field["desc"], fieldvalue=fieldvalue.replace("\"", "\'"))

            CVE = CVE + """</CV_ENTRY_ML>\n<<CV_ENTRY_ML CVE_ID=\"{id}\" EXT_REF=\"signbank-ecv\"><CVE_VALUE DESCRIPTION=\"{description}\" LANG_REF=\"eng\">{title}</CVE_VALUE></CV_ENTRY_ML>\n"""   .format(id=eco_id, description=eco_description, title=eco_title)
