...
class Bot(Client):
    ...
    def parse_command(self, command: str):
        ...
        if len(splited) < 2 or not splited[1].startswith('/'):
            return {}
    
        output = {
            "options": {},
            "ping statement": splited[0].strip(),
            "command": splited[1].lstrip('/').strip(),
        }
    ...
    def valid_command(self, resp: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        if parsed == {} or parsed["ping statement"].strip("@") != self.user.username:
            return (False, {"None": None})
        return (True, parsed)
