    ...
    with torch.inference_mode(inference):
        ...
        if not os.path.exists(dvector_path):
            nlp2.download_file(
                ...
                f'{Path(__file__).parent}')
        wav2mel = torch.jit.load(wav2mel_path).to(device)
        ...
        if not torch.is_tensor(wav_tensor):
            ...
            mel_tensor = wav2mel(wav_tensor.to(device), sample_rate)  # shape: (frames, mel_dim)
        else:
            mel_tensor = wav2mel(wav_tensor.to(device), sampling_rate)
...
def extract_x_vector(wav_path=None, wav_tensor=None, sampling_rate=16000, inference=True):
    ...
    with torch.inference_mode(inference):
        ...
        classifier = EncoderClassifier.from_hparams(source="speechbrain/spkrec-xvect-voxceleb",
                                                    savedir="pretrained_models/spkrec-xvect-voxceleb").to(device)
        if not torch.is_tensor(wav_tensor):
