...
> **Alacorder should automatically download and install missing dependencies upon setup, but you can also install them yourself with `pip`: `pandas`, `numpy`, `PyPDF2`, `openpyxl`, `xlrd`, `xlwt`, `xarray`, `numexpr`, `click`, and `bottleneck`. Recommended dependencies: `xlsxwriter`, `jupyter`, `tabulate`, `matplotlib`.**
