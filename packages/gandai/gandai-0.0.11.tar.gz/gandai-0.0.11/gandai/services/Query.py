...
def search_keys_query() -> pd.DataFrame:
    keys = ds.keys(f"searches/")
    return pd.DataFrame(dict(
        search_key = list(set([k.split("/")[1] for k in keys]))
    ))
    
def conflicts_query() -> pd.DataFrame:
    df = pd.read_feather("gs://ph-gandai-dev/global/dealcloud_conflict_domain.feather")
    return df