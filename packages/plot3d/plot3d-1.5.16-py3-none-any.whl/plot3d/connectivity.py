...
def find_matching_blocks(block1:Block,block2:Block,block1_outer:List[Face], block2_outer:List[Face],tol:float=1E-6,full_face_match:bool=False):  
    ...
    Args:
        ...
        full_face_match (bool, Optional): use full face matching to speed up the search. Do this only if you know you don't have any partial matches. 
    ...
    while match:
        ...
        for p in range(len(block1_outer)):
            ...
            for q in range(len(block2_outer)):
                ...
                df, split_faces1, split_faces2 = get_face_intersection(block1_face,block2_face,block1,block2,tol,full_face_match)
...
def get_face_intersection(face1:Face,face2:Face,block1:Block,block2:Block,tol:float=1E-6,full_face_match:bool=False):
    ...
    Args:
        ...
        full_face_match (bool): Use full face match for speed searching
    ...
    # General Search
    if full_face_match:
        combo = product([0,X1.shape[0]],[0,X1.shape[1]])
    else:
...
def connectivity_fast(blocks:List[Block],check_nearest:int=18,full_face_match:bool=False):
    ...
    Args:
        ...
        check_nearest (int): option to check the nearest N blocks by default this is 18 but if you set it to -1 then it will check with all blocks
        full_face_match (bool): Use full face matching to increase speed. Use this only if you dont have any partial matches. 
    ...
    # Find Connectivity 
    face_matches, outer_faces_formatted = connectivity(new_blocks,check_nearest,full_face_match)
    # scale it up
...
def connectivity(blocks:List[Block],check_nearest:int=18,full_face_match:bool=False):
    ...
    if check_nearest <0:
        combos = combinations_of_nearest_blocks(blocks,len(blocks)) # Find the 6 nearest Blocks and search through all that. 
    else:
        combos = combinations_of_nearest_blocks(blocks,check_nearest)
    ...
    for indx in t:     # block i        
        ...
        df_matches, blocki_outerfaces, blockj_outerfaces = find_matching_blocks(blocks[i],blocks[j],block_outer_faces[i],block_outer_faces[j],full_face_match=full_face_match)    # This function finds partial matches between blocks
