...
class DatasetClient:
    ...
    if we need annotations for that specific model """
    def download_annotations_for_project(self, project_id: str):
        self._dataset_sync_tool.download_project(project_id)
    ...
    @param project_id - id of collection
    if we need annotations for that specific model """
    def download_annotations_for_project_v2(self, project_id: str, status_list, is_annotated_only):
        self._dataset_sync_tool.download_project_v2(project_id, status_list, is_annotated_only)
