...
class AsyncWebClient(AsyncBaseClient):
    ...
    async def get(self, url, params=None, headers=None, origin=None):
        if origin is None: # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('GET', req.get('url'), headers=req.get('headers'), params=params, origin=origin)

    async def post(self, url, data, params=None, content=None, body=None, files=None, headers=None, origin=None):
        if origin is None:  # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('POST', req.get('url'), headers=req.get('headers'), data=data, body=body,
                               params=params, content=content, files=files, origin=origin)

    async def put(self, url, data, params=None, content=None, body=None, files=None, headers=None, origin=None):
        if origin is None:  # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('PUT', req.get('url'), headers=req.get('headers'), data=data, body=body,
                               params=params, content=content, files=files, origin=origin)

    async def patch(self, url, data, params=None, content=None, body=None, files=None, headers=None, origin=None):
        if origin is None:  # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('PATCH', req.get('url'), headers=req.get('headers'), data=data, body=body,
                               params=params, content=content, files=files, origin=origin)

    async def delete(self, url, params=None, headers=None, origin=None):
        if origin is None:  # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('DELETE', req.get('url'), headers=req.get('headers'), params=params, origin=origin)

    async def options(self, url, params=None, headers=None, origin=None):
        if origin is None:  # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('OPTIONS', req.get('url'), headers=req.get('headers'), params=params, origin=origin)

    async def head(self, url, params=None, headers=None, origin=None):
        if origin is None:  # Depends용
            origin = self._verify_origin()
        req = self.create_request(origin, url, headers)
        return await self.send('HEAD', req.get('url'), headers=req.get('headers'), params=params, origin=origin)

    async def send(self, method, url, headers, params=None, content=None, files=None, data=None, body=None, origin=None):
        async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
            req = client.build_request(method, url, headers=headers, params=params, content=content,
                                       files=files, json=data, data=body)
            try:
                ...
                if res.status_code != 200:
                    ...
                    if r and error_code == PLATFORM_TOKEN_EXPIRED_ERROR_CODE and origin and not self.open_api_token:
                        ...
                        self.send(method, url, headers, params=params, content=content,
                                  files=files, data=data, body=body)
                return res
