...
class Lamp:
    def __init__(self, key: str = 'GL', ip: str | None = None, port: int | None = None,
    ...
    def __send_request(self, *args, **kwargs):
        ...
        if self.enable_task_list and not self.__check_time(delay):
            print(1)
            if self.enable_task_list:
        ...
        for i in range(5):
            self.__sock.sendto(message.encode(), ('255.255.255.255', self.port))
    ...
    def __check_time(self, delay=0):
        return int(_time() * 1000) - self.__last_request_time > 500 + delay
