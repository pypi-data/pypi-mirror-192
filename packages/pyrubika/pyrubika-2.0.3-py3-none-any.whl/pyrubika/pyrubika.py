...
class rubika:
	...
	def sendSticker(self,guid,emoji,w_h_rati,sticker_id,file_id,access_hash,set_id):
		inData = {"method":"sendMessage","input":{"object_guid": guid,"rnd": f"{randint(100000,999999999)}","sticker": {"emoji_character": emoji,"w_h_ratio": w_h_rati,"sticker_id": sticker_id,"file": {"file_id": file_id,"mime": "png","dc_id": 32,"access_hash_rec": access_hash,"file_name": "sticker.png"},"sticker_set_id": set_id}},"client":SetClines.android}
		while True:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
			except:continue
