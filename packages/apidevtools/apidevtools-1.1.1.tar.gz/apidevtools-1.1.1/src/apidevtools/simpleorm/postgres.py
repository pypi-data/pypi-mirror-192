...
class PostgreSQL(BaseORM):
    ...
    async def select(self, query: str, args: tuple[Any, ...] = (), schema_t: SchemaType = dict, reldepth: int = 0) -> Records:
        async with self as connection:
            records = await connection.fetch(query, *args)
        records = Records(records, schema_t)
        if reldepth > 0 and schema_t is not dict:
            for index, record in enumerate(records.all()):
                for relation in record.relations():
                    ...
                    instances = (await self.select(query, args, relation.rel_schema_t, reldepth - 1)).all()
