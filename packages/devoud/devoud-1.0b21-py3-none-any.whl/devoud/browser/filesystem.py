...
class FileSystem:
    def __init__(self):
        ...
        self.__os_user_dir = {'linux': Path(f'{Path.home()}/.local/share/devoud/user'),
        ...
        self.__user_dir = self.__os_user_dir
        os.chdir(self.__root)  # сменить текущий каталог
        self.check_program_files()

        # добавить каталоги для ресурсов
    ...
    def create_launch_shortcut():
        ...
        make_shortcut(f'{root}/Devoud.py', name='Devoud', icon=icon[platform],
    ...
    def open_in_file_manager(path):
        command = {'win32': ["explorer", path],
                   'darwin': ["open", path],
                   'linux': ["xdg-open", path]}
        subprocess.Popen(command[platform])
