...
class ControlPage(EmbeddedView):
    ...
    def __init__(self, parent):
        ...
        self.project_site_button.clicked.connect(lambda: self.window().tab_widget.create_tab('https://codeberg.org/OneEyedDancer/Devoud', end=False))
        ...
        self.new_page_box.setCurrentText(self.settings.get('newPage'))
    ...
    def create_themes_buttons(self):
        for path, dirs, files in os.walk("./ui/themes"):
    ...
    def change_default_search(self):
        self.settings.set('searchEngine', self.search_box.currentText())
        self.window().address_panel.search_box.setCurrentText(self.settings.get('searchEngine'))
        self.window().new_page = self.new_page_dict.get(self.settings.get('newPage'))()

    def change_new_page(self):
        page = self.new_page_box.currentText()
        self.settings.set('newPage', page)
        self.window().new_page = self.new_page_dict.get(page)()
    ...
    def history_checkbox_command(self):
        if self.history_checkbox.isChecked():
            if QMessageBox.question(self.parent, 'Подтверждение операции', 'Это действие удалит всю вашу историю. Продолжить?') == QMessageBox.Yes:
    ...
    def cookie_checkbox_command(self):
        if self.cookie_checkbox.isChecked():
            if QMessageBox.question(self.parent, 'Подтверждение операции', 'Это действие удалит все текущие Cookie-файлы. Продолжить?') == QMessageBox.Yes:
    ...
    class SettingsCheckBox(QCheckBox):
        def __init__(self, parent, text, option, command=None, command_exec='after', restart: bool = False, tooltip=None):
            super().__init__(parent)
            self.setText(text)
    ...
    class HistoryListWidget(ContainerListWidget):
        ...
        def show_empty_list_info(self):
            if not self.window().settings.get('saveHistory'):
