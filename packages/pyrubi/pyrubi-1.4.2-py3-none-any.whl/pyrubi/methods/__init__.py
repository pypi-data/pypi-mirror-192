...
from ..connection.hand_shake import hand_shake
...
class methods:
    ...
    def __init__(self, auth):
        self.auth = auth
        self.hs = hand_shake(auth).handler
    ...
    def send_image(self, chat_id, file, caption = None, message_id = None, thumbnail = None):
        ...
        data = {
            'file_inline': {
                ...
                'file_id': req[0]['id'],
                'type':'Image',
                'file_name': file if not 'http' in file else f'pyrubi {randint(1, 100)}.png',
    ...
    def send_video(self, chat_id, file, caption = None, message_id = None):
        ...
        data = {
            'file_inline': {
                ...
                'dc_id': req[0]['dc_id'],
                'access_hash_rec': req[1],
                'file_name': file if not 'http' in file else f'pyrubi {randint(1, 100)}.mp4',
    ...
    def send_voice(self, chat_id, file, caption = None, message_id = None, time = None):
        ...
        data = {
            'file_inline': {
                ...
                'file_id': req[0]['id'],
                'type': 'Voice',
                'file_name': file if not 'http' in file else f'pyrubi {randint(1, 100)}.ogg',
                'size': tools.get_file_size(file),
    ...
    def send_music(self, chat_id, file, caption = None, message_id = None):
        ...
        data = {
            'file_inline': {
                ...
                'dc_id': req[0]['dc_id'],
                'access_hash_rec': req[1],
                'file_name': file if not 'http' in file else f'pyrubi {randint(1, 100)}.mp3',
                'width': 0,
                'height': 0,
    ...
    def send_file(self, chat_id, file, caption = None, message_id = None, mime=None):
        req = self.file_upload(file)
        metadata = tools.check_metadata(caption)
        data = {
            'file_inline': {
                ...
                'file_id': req[0]['id'],
                'type': 'File',
                'file_name': file if not 'http' in file else f'pyrubi {randint(1, 100)}.{mime or tools.get_mime_from_url(file)}',
                'size': tools.get_file_size(file),
                'mime': file.split('.')[-1] if not 'http' in file else mime or tools.get_mime_from_url(file),
                'access_hash_rec': req[1]
            },
            'object_guid': chat_id,
            'rnd': str(randint(100000,999999999)),
    ...
    def get_chat_join_link(self, chat_id):
    ...
    def upload_avatar(self, chat_id, file, thumbnai = None):
        main_id = self.file_upload(file)[0]['id']
        return self.method(
            'uploadAvatar',
            {
                'object_guid': chat_id,
                'thumbnail_file_id': self.file_upload(thumbnai)[0]['id'] if thumbnai != None else main_id,
    ...
    def request_file(self, file):
        print(tools.get_mime_from_url(file))
        return self.method(
            'requestSendFile',
            {
                'file_name': file if not 'http' in file else f'pyrubi library {randint(1, 100)}',
        ...
        while True:
            try:
                if len(bytef) <= 131072:
                    ...
                    return [req, j['data']['access_hash_rec']]
                else:
                    ...
                    return [req, p['data']['access_hash_rec']]
            except:
                 continue