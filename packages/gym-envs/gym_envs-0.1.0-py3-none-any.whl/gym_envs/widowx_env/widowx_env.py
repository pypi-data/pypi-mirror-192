...
FIXED_GOAL_COORDS_SPHERE = np.array([.14, -0.2, 0.26])
...
class WidowxEnv(gym.Env):
    ...
    def __init__(
        ...
        self.new_joint_positions = np.zeros(8)
        ...
        self.pybullet_action = np.zeros(8)
        ...
        self.pybullet_action_min = np.array([-0.05, -0.025, -0.025, -0.025,-0.025, -0.05, -0.01, -0.01]) * self.pybullet_action_coeff # old values
        self.pybullet_action_max = np.array([0.05, 0.025, 0.025, 0.025, 0.025, 0.05, 0.01, 0.01]) * self.pybullet_action_coeff # old values
        ...
        if self.joint_limits == "small":
            ...
            self.joint_min = np.array([-2.6, -1.5, -1.5, -1.7,-1.7, -2.6,-2.6, -0.05])
            self.joint_max = np.array([2.6, 0.8, 1.5, 1.7,1.7, 2.6,2.6, 0.05])
        elif self.joint_limits == "large":
            # self.joint_min = np.array([-3.2, -3.2, -3.2, -3.2,-3.2, -3.2, 0.015,0.015])
            # self.joint_max = np.array([3.2, 3.2, 3.2, 3.2,3.2, 3.2, 0.037,0.037])
            self.joint_min = np.array([-3.2, -3.2, -3.2, -3.2,-3.2, -3.2, 0.015,-0.037])
            self.joint_max = np.array([3.2, 3.2, 3.2, 3.2,3.2, 3.2, 0.037,-0.015])
        if self.obs_type == 1:
    ...
    def _get_joint_info(self):
        ...
        info = p.getJointStates(self.arm, [0,1,2,3,4,5,10,11])
        pos, vel, rf, t = [], [], [], []
    ...
    def step(self, action):
        ...
        self.gap = np.linalg.norm(self.joint_positions[6] - self.joint_positions[7])
        print(f'first position = {self.joint_positions[6]},second position = {self.joint_positions[7]}')
