...
#lookBackInDays = str
    ...
    def get_incidents(self, incidentId = 0, alertStatus=['New','InProgress', 'Resolved'], severity=[256, 128, 64, 32], pageIndex= 1, lookBackInDays = 60, pageSize = 50, sourceFilter=[]):
...
#                raise Exception("Reset")
        ...
        if incidentId:
            ...
            return [i for i in json.loads(response.text) if i["DetectionSources"][0] not in sourceFilter ]
...
#                                dbgPrint.info("Title: %s" % alertDisplayName)
                                alertIds.append({"AlertId": providerAlertId, "DisplayName": alertDisplayName})
