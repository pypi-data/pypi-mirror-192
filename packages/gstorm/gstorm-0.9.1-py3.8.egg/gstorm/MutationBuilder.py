...
class MutationBuilder():
    ...
    def compile_singular(self, data, children, batch=False, result_type: str = 'complete'):
        ...
        if result_type == 'complete':
            ...
            fields = ['id']
        typehints = get_type_hints(type(data))
        try:
        ...
        searchby = '' if self.Type not in [
            MutationType.UPDATE] else f'findBy: {{ {key}: "{data[key]}" }} {(type(data).__name__.lower())}: {{'
        # compiled result:
    ...
    def run(self, result_type: str = 'complete') -> dict:
        ...
        if type(_mutation) == str:
            ...
            # This check fixes a bug that crashed when the mutation was unsuccessful
            data['result'] = {} if not data.get(
                'result') else data.get('result')
            if type(self.data) == list:
                # ! BATCH MUTATION
