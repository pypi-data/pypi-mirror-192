...
def path(datas: list[str], gfa_style: GfaStyle) -> dict:
...
def walk(datas: list[str], gfa_style: GfaStyle) -> dict:
...
class Graph():
    ...
    def compute_networkx(self, node_prefix: str) -> MultiDiGraph:
        ...
        for node in self.segments:
            self.graph.add_node(
                f"{node_prefix}_{node.datas['name']}",
                title=f"Len. {str(node.datas['length'])}",
                color='darkslateblue',
                size=log10(node.datas["length"])
            )
        palette: list = get_palette(len(path_list := self.get_path_list()))
        if len(path_list) > 0:
            visited_paths: int = 0
            for visited_path in path_list:
                for i in range(len(visited_path.datas["path"])-1):
                    ...
                    self.graph.add_edge(
                        f"{node_prefix}_{left_node}",
                        f"{node_prefix}_{right_node}",
            ...
            for edge in self.lines:
                self.graph.add_edge(
                    f"{node_prefix}_{edge.datas['start']}",
                    f"{node_prefix}_{edge.datas['end']}",
                    color='darkred',
