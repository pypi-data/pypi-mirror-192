...
def gen_ip_image(ip, sim_job, fsoc_core_name, fsoc_core_flist_path):
    ...
    if sim_job.simulator == common.simulators_enum.VIVADO:
        timestamp_start = common.timestamp()
        cmp_log_file_path = cmp_viv_master_flist(ip, ip.vendor, ip.name, flist_path, sim_job, ip.is_local)
        if not sim_job.dry_run:
            timestamp_end = common.timestamp()
            errors = scan_cmp_log_file_for_errors(cmp_log_file_path, sim_job)
        if len(errors):
            common.error("Errors during compilation of IP '" + ip_str + "':")
            for error in errors:
                common.error("  " + error)
            sim.kill_progress_bar()
            common.fatal("Stopping due to compilation errors. Full log: " + cmp_log_file_path)
        log_cmp_history_ip(ip, cmp_log_file_path, sim_job, timestamp_start, timestamp_end)
        ...
        timestamp_start = common.timestamp()
        elab_log_file_path = elaborate_viv_master_flist(ip, sim_job, elab_out)
        if not sim_job.dry_run:
            timestamp_end = common.timestamp()
            errors = scan_elab_log_file_for_errors(elab_log_file_path, sim_job)
            if len(errors):
                common.error("Errors during elaboration of IP '" + ip_str + "':")
                for error in errors:
                    common.error("  " + error)
                sim.kill_progress_bar()
                common.fatal("Stopping due to elaboration errors. Full log: " + elab_log_file_path)
            log_elab_history(ip, elab_log_file_path, sim_job, timestamp_start, timestamp_end)
            ip.is_elaborated[sim_job.simulator] = True
            return cmp_log_file_path, elab_log_file_path
    else:
        timestamp_start = common.timestamp()
...
def cmp_viv_master_flist(ip, vendor, name, flist_path, sim_job, is_local):
    defines = sim_job.cmp_args
    sim_str = common.get_simulator_short_name(sim_job.simulator)
    arg_list = []
    cmp_args_list = convert_compilation_args(sim_job)
...
def elaborate_viv_master_flist(ip, sim_job, elab_out):
    pass
