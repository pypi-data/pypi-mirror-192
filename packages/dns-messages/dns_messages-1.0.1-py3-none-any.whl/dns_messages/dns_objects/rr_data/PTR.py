...
class PTR(ResourceRecord):
    ...
    def __str__(self) -> str:
        return 'PTR[domain_name={}]'.format(self.domain_name)

    def get_RR_type(self) -> RRType:
        return RRType.PTR

    @staticmethod
    def from_bytes(raw_bytes: bytes, raw_bits: List[int], byte_offset: int, rr_data_length: int, name: str, rr_class: RRClass, ttl: int) -> 'PTR':
        _, domain_name = parse_name(raw_bytes=raw_bytes, raw_bits=raw_bits, byte_index=byte_offset)
        return PTR(name=name, rr_class=rr_class, ttl=ttl, domain_name=domain_name)
