...
def deploy_template(tpl_file_path: str, dst_path: str, patcher: str, pdv: PdvType,
    ...
    if dst_file.startswith(MOVE_TPL_TO_PKG_PATH_NAME_PREFIX):
        dst_file = dst_file[len(MOVE_TPL_TO_PKG_PATH_NAME_PREFIX):]
...
def refresh_templates(pdv: PdvType, logger: Callable = print, **replacer: Replacer) -> Set[str]:
    ...
    for tpl_prj in cast(List[RegisteredTemplateProject], pdv_val(pdv, 'tpl_projects')):
        ...
        for tpl_file_path in [file for file in glb(joi(tpl_path, "**/.*")) + glb(joi(tpl_path, "**/*")) if isf(file)]:
            ...
            if dst_path.startswith(SKIP_IF_PORTION_DST_NAME_PREFIX):
                if is_portion:
                    continue
                dst_path = dst_path[len(SKIP_IF_PORTION_DST_NAME_PREFIX):]
            elif dst_path == '.' and bna(tpl_file_path).startswith(SKIP_IF_PORTION_DST_NAME_PREFIX) and is_portion:
                continue
...
def _check_types_linting_tests(pdv: PdvType):
    ...
    with _in_prj_dir_venv(project_path):
        ...
        cov_db = ".coverage"
        _cl(63, "python -m pytest --ignore-glob=**/templates/* --doctest-modules"
                f"{''.join([' -v' for arg in args if arg == '-v'])}"
                f" --cov-report html{''.join([' --cov=' + package for package in root_packages])}"
                f" {pdv_str(pdv, 'TESTS_FOLDER')}/ {' '.join(root_packages)}")
