...
def define_reference_load_pos(h_high,h_mudline):
    command ='''
    ...
    structure node initialize displacement-x 0.0
    structure node initialize displacement-rotational-x 0.0
    structure node initialize velocity-x 0.0
    structure node initialize velocity-rotational-x 0.0
    structure node initialize displacement-y 0.0
    structure node initialize displacement-rotational-y 0.0
    structure node initialize velocity-y 0.0
    structure node initialize velocity-rotational-y 0.0
    structure node initialize displacement-z 0.0
    structure node initialize displacement-rotational-z 0.0
    structure node initialize velocity-z 0.0
    structure node initialize velocity-rotational-z 0.0
    '''
    it.command(command)

    command = '''
    structure node group 'loading_ref' slot 'reference' range position-z {} group 'Skirt' slot 'SC'
    ...
    '''.format(h_mudline,h_high,h_high,h_high,h_high,h_high,h_high)
    it.command(command)
