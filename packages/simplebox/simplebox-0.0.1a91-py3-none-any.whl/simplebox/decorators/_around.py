...
def __do_around(func: Callable, args: Tuple = None, kwargs: Dict = None, opts: Dict = None):
    before: List[Callable] or Callable = opts.get("before")
    after: List[Callable] or Callable = opts.get("after")
    catch: bool = opts.get("catch")
    args_ = () if args is None else args
    kwargs_ = {} if kwargs is None else kwargs
    call_back_func_kwargs = func_full_args(func, args, kwargs)
    _run_hook_func(before, call_back_func_kwargs)
    ...
    try:
        ...
        _run_hook_func(after, call_back_func_kwargs)
