...
def _configure_dapr_app(_app, function, ctx: FunctionContext):
    ...
    if inputs is not None:
        for input in inputs.values():
            inputTypeList = str(input.componentType).split(".")
            if len(inputTypeList)>2:
                raise InvalidFuncContextInputType(
                    "Invalid input componentType {}".format(input.componentType)
                )
            type = inputTypeList[0]
            component = inputTypeList[1]
            if type == _DAPR_BINDINGS:
                _app.binding(input.componentName)(function)
                print("binding {}".format(input.componentName))
            elif type == _DAPR_PUBSUB:
                _app.subscribe(topic=input.uri, pubsub_name=input.componentName)(function)
                print("sub topic {}, pubsub name {}".format(input.uri, input.componentName))
            else:
                raise InvalidFuncContextInputType(
                    "Invalid input componentType {}".format(input.componentType)
                )
        ...
        if not self.app:
            ...
            if  ff == "":
                ...
                if _ctx.runtime == KNATIVE_FUNC:
                    print("start http app")
                    self.app = create_app(self.target, self.source, self.signature_type)
                elif str(_ctx.runtime).lower() == ASYNC:
                    signature_type = "of"
                    print(datetime.now())
