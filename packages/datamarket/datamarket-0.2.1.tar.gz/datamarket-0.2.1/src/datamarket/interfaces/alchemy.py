    ...
    def get_schema_from_table(table):
        if isinstance(table.__table_args__, tuple):
            ...
            if "schema" in table.__table_args__:
                return table.__table_args__["schema"]
    ...
    def create_tables(self, tables):
        for table in tables:
            ...
            with self.engine.connect() as conn:
                ...
                if not self.engine.dialect.has_table(conn, table.__tablename__, schema=schema):
                    logger.info(f"creating table {table.__tablename__}...")
                    conn.execute(DDL(f"CREATE TABLE {schema}.{table.__tablename__}"))
    ...
    def drop_tables(self, tables):
        for table in tables:
            ...
            with self.engine.connect() as conn:
                ...
                if self.engine.dialect.has_table(conn, table.__tablename__, schema=schema):
