...
class 〇字段列表(〇字段):
    ...
    >>> authors = 〇字段列表(StringField('Name', [validators.DataRequired()]))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(标签, 验证器々, 默认值=默认值, **kwargs)
        if self.filters:
            raise TypeError(
        ...
        self.unbound_field = 未绑定字段
        self.min_entries = 最小条目数
        self.max_entries = 最大条目数
        self.last_index = -1
        self._prefix = kwargs.get("_prefix", "")
        self._separator = separator
        self._field_separator = 未绑定字段.kwargs.get("separator", "-")
