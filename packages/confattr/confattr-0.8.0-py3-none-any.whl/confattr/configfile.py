...
class Message:
	...
	def __repr__(self) -> str:
		return f'{type(self).__name__}(%s)' % ', '.join(f'{a}={getattr(self, a)!r}' for a in self.__slots__)
...
class ConfigFile:
	...
	def parse_line(self, line: str) -> None:
		...
		self.parse_splitted_line(ln_splitted)
	...
	def parse_splitted_line(self, ln_splitted: 'Sequence[str]') -> None:
...
class Include(ConfigFileArgparseCommand):
	...
	def init_parser(self, parser: ArgumentParser) -> None:
		parser.add_argument('path', help='The config file to load. Slashes are replaced with the directory separator appropriate for the current operating system.')
