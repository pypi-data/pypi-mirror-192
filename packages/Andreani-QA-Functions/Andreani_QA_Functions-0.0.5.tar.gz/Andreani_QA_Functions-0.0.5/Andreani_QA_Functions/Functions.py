...
import Andreani_QA_parameters.Encriptor as Encriptor
...
PATH_FUNCTIONS = os.path.join(Parameters.Parameters.current_path, 'functions')
...
class Functions(Parameters):
    global_date = time.strftime(Parameters.Parameters.date_format)  # formato aaaa/mm/dd
    ...
    usuario_pybot_email = Encriptor.Encriptor.get_password_from_xml_file_encrypted(Encriptor, "CLAVES", "id", "Email de Pybot", "USER")
    email_pybot = Encriptor.Encriptor.get_password_from_xml_file_encrypted(Encriptor, "CLAVES", "id", "Email de Pybot", "USER")
    password_sharepoint = ""
    ...
    def set_proyect(self, project_name=None):
        ...
        try:
            ...
            if not Parameters.Parameters.manual_increment:
        ...
        if Parameters.Parameters.environment == "Windows":
            self.path_downloads = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\downloads"
            self.path_evidences = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\inputs"
            self.path_files = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\files"
            self.path_images = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\outputs"
            self.path_output = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\outputs"
            self.path_json = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\pages"
            self.path_resource = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\src\\resources"
            self.path_config = f"{Parameters.Parameters.current_path}\\projects\\{str(self.project_name)}\\config.yml"
            self.path_jmeter_executor = Parameters.Parameters.path_jmeter
        ...
        if os.environ.get("env"):
            ...
            Parameters.env = "DataTest"
        Functions.full_read_excel(self, None, Parameters.Parameters.env)
    ...
    def set_retry(self, numbers_retries):
        ...
        print(f"La cantidad de reintentos configurada es {Parameters.Parameters.number_retries}.")
    ...
    def get_file_name_stored():
        ...
        return Parameters.Parameters.file_name_stored
    ...
    def get_path_system():
       ...
       """
        ...
        return Parameters.Parameters.current_path
    ...
    def get_row_excel():
        ...
        print(f"El numero del registro consultado es: {Parameters.Parameters.row} .")
        return Parameters.Parameters.row
    ...
    def set_increment_row():
        ...
        Parameters.Parameters.row += 1
        Parameters.manual_increment = True
        print(f"El numero del registro fue aumentado a: {Parameters.Parameters.row}")
    ...
    def set_restore_excel_row():
        ...
        Parameters.row = 2
        print(f"El numero del registro fue restaruado a: {Parameters.Parameters.row}")
    ...
    def set_timeout_base_sql_server(self, time_seconds):
        ...
        time_timeout = Parameters.Parameters.timeout_base_sql_server
    ...
    def establish_connection_sqlserver(self, server, base, user, password):
        ...
        try:
            ...
            if conection is not None:
                conection.timeout = Parameters.Parameters.timeout_base_sql_server
                return conection
    ...
    def send_message_teams(channel, title, message=None, sections=None):
        ...
        message_teams.color(Parameters.Parameters.teams_notifications_colors)
    ...
    def footer_signature_teams():
        ...
        signature = f'''<footer>
                          <p style="color: {Parameters.Parameters.teams_focus_test_colors}"><strong>Equipo Pybot</strong></p>
    ...
    def create_table(list_data_head: list, list_data_content: list, format_table="HTML"):
        ...
        if format_table == "HTML":
            ...
            for row in list_data_head:
                table_head_html = f'{table_head_html}<th style="border: 1px solid gray; padding: 1rem 2rem 1rem 2rem; background-color: {Parameters.Parameters.teams_notifications_colors}; text-align: center; color: white;">{row}</th>'
            table_head_html = f'<tr>{table_head_html}</tr>'
            for rows in list_data_content:
                for col in rows:
    ...
    def full_read_excel(self, file_name=None, specific_sheet=None, test_cases_name_list=None):
        ...
        try:
            ...
            if not Parameters.Parameters.manual_increment:
                ...
                self.data_resource = data_global_resource[int(Parameters.Parameters.row)-1]
    ...
    def exception_logger(exception):
        ...
        if Parameters.Parameters.loggin_exceptions:
            print(exception)
