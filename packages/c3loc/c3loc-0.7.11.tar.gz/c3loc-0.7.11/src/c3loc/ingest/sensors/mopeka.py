...
class Mopeka:
    ...
    def tank_process(tank1, tank2, temp):
        ...
        raw = (tank2 * 256) + tank1
        raw = raw & 0x3fff
        tank_height = int(raw * (0.573045 + (-0.002822 * temp) + (-0.00000535 * temp**2)))
        tank_confidence = (tank2 >> 6) & 0x3
        return (tank_height, tank_confidence)
    
    @classmethod
    def from_bytes(cls, b: bytes):
        (batt, temp, tank1, tank2, sensor_id) = struct.unpack("<BBBB3s", b[5:12])
        batt = (batt & 0x7f) / 32
        temp = temp & 0x7f
    ...
    def to_sensor(self):
        return [(SensorType.BatteryPercent, self.battery_pct().to_bytes(1, 'little')),
                (SensorType.Temperature, (self.temperature*10).to_bytes(2, 'little', signed=True)),
                (SensorType.TankHeight, self.tank_fill_mm.to_bytes(2, 'little')),
                (SensorType.TankConfidence, self.tank_quality.to_bytes(1, 'little'))]
