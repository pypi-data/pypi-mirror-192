...
class ListenerProtocol(object):
    ...
    async def process_mopeka(self, data, conn):
        ...
        (db_id,) = await conn.fetchrow(
                'INSERT into tags (bid, type, mac) '
                'VALUES ($1, \'MacBeacon\', $2) '
                'ON CONFLICT ON CONSTRAINT tags_bid_key '
                'DO UPDATE SET last_seen = (now() at time zone \'utc\') '
                'RETURNING id', m.sensor_id, mac_addr)
    ...
    async def process_alertwet(self, b, conn):
        ...
        row = await conn.fetchrow(
            'INSERT into tags '
            '(bid, type, battery_pct, alarm_active, attrs, last_seen, zone_id, distance) '
            'VALUES ($1, \'AlertWet\', $2, $3, $4, (now() at time zone \'utc\'), $5, $6) '
        ...
        await conn.fetch(
            'INSERT into log (tag_id, zone_id, listener_id, distance, variance, reason)'
            'VALUES ($1, $2, $3, $4, $5, $6)',
