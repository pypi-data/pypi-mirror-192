...
async def get_folder(path: str, request: Request) -> Folder:
    ...
    full_path = Path(request.app.settings.project_root) / path
    relative_path = full_path.relative_to(request.app.settings.project_root)

    name = full_path.name
    files = [
        {
            "name": f.name,
            "path": str(f.relative_to(request.app.settings.project_root)),
