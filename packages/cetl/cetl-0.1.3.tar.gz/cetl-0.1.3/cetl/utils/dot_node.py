...
def init_nodes_text(_nodename2transformer):
    temp_dict={}
...
def add_dot_node(DataPipeline, transformers_set):
    self=DataPipeline
    ...
    for nodename in transformers_set:
        if "parallelTransformer" in nodename:
            ...
            ############### description text
            append_description_node_text(self.nodes_text, nodename, description, self.limit)
            ############### nodename text
            append_node_text(self.nodes_text, nodename, f"Task ID: {nodename}", self.limit, apply_limit=True)
            ############### Input text
            input = transformer.total_input
            input_textline = f"Input: {input:.0f} records"
            append_node_text(self.nodes_text, nodename, input_textline, self.limit, apply_limit=True)
            ############### Output text
            output = transformer.total_output
            output_textline = f"Output: {output:.0f} records"
            append_node_text(self.nodes_text, nodename, output_textline, self.limit, apply_limit=True)
            # get the time of execution
            total_time = transformer.total_time
            total_time_textline = f"Take time: {total_time:.4f} seconds"
            append_node_text(self.nodes_text, nodename, total_time_textline, self.limit, apply_limit=True)
