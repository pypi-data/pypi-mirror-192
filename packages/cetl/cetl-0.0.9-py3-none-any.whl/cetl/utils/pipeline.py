...
class DataPipeline:
    ...
    def __init__(self, cfg):
        ...
        self.limit = 25 #text line limit of characters in node
        self.nodes_text = self.init_nodes_text()
    ...
    def init_nodes_text(self):
        temp_dict={}
        for key in self._nodename2transformer.keys():
            temp_dict[key]=""
    ...
    def append_node_text(self, node_name, append_text):
        self.nodes_text[node_name] = self.nodes_text[node_name]+append_text

    def get_node_text(self, node_name):
        return self.nodes_text[node_name]
    ...
    def generate_node_text(self, 
        ...
        if not description:
            self.append_node_text(node_name, f"no description\l{sep_line}{sep_line}\l")
        else:
            self.append_node_text(node_name, f"{description}\l{sep_line}{sep_line}\l")


        ####################### node_name
        self.append_node_text(node_name, f"Node name: {node_name}\l")
        ...
        self.append_node_text(node_name, f"Input: {input_text}\l")

        ####################### output
        if len(output)>limit:
            output = output[:limit]+"..."
        self.append_node_text(node_name, f"Output: {output}\l")
    ...
    def create_dot_graph(self):
        ...
        for nodename in transformers_set:
            if "parallelTransformer" in nodename:
                ...
                transformer = self._nodename2transformer[nodename]
                ...
                self.generate_node_text(nodename, description, inputs, output, self.limit) #basic text

                # get the time of execution
                total_time = transformer.total_time
                total_time_textline = f"Take time: {total_time:.4f} seconds"
                if len(total_time_textline)>self.limit:
                    total_time_textline = total_time_textline[:self.limit]+"..."
                self.append_node_text(nodename, f"{total_time_textline}\l")
                
                
                self._dot.node(nodename, self.nodes_text[nodename])
