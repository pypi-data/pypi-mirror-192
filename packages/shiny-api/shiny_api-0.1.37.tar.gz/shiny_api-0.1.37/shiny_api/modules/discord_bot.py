...
"Print label.  If it starts with a number, print that quantity."


@bot_client.command(name="reload")
async def reload(ctx):
    await ctx.send("Reload...")
    await bot_client.tree.sync()


@commands.hybrid_command(name="label", with_app_command=True)
@commands.is_owner()
async def label(context: commands.Context, command: Optional[str]):
    if len(command[0]) == 0:
        return
    quantity = 1
    date_bool = True
