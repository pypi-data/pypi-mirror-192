...
def print_text(text: List[str] = None, barcode: str = None, text_bottom: str = None, quantity: int = 1, print_date: bool = True):
    ...
    if len(barcode) > 0 and barcode.isdigit():
        centered_left = 15 - (len(barcode))
        label.origin(x=centered_left, y=current_origin)
        label.barcode(barcode_type="2", code=barcode, height=BARCODE_HEIGHT)
        label.endorigin()
        current_origin = current_origin + (BARCODE_HEIGHT / 9) + 2
    elif len(barcode) > 0:
        centered_left = 8 - (len(barcode))
        label.origin(x=centered_left, y=current_origin)
        label.barcode(barcode_type="3", code=barcode, height=BARCODE_HEIGHT)
        label.endorigin()
        current_origin = current_origin + (BARCODE_HEIGHT / 9) + 2
