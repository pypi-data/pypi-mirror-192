...
class Experiment(abc.ABC, pydantic.BaseModel):

    rig: np_config.Rig = np_config.Rig()
    "Computer hostnames and configuration for the rig we're currently running on."
    services: tuple[Service, ...]
    "Devices, databases etc. Order will be preserved when called, but not necessarily called altogether."
    config: dict[str, dict[str, Any]]
    "Top-level keys include names of Services. Each Service then has a config dict, applied with `apply_config_to_services`"
...
class WithLims(Experiment):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initializing {__class__.__name__}({labtracks_mouse_id}, {lims_user_id})"
        )
    ...
    def generate_ecephys_session(
        ...
        return np_session.lims.generate_ecephys_session(
            mouse=labtracks_mouse_id, user=lims_user_id
        )
    ...
    def session(self, value: str | np_session.lims.SessionInfo):
        if not isinstance(value, np_session.lims.SessionInfo):
            value = np_session.Session(value)
        self._session = value

    @property
    def mouse(self) -> np_session.MouseInfo:
        return self._mouse
