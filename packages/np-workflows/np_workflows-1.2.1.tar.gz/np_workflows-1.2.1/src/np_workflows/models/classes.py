...
class NpUltra(Experiment):
    ...
    stimulus_services: tuple[Service, ...] = (NoCamstim,)
    ...
    def __init__(self, labtracks_mouse_id) -> None:
        ...
        self.config: dict[str, dict[str, Any]] = (
            ...
            | np_config.from_zk(
                f"/projects/np_workflows/{self.__class__.__name__}/configuration"
            )
            | np_config.from_zk(f"/rigs/NP.{self.rig_idx}")
        )
        self.session = (
            f"{labtracks_mouse_id}_{datetime.datetime.now().strftime('%Y%m%d')}"
        )
    ...
    def initial_services(self) -> tuple[Service, ...]:
        return tuple(
            set(self.services)
            - set(self.stimulus_services)
            - set(self.photodoc_services)
        )
...
class Pretest(WithLims):
    ...
    stim_services: tuple[Service, ...] = (NoCamstim,)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.debug(
            f"{__class__.__name__} modifying mouse ID to use {labtracks_mouse_id} for pretest"
        )
        super().__init__(
            labtracks_mouse_id, lims_user_id, lims_session_id, *args, **kwargs
        )
