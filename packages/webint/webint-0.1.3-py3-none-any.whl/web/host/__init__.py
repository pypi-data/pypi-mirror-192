...
class Machine(MachineBase):
    ...
    def setup_app(self, protected_path, package):
        """Install administrative interface."""
        self.run("/root/python/bin/python3 -m venv app")
        self.run(f"/root/runinenv app pip install {package}")
        # TODO put web.host.admin:app behind supervisor
        # TODO move nginx conf to conf.d's
        #   TODO replace initial IP-site conf.d with admin server
        # XXX self.run(
        # XXX     f"cat > {self.torrc}",
        # XXX     stdin=textwrap.dedent(
        # XXX         f"""\
        # XXX         HiddenServiceDir {self.tor_data}/main
        # XXX         HiddenServicePort 80 127.0.0.1:80

        # XXX         # leave this here for cat'ing over SSH..."""
        # XXX     ),
        # XXX )
