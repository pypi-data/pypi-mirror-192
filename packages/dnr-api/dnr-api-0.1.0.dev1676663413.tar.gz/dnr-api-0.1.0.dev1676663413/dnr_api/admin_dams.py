...
def _make_url(route: str) -> str:
    return Config.make_url(f"/api/v1/AdminDams/{route}")
...
def get_dam(dam_id: int) -> dict[str, Any]:
    ...
    dam_result : dict[str, Any]
        A dam result object
    """
    return get_detail_view(_make_url(f"Dam?id={dam_id}"), Config.dnr_token())
...
def get_all_dams(progress_bar: bool = True) -> pd.DataFrame:
    ...
    return get_paginated_list_view(
        _make_url("AllDams?page=1"), progress_bar, Config.dnr_token()
    )
