def TXT_create_Conv_Net(text_train, text_test, WIN_SIZE = 1000, WIN_HOP = 100, CLASS_COUNT = 6, vocab_size_list = [1000, 5000], embedding_subsets = False, BoW_subsets = True,
                        ...
                        if_create_dataset_in_bot = 0, lr_list = [0.0001, 0.001, 0.005, 0.002], opt_list = [tf.optimizers.Adam, tf.optimizers.RMSprop, tf.optimizers.SGD, tf.optimizers.AdamW, tf.optimizers.Adadelta, tf.optimizers.Adagrad, tf.optimizers.Adafactor, tf.optimizers.Nadam, tf.optimizers.Adamax, tf.optimizers.Ftrl]):
