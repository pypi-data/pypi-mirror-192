...
from utilities.pomo.valuation.isone.constraints import ExpressionsBlock
import logging
...
class ValuationOptimizerISONE(optimizer.Optimizer):
    """A framework wrapper class for creating Pyomo ConcreteModels for valuating energy storage in ISONE electricity market."""
    ...
    def _process_results(self):
        ...
        if self.market_type == "pfp":
            ...
            rev_reg = np.cumsum(
                np.array(
                    [
                        (m.mi_mult[t] * m.price_reg_service[t] + m.price_regulation[t])
                        * m.perf_score[t]
                        * m.q_reg[t].value
                        for t in m.time
                    ]
