...
from utilities.pomo import optimizer
from utilities.pomo.valuation.miso.constraints import ExpressionsBlock
import logging
...
class ValuationOptimizerMISO(optimizer.Optimizer):
    """A framework wrapper class for creating Pyomo ConcreteModels for valuating energy storage in MISO electricity market."""
    ...
    def _set_model_param(self):
        ...
        if not hasattr(m, "Power_rating"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No Power_rating provided, setting default..."
            )
            m.Power_rating = 20

        if not hasattr(m, "R"):
            # Discount/interest rate [hour^(-1)].
            logging.debug("ValuationOptimizerMISO: No R provided, setting default...")
            m.R = 0

        if not hasattr(m, "Energy_capacity"):
            # Energy capacity [MWh].
            logging.debug(
                "ValuationOptimizerMISO: No Energy_capacity provided, setting default..."
            )
            m.Energy_capacity = 5
        ...
        if not hasattr(m, "Self_discharge_efficiency"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No Self_discharge_efficiency provided, setting default..."
            )
            m.Self_discharge_efficiency = 1.00
        elif getattr(m, "Self_discharge_efficiency") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: Self_discharge_efficiency provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        if not hasattr(m, "Round_trip_efficiency"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No Round_trip_efficiency provided, setting default..."
            )
            m.Round_trip_efficiency = 0.85
        elif getattr(m, "Round_trip_efficiency") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: Round_trip_efficiency provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        if not hasattr(m, "Reserve_reg_min"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No Reserve_reg_min provided, setting default..."
            )
            m.Reserve_reg_min = 0
        elif getattr(m, "Reserve_reg_min") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: Reserve_reg_min provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        if not hasattr(m, "Reserve_reg_max"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No Reserve_reg_max provided, setting default..."
            )
            m.Reserve_reg_max = 0
        elif getattr(m, "Reserve_reg_max") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: Reserve_reg_max provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        if not hasattr(m, "State_of_charge_min"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No State_of_charge_min provided, setting default..."
            )
            m.State_of_charge_min = 0
        elif getattr(m, "State_of_charge_min") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: State_of_charge_min provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        if not hasattr(m, "State_of_charge_max"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No State_of_charge_max provided, setting default..."
            )
            m.State_of_charge_max = 1
        elif getattr(m, "State_of_charge_max") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: State_of_charge_max provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        if not hasattr(m, "State_of_charge_init"):
            ...
            logging.debug(
                "ValuationOptimizerMISO: No State_of_charge_init provided, setting default..."
            )
            m.State_of_charge_init = 0.5
        elif getattr(m, "State_of_charge_init") > 1.0:
            logging.warning(
                "ValuationOptimizerMISO: State_of_charge_init provided is greater than 1.0, interpreting as percentage..."
            )
        ...
        try:
            ...
            m.fraction_reg_up = np.array(
                [m.fraction_reg_up] * len(m.price_electricity)
            )
        ...
        try:
            if not getattr(m, "perf_score", None):
                logging.debug(
                    "ValuationOptimizer: No perf_score provided, setting default..."
                )
                m.perf_score = 0.95
        except ValueError:  # perf_score is array-like
            if np.isnan(m.perf_score).any():
                logging.debug(
                    "ValuationOptimizer: perf_score array-like provided has None values, setting default..."
                )
                m.perf_score = 0.95
        ...
        try:
            ...
            logging.warning(
                "ValuationOptimizer: A perf_score array was provided but is shorter than the price_electricity array."
            )
            raise (
                IncompatibleDataException(
                    "ValuationOptimizer: There was a mismatch in array sizes between perf_score and price_electricity."
                )
            )
