...
from utilities.pomo.valuation.nyiso.constraints import ExpressionsBlock
import logging
...
class ValuationOptimizerNYISO(optimizer.Optimizer):
    """A framework wrapper class for creating Pyomo ConcreteModels for valuating energy storage in NYISO electricity market."""
            ...
            rev_reg = np.cumsum(
                np.array(
                    [
                        ...
                        * (1 - 1.1 * (1 - m.perf_score[t]))
                        + m.price_electricity[t]
                        * (
                            m.q_reg[t].value * m.fraction_reg_up[t]
