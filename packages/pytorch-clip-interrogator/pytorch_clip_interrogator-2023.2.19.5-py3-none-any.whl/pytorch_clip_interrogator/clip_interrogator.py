...
class CLIPInterrogator:
    ...
    def __call__(
        ...
        # interrogate
        medium = self.vocab.mediums(image_features, 1)
        artist = self.vocab.artists(image_features, 1)
        movement = self.vocab.movements(image_features, 1)
        flaves = ", ".join(self.vocab.flavors(image_features, max_flavors))

        if caption.startswith(medium):
            prompt = f"{caption}, {artist}, {movement}, {flaves}"
        else:
            prompt = f"{caption}, {medium} {artist}, {movement}, {flaves}"
    ...
    def _image_to_features(self, image: PIL.Image.Image) -> np.ndarray:
