...
class CommandSerializer:
    ...
    def encode(self, root_command, connection_type, tcp_address=None, runtime_version=0):
        queue: Deque[PythonCommand] = deque()
        queue.append(root_command)
        self.insert_into_buffer([root_command.target_runtime.value, runtime_version])
        ...
        self.insert_into_buffer([RuntimeLib.python.value, root_command.command_type.value])
    ...
    def serialize_recursively(self, queue):
        ...
        if len(command.get_payload()) > 0:
            if isinstance(command.get_payload()[0], PythonCommand):
