...
class GeneratorHandler:
    ...
    def analyze(self, package_path):
        ...
        structured_command_for_class = PythonCommand(RuntimeLib.python, PythonCommandType.GenerateLib, [])

        while len(handlers.types_to_analyze) > 0:
            structured_command_for_class = structured_command_for_class. \
                add_arg_to_payload(handlers.GENERATOR_HANDLER[PythonCommandType.GetType].
    ...
    def generate(self, command, file_path):
        ...
        for i in range(0, len(command.get_payload())):
            ...
            if isinstance(command.get_payload()[i], PythonCommand):
