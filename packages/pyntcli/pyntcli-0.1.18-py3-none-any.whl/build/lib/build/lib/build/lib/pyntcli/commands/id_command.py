...
def pyntid_usage():
    return ui_thread.PrinterText(*ui_thread.pynt_header()) \
        .with_line(*ui_thread.pynt_version()) \
        .with_line("") \
        .with_line("View your pynt-id to use when running pynt in CI pipeline") \
        .with_line("") \
...
class PyntShowIdCommand(sub_command.PyntSubCommand): 
    ...
    def run_cmd(self, args: argparse.Namespace):
        ...
        ui_thread.print((json.dumps(open(creds_path, "r").read(), indent=4)))
        time.sleep(1)
