...
def stackTodb_flipcart(dataFrame, dbTableName, skima):
    print(dataFrame)
...
# CSV Cleaning
def csv_cleaning_flipcart(csvFile, csv_folder_cleaned):
...
def csvCleaner_flipcart(csv_folder, csv_folder_cleaned):
    fileList = os.listdir(csv_folder)

    for i in range(len(fileList)):
        csv_cleaning_flipcart(csv_folder + "\\" + fileList[i], csv_folder_cleaned)
        print(fileList[i] + " is cleaned")

# CSV Encoding
def csv_encoding_flipcart(csvFile, csv_folder_encoding):
...
def csvEncoder_flipcart(csv_folder, csv_folder_encoding):
    fileList = os.listdir(csv_folder)

    for i in range(len(fileList)):
        csv_encoding_flipcart(csv_folder + "\\" + fileList[i], csv_folder_encoding)
        print(fileList[i] + " is encoded")

def commonPreprocess_flipcart(dataframe):
...
def titleSummaryMaker_flipcart(csv):
    pool = pd.read_csv(csv, sep=',', engine='python', encoding='latin-1')
...
def csvImporter_flipcart(csv_folder, csv_folder_encoding, csv_folder_cleaned, db_table_name):
    column_name = ["retailer_id", "retailer_name", "retailer_sku", "upc", "model_number", "title", "brand", "category", "subcategory", "week_id", "week_ending", "retail_sales", "units_sold", "retail_price"]
    ...
    for i in range(len(fileList)):
        ...
        stackTodb_flipcart(dataframe, db_table_name)
        

def addPoolColumn_flipcart(dataframe, keyword_pool):

    pool = keywordPool_flipcart(keyword_pool)
    title_mapping = titleSummaryMaker_flipcart(keyword_pool)
...
def csvPreprocess_flipcart(csv_folder, csv_folder_cleaned, csv_folder_encoding):

    csvEncoder_flipcart(csv_folder, csv_folder_encoding)
    csvCleaner_flipcart(csv_folder_encoding, csv_folder_cleaned)
...
def csvImporter_NEW_flipcart(csv_folder_cleaned, keyword_pool, skima):
    column_name = ["retailer_id", "retailer_name", "retailer_sku", "upc", "model_number", "title", "brand", "category", "subcategory", "week_id", "week_ending", "retail_sales", "units_sold", "retail_price"]
    ...
    for i in range(len(fileList)):
        ...
        if "Laptops" in fileList[i]:
            ...
            # commonPreprocess(dataframe)
            print("-----No Pool Check-----")
            stackTodb_flipcart(dataframe, db_table_name, skima)
        ...
        if "Watches" in fileList[i]:
            ...
            print("-----Pool Check-----")
            stackTodb_flipcart(addPoolColumn_flipcart(dataframe, keyword_pool), db_table_name, skima)
...
def keywordCheck_flipcart(csv_folder_cleaned, pool_check_folder, keyword_pool):
    column_name = ["retailer_id", "retailer_name", "retailer_sku", "upc", "model_number", "title", "brand", "category", "subcategory", "week_id", "week_ending", "retail_sales", "units_sold", "retail_price"]
