...
class CustomJsonFormatter(jsonlogger.JsonFormatter):
    def add_fields(self, log_record, record, message_dict):
        ...
        print(f"ENABLE_CAPTURE_REQUEST:{os.environ.get(ENABLE_CAPTURE_REQUEST)}")
        print(f"ENABLE_CAPTURE_RESPONSE:{os.environ.get(ENABLE_CAPTURE_RESPONSE)}")
...
def init(path, logger) -> loguru_logger:
    ...
    return logger.opt(lazy=True).opt(colors=False).opt(raw=True)
