...
from sunpeek.core_methods import VerifyValidateMode
...
class Component(AttrSetterMixin):
    ...
    def map_sensor(self, sensor: Sensor, slot_name: str):
        ...
        if sensor is not None and slot_name in real_slots:
            ...
            if slot_name in self.sensors:
                del self.sensors[slot_name]._mappings[
                    self.sensors[slot_name]._mappings.index(self.sensor_map[slot_name])]
    ...
    def map_vsensor(self, slot_name: str, can_calculate: bool = True):
        ...
        assert self.has_virtual_slot_named(slot_name), \
            f'Cannot map virtual sensor because slot {slot_name} of {self} does not accept virtual sensors.'
        try:
        ...
        # Create virtual sensor and map to slot
        # Sensor needs to store compatible_unit of sensor_type, so it can later check if vsensor calc results are ok.
        vsensor = Sensor(is_virtual=True,
                         can_calculate=can_calculate,
                         plant=self.plant,
                         raw_name=f"{slot_name}___virtual_{uuid.uuid4().hex}",
