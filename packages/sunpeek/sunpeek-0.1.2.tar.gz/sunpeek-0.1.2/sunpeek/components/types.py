...
# from api import models, base_model
...
# class SensorType(ORMBase, AttrSetterMixin):
#     """
#     Holds the definition of sensor types, such as ambient temperature, global radiation etc. Including uncertainties for
#     different uncertainty classes and installation conditions.
#
#     Attributes
#     ----------
#     name : str
#     description : str
#     compatible_unit_str : str
#         String of a physical unit to which the sensor type values must be compatible with.
#     lower_replace_* : Quantity
#         Lower replacement interval: All measurements between _min and _max are replaced by _value. All measurements
#         lower than _min are set to NaN.
#     upper_replace_* : Quantity
#         Upper replacement interval: All measurements between _min and _max are replaced by _value. All measurements
#         greater than _max are set to NaN.
#     """
...
#     compatible_unit_str = Column(String)
#     _info_checks = Column(JSON)
#
#     lower_replace_min = ComponentParam('no_check')
#     lower_replace_max = ComponentParam('no_check')
#     lower_replace_value = ComponentParam('no_check')
#     upper_replace_min = ComponentParam('no_check')
#     upper_replace_max = ComponentParam('no_check')
#     upper_replace_value = ComponentParam('no_check')
#
#     # Commented since we're not implementing uncertainty propagation right now. Left commented for later, just in case.
#     # *_maxerr_const : pint Quantity
#     #     Values of the constant (absolute) uncertainty for each uncertainty class: high, med, low
#     # *_maxerr_perc : pint Quantity
#     #     Percentage (relative) uncertainty for each uncertainty class: high, med, low.
#     # *_install_maxerr_const
#     #     Values of the constant (absolute) uncertainty for each uncertainty class: perfect, standard, bad. perfect_maxerr_const is always 0
#     # *_install_maxerr_const
#     #     Percentage (relative) uncertainty uncertainty for each uncertainty class: perfect, standard, bad. perfect_maxerr_perc is always 0%
#     # equation_id
#     #     Reference for the combined uncertainty calculation to use for this sensor type.
#     # high_maxerr_const = ComponentParam('no_check')
#     # high_maxerr_perc = ComponentParam('percent')
#     # medium_maxerr_const = ComponentParam('no_check')
#     # medium_maxerr_perc = ComponentParam('percent')
#     # low_maxerr_const = ComponentParam('no_check')
#     # low_maxerr_perc = ComponentParam('percent')
#     # standard_install_maxerr_const = ComponentParam('no_check')
#     # standard_install_maxerr_perc = ComponentParam('percent')
#     # poor_install_maxerr_const = ComponentParam('no_check')
#     # poor_install_maxerr_perc = ComponentParam('percent')
#     # equation_id = Column(String)
#
#     @property
#     def perfect_maxerr_const(self):
#         return Q(0, self.min_limit.unit)
#
#     @property
#     def perfect_maxerr_perc(self):
#         return Q(0, 'percent')
#
#     @classmethod
#     def get_type(cls, type_name, session):
#         try:
#             return session.query(cls).filter(cls.name == type_name).one()
#         except sqlalchemy.exc.NoResultFound:
#             raise ValueError(f'No entry found for {type_name}')
#         except sqlalchemy.exc.MultipleResultsFound:
#             raise ValueError(f'{type_name} is not unique, more than 1 result found.')
#
#     @property
#     def info_checks(self):
#         if self._info_checks is not None:
#             return self._info_checks
#         else:
#             return {}
...
#
#     unc_cls_abs_select = {'low': SensorType.low_uncertainy_abs, 'med': SensorType.med_uncertainy_abs,
#                           'high': SensorType.high_uncertainty_abs}
#     unc_cls_rel_select = {'low': SensorType.low_uncertainy_rel, 'med': SensorType.med_uncertainy_rel,
#                           'high': SensorType.high_uncertainty_rel}
#     inst_rel_select = {'fair': SensorType.good_install_rel, 'bad': SensorType.bad_install_rel}
#     inst_abs_select = {'fair': SensorType.good_install_abs, 'bad': SensorType.bad_install_abs}
#
#     s = select(unc_cls_abs_select[self.uncertainty_class]).where(
#         (Sensor.raw_name == self.raw_name) & (Sensor.plant_id == self.plant_id))
#     with Session(module_engine) as session:
#         r = session.execute(s).one()
#
#     return r
#
#     # return combined.min_limit
#
#     # Should combine uncertainties from self.uncertainty_class and self.installation_condition
