...
import pvlib as pv
# from common.components import Plant
# from common.components import Array
# from common.components import Sensor
...
# TODO getting circular import errors with these...
# from common.virtuals import _get_aoi
# from common.virtuals import to_rd
...
#     """
#     # return *(None if elem is None else uu.to_s(elem, unit_str='W m**-2') for elem in args),
#     to_unit = lambda x: uu.to_s(x, unit_str='W m**-2')
#     return *(to_unit(np.nan) if elem is None else to_unit(elem) for elem in args),
...
def _get_orientation(c_in):
    ...
    if c_in is None:
        return None
    if isinstance(c_in, cmp.Array):
        tilt = c_in.tilt.m_as('deg')
        azim = c_in.azim.m_as('deg')
    elif isinstance(c_in, cmp.Sensor):  # Assuming radiation sensor
        try:
            tilt = c_in.info['tilt'].m_as('deg')
            azim = c_in.info['azim'].m_as('deg')
        except KeyError:
            raise VirtualSensorCalculationError('Sensor has no sensor info "tilt" or "azim".')
    else:
        raise TypeError('Was expecting components.Array or components.Sensor as input.')
    return tilt, azim
