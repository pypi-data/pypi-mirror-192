...
# from sunpeek.components.sensor import Sensor # TODO: importing Sensor in this module is kind of messy
...
class Context:
    ...
    def get_sensor_data(self, sensor):
        ...
        # print(len(s))  # TODO Remove, debug only
        return s
    ...
    def store_data(self, sensor, data: pd.Series):
        ...
        if self.datasource == 'df':
            ...
            # print(len(hit_db.get_sensor_data(hit_db.get_db_connection(),
            # print(len(hit_db.get_sensor_data(self.session.bind.raw_connection(),
            #                                  sensor_names=sensor.raw_name,
            #                                  table_name=self.plant.raw_table_name,
            #                                  start_timestamp=str(self.eval_start),
            #                                  end_timestamp=str(self.eval_end))
            #           ))
            # TODO BUG This adds the number of data rows to the existing database rows. use update instead
            # data_save_ok = hit_db.execute_batch(hit_db.get_db_connection(),
            # data_save_ok = hit_db.execute_batch(self.session.bind.raw_connection(),
            #                                     data.to_frame(name=sensor.raw_name),
