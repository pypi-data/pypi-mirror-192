...
def send_inj_stream_layer(dag, config, opts, prefix):
	...
	# add job to the dag
	dag.layer(name='send_inj_stream', submit_description = htcondor.Submit(opts), retries = '1000')
	return dag
...
def inspinjmsg_find_layer(dag, config, opts, prefix):
	...
	tag = config['tag']
	topics = get_topics(config['data-source'], config['jobs']['inspinjmsg_find']['input_topic'], f'{tag}.testsuite')
	job_args.update({'input_topic': topics})
	opts.update(add_job_args(job_args))

	# add job to the dag
	dag.layer(name='inspinjmsg_find', submit_description = htcondor.Submit(opts), retries = '1000')
	return dag
...
def igwn_alert_listener_layer(dag, config, opts, prefix):
	...
	# add job to the dag
	dag.layer(name='igwn_alert_listener', submit_description = htcondor.Submit(opts), retries = '1000')
	return dag
...
def vt_layer(dag, config, opts, prefix):
	...
	## for each datasource provided in the config 
	## we need to add one calculate-expected job
	## if that option is given in the config. Then
	## we need to add a job to compute VT with each
	## far threshold
	farstrings = FARSTRINGS_DICT.values()
	num_far_jobs = len(farstrings)
	tag = config['tag']
	for i, source in enumerate(config['data-source']):
		# add job arguments
		job_args = {}
		...
		if 'bootstrap-vt' in job_args.keys():
			job_args.update({
				'scald-config': f'{config["metrics"][source]["config"]}'
			})

		if 'calculate-expected' in job_args.keys():
			opts.update(add_job_args(job_args))
			dag.layer(name=f'expected_vt_{int(i):04d}', submit_description = htcondor.Submit(opts), retries = '3')
		...
		for f, farstring in enumerate(farstrings):
			job_args.update({
				'far-threshold': farstring,
			})

			opts.update(add_job_args(job_args))
			dag.layer(name=f'vt_{((num_far_jobs * int(i)) + f):04d}', submit_description = htcondor.Submit(opts), retries = '3')
...
def snr_consistency_layer(dag, config, opts, prefix):
	...
	tag = config['tag']
	topics = get_topics(config['data-source'], config['jobs']['snr_consistency']['input_topic'], f'{tag}.testsuite')
	...
	# add job to the dag
	dag.layer(name=f'snr_consistency', submit_description = htcondor.Submit(opts), retries = '3')
