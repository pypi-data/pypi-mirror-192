...
class DatasetVersion(Dao, Taggable):
    ...
    def list_assets(
        ...
        assets = mlt.do_paginate(
            ...
            offset,
            page_size,
            partial(self._do_list_assets, q, order_by, filenames, object_names, ids),
        )
    ...
    def build_coco_file_locally(
        ...
        Returns:
            A COCO File object
        """
        builder = COCOFileBuilder()
        asset_ids = builder.load_coco_annotations(self.load_annotations(worker, status))
        builder.load_coco_images(self.find_all_assets(ids=asset_ids))
        builder.load_coco_categories([label.name for label in self.list_labels()])
