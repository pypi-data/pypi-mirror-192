...
class WrapModel(BaseModel):
    def predict(self, items: List[Any], triggered_by_mop) -> List[Any]:
...
def mop_run(raw_data: any, triggered_by_mop: bool = False, **kwargs) -> any:
    if batch_model is not None:
        raw_data = raw_data if isinstance(raw_data, list) else [raw_data]
        inference_result = batch_model.predict(raw_data, triggered_by_mop=triggered_by_mop)
