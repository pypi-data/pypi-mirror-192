...
* 版本号：{version_code}
* 包大小：{size}
![](https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExNjEwNjIxM2JkZDhhNWI5MWE1YmFlYjE2NzllYjAyMjgxMTc0NmU4MiZjdD1n/8v09YkqhKeQBhGE7yF/giphy.gif)
"""
...
def build_web(md5, size, version_name, name, changelog, user, repository, workflow, apk_repository, apk_branch, apk, headers):
...
def upload_to_github(md5, size, apk, user, apk_repository, headers, send, name, url, changelog, secret, token, at, version_name, version_code):
    print(">>>> start upload <<<<")
    content = ""
    ...
    if (r.status_code == 200 or r.status_code == 201):
        ...
        if send == 'True':
            send_to_dingding(md5, size, name, url, changelog, secret,
                             token, at, version_name, version_code)
    else:
        # 上传失败再次上传
        upload_to_github()


def send_to_dingding(md5, size, name, url, changelog, secret, token, at, version_name, version_code):
    print(">>>> send to dingding <<<<")
    content = content_template.format(name=name, url=url, changelog=changelog,
                                      md5=md5, version_name=version_name, version_code=version_code, size=size)
    atMobiles = []
    for i in at:
        atMobiles.append(i)
    ...
    body = {"msgtype": "markdown", "markdown": {"text": content, "title": "更新日志"},
...
def main():
    ...
    for channel in channels:
        ...
        md5 = ""
        size = ""
        build_web(md5, size, version_name, name, changelog, user,
                  repository, workflow, apk_repository, apk_branch, apk, headers)
        change_version(output, input, version_code, version_name)
        ...
        upload_to_github(md5, size, apk, user, apk_repository, headers, send,
                         name, url, changelog, secret, token, at, version_name, version_code)
