...
def get_mims_summary_after_prompt_wearstate(wear_state, sec_after, prompt_time, df_mims_swan_minute_combined, date_in_study):
    ...
    if len(window_start_idices) > 0:
        ...
        for idx in range(window_start_idx, window_end_idx + 1):
            swan_pred = df_mims_swan_minute_combined.loc[idx, "SWAN_PREDICTION"]
            # print(swan_pred)
            if swan_pred != wear_state:
                continue
            mims_value = df_mims_swan_minute_combined.loc[idx, "MIMS_SAMPLE_NUM"]
    ...
    return mims_summary_wearstate, mims_wearstate_min
        ...
        if df_mims_combined.shape[0] == 0:
            ...
            for sec_after in sec_after_list:
                ...
                for wear_state in ["Wear", "Sleep", "Nonwear"]:
                    mims_summary_wearstate, min_wearstate = get_mims_summary_after_prompt_wearstate(wear_state,
                                                                                                    sec_after,
                                                                                                    prompt_time,
    ...
    for sec_after in sec_after_list:
        ...
        df = pd.concat(
            [df, mims_summary_wearstate_after_df[col * 3 + 2].rename(
                "MIMS_NONWEAR_WEAR_" + str(sec_after // 60) + "MIN_AFTER")],
            axis=1)
        df = pd.concat(
