...
def parse_config():
    ...
    # if found config file logger which one was found and path
    if file:
        logger.log(message=f"Config file found at {file} and path {os.path.abspath(file)}", log_level="info", to_file=True, to_terminal=False)
    if not config:
