...
class Interval(Annotation):
    ...
    def adjust(self, zprofile):
        ...
        if not isinstance(zprofile, zProfile):
            return False
        elif zprofile.interpolate(self.x_start) and zprofile.interpolate(self.x_end):
            self.y_start = zprofile.interpolate(self.x_start)
            self.y_end = zprofile.interpolate(self.x_end)
            return True
        else:
            return False  
