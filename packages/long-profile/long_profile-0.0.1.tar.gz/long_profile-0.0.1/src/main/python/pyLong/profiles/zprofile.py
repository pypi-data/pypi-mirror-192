...
class zProfile:
    ...
    def export(self, filename, delimiter="\t", separator=".", decimals=3, reverse=False):
        ...
        >>> zprofile.export("profile.txt", delimiter=";", separator=".", decimals=2)
    ...
    def intersect(self, zprofile):
        ...
        if not isinstance(zprofile, zProfile):
            ...
            if isinstance(xs, list) and isinstance(zs, list):
                xz = [(x, z) for x, z in zip(xs, zs)]
                return xz
            else:
                return None
    ...
    def xz(self, xz):
        ...
        xz.sort()
        self._xz = [(item[0], item[1]) for item in xz]
