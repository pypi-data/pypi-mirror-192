...
def host(ctx):
    ...
    for line in data:
        if line.startswith("Location: "):
            ...
            if "/" in path:
                use_key = "/"
            path1 = glob.glob(f'{path}{use_key}youdotcom{use_key}api_1.py', recursive=True)
            path2 = glob.glob(f'{path}{use_key}youdotcom{use_key}api_2.py', recursive=True)
    ...
    api = subprocess.Popen(["python",f"{path1[0]}"], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    backend = subprocess.Popen(["python",f"{path2[0]}"])
    # api.terminate()
    # backend.terminate()
