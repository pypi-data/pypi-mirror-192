...
class PALBase(hana_ml.ml_base.MLBase, ModelSavingServices):
    ...
    def create_apply_func(self, func_name, data, key=None, model=None, output_table_structure=None):
        """
        ...
        if '#' in model.select_statement:
            raise("Temporary table in model dataframe is not supported when creating TUDF!")
        #check key
        if key is None:
            if data.index:
                key = data.index
        ...
        if output_table_structure is None:
            if 'Automatic' in self.__str__() or 'Pipeline' in self.__str__():
                ...
                raise("Please specify the output table structure.")
        ...
        func_header = "CREATE FUNCTION {0}(in0 Table({1})) RETURNS TABLE({2})\nAS BEGIN\n".format(func_name,
                                                                                                input_tt_str[:-1],
                                                                                                out_tt_str[:-1])
        #create function body
        body = re.search(r'DECLARE [\s\S]+UNNEST\(:param_name, :int_value, :double_value, :string_value\);', self._predict_anonymous_block).group(0)
        model = "\nin1 := {};".format(model.select_statement)
        #return table
        return func_header + body + model + "\nCALL " + self._predict_call + "\nRETURN SELECT * FROM :{};".format("out0") + "\nEND"
