...
from resources import getResourceName
...
class quantum_lab():
    ...
    def __init__(self,
        ...
        self.volumeName = self.getDirpath(volumeName)
        self.resourceName = getResourceName(self.resourceId)
        self.url = "http://job-management.qcc.svc.cluster.local"

        if not isinstance(self.volumeName, str):
            raise TypeError(f"{volumeName} not found.")
        if "ERROR" in self.resourceName:
            raise ValueError(f"{resourceId} not found.")


    def getDirpath(self, dirPath):
        dirList = os.listdir(f"/home/jovyan/{dirPath}")
        for dirName in dirList:
            if self.volumeName == dirName:
