...
def auth_register_response(response, user_id, priv_key_bytes) -> str:
    ...
    auth_utils.auth_create_api_key_with_save(user_id=user_id, password=password)
    shutil.rmtree(TMP_DIR_PATH)
    return f"Register successful! You can now use the CLI. Config file is saved in {get_config_path()}"
