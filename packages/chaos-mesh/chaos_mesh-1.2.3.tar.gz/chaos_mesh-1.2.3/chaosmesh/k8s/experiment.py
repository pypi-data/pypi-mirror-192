...
class ChaosExperiment(CustomObjectsApi, ABC):
    ...
    def resume(self, namespace, name):
        """
        Resumes a running ChaosMesh experiment.

        This method updates the existing annotation to the paused experiment in order to resume it.

        Args:
            namespace (str): The namespace where the experiment is paused.
            name (str): The name of the experiment.

        """
        self.add_annotation(namespace=namespace, name=name, annotations_map={"experiment.chaos-mesh.org/pause": '-'})
