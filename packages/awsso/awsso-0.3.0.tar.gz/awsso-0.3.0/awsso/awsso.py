...
#    parser_sso_logout.set_defaults(func=_sso_logout)
    ...
    parser_env_get.add_argument('get',
    ...
    parser_env_load.add_argument('load',
                        help='lood local file into env.')
    parser_env_load.set_defaults(func=_env_load)


    parser_tf = subparsers.add_parser('tf')
    parser_tf.add_argument('cmd',
                        nargs='*',
    ...
    parser_ec2_search.add_argument('search',
    ...
    parser_image_pull.add_argument('pull',
                        help='pull service image')
    parser_image_pull.set_defaults(func=_image_pull)

    parser_image_stat = parser_image_sub.add_parser('stat')
    parser_image_stat.add_argument('stat',
                        help='showing current image version in specific envs.')
    parser_image_stat.set_defaults(func=_image_stat)
    ...
    if len(sys.argv) == 2 and sys.argv[1] in ('-l', '-c', '-s'):
        if sys.argv[1] == '-l':
            _sso_login()
        if sys.argv[1] == '-c':
            _sso_config()
...
#            _sso_logout()
        ...
        if args.func == _ec2_search:
            _ec2_search(args.search)
        if args.func == _ec2_login:
            _ec2_login(args.login)
        if args.func == _sm_search:
            _sm_search(args.search)
        if args.func == _sm_get:
            _sm_get(args.get)
        if args.func == _sm_create:
            _sm_create()
        if args.func == _sm_update:
            _sm_update(args.update)
        if args.func == _sm_delete:
            _sm_delete(args.delete)
        if args.func == _image_list:
            _image_list(args.list)
        if args.func == _image_pull:
            _image_pull(args.pull)
        if args.func == _image_stat:
            _image_stat(args.stat)
...
def _image_pull(image):
    subprocess.run('aws ecr get-login-password --region {1} | docker login --username AWS --password-stdin {0}.dkr.ecr.{1}.amazonaws.com'.format(AWS_OPS1_ACCOUNT, AWS_OPS1_REGION),
        shell=True,
        stderr=sys.stderr,
        stdout=sys.stdout,
        check=True)
