...
class Ui_PresetEchoesPatches(object):
    ...
    def retranslateUi(self, PresetEchoesPatches):
        ...
        self.new_patcher_label.setText(QCoreApplication.translate("PresetEchoesPatches", u"<html><head/><body><p>Activates the use of the new patcher in addition to the existing one.</p><p>With this enabled, certain in-game puzzles will have their solution randomized, but are also modified to have better accessibility.<br/>It's also required for certain new features, such as Inverted Aether.</p><p>This setting causes additional stuttering during gameplay, especially on Dolphin. It's recommended to unlock disc read speeds.</p></body></html>", None))
