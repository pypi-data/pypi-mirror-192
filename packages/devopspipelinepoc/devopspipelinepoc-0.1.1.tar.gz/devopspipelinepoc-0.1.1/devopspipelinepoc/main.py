...
def add_dockerfile(path: str):
    ...
    pkg_dir = sys.modules['devopspipelinepoc'].__path__[0]
    ...
    if config.image_type == 'scrapy':
        docker_file_path = os.path.join(pkg_dir, 'dockerfiles/scrapy/Dockerfile')
        shutil.copy(docker_file_path,path)
    elif config.image_type == 'selenium':
        docker_file_path = os.path.join(pkg_dir, 'dockerfiles/selenium/Dockerfile')
        shutil.copy(docker_file_path,path)
    else:
...
def add_worker_file(path: str):
    ...
    if return_type is str:
        worker_file_path = os.path.join(pkg_dir, 'worker_files/path_output/worker.py')
        shutil.copy(worker_file_path,path)
    elif return_type is list:
        worker_file_path = os.path.join(pkg_dir, 'worker_files/data_output/worker.py')
        shutil.copy(worker_file_path,path)
    else:
