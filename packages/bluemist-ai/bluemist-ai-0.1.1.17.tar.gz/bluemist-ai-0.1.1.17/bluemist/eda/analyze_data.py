...
def perform_eda(data,
    ...
    """
        ...
        target_variable: str
            Target to be predicted. Used only if ``provider`` is ``autoviz``
    ...
    if provider in valid_providers:
        ...
        if provider == 'pandas-profiling':
            ...
            AV = AutoViz_Class()
            save_plot_dir=EDA_ARTIFACTS_PATH + '/' + 'autoviz'
            AV.AutoViz(filename='', sep=',', dfte=data, chart_format='html', depVar=target_variable,
                       max_rows_analyzed=data.shape[0], save_plot_dir=save_plot_dir)
            logger.info('Output html files saved under :: {}'.format(save_plot_dir))
            display('Output html files saved under :: {}'.format(save_plot_dir))
    else:
