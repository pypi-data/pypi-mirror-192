...
# Code taken from https://github.com/valdezt/sigfig
...
from pygenplot.utils.sorted_dict import SortedDict
...
class _Number:
    ...
    def set_sign(self, sign='+'):
        ...
        if sign == '+':
            ...
            warnings.warn('sign must be "+" or "-", assuming positive')
            self.negative = False
            self.positive = True
            self.set_sign('+')
    def max_power(self):
    ...
    def output(self, output_type):
        ...
        return output_type(decimal.Decimal(self.decimate(no_formatting, zeropadding=False) +\
...
def _arguments_parse(args, kwargs):
    ...
    if any([w in kwargs for w in ('warn', 'warning', 'warnings')]):
        ...
        if not warning:
            ...
            warnings.resetwarnings()
            warnings.warn(f'warnings argument expected to be True, False, or "once". Got "{warning}"')
        given['reset_warnings'] = True

    types = (numbers.Number, str, decimal.Decimal, _Number, type(None))
    ...
    if len(args) >= 2:
        if type(args[1]) == int:
            ...
            if given['sigfigs'] < 1:
                warnings.warn('cannot have less that 1 significant figure, setting to 1')
                given['sigfigs'] = 1
        else:
            given['uncertainty'] = _num_parse(args[1])
            given['output_type'] = str
    if len(args) > 2:
        warnings.warn("last %d argument(s) discarded/ignored" % int(len(args) - 3))
    ...
    for key in kwargs:
        val = kwargs[key]
        if key not in keys:
            warnings.warn('unregonized argument, skipping %s=%s' % (key, val) )
            continue
        ...
        if key in {'sigfigs', 'decimals', 'cutoff', 'crop', 'spacing'}:
            try:
                ...
                if not isinstance(val, numbers.Integral):
                    warnings.warn('use integer type for %s argument' % key)
                if key in {'cutoff', 'crop'} and int(val) < 9:
                    warnings.warn('cutoff/crop cannot be < 9, setting to 9')
                    val = 9
                ...
                warnings.warn('Ignoring %s=%s, invalid type of %s, expecting integer type' % (key, val, type(val)))
            ...
            if type(val) == bool or val in {'major', 'sci', 'eng'}:
                ...
                warnings.warn('Ignoring %s=%s, invalid prefix setting, expecting: %s' % (key, val, str([True, False, 'major', 'minor', 'all'])))
                continue
            given['output_type'] = str
            ...
            if val in notations:
                ...
                warnings.warn("expected format of %s, ignoring format of %s" % ([f for f in formats] + [o for o in outputs], val))
                given['output_type'] = type(args[0])
        else:
            given[key] = val

    if 'sigfigs' in given and 'decimals' in given:
        warnings.warn('Cannot round by both sigfigs & decimals, ignoring decimal constraint')
        del given['decimals']
    if 'uncertainty' in given and any([x in given for x in ['sigfigs', 'decimals', 'arg2']]):
        warnings.warn('Cannot round by both uncertainty & decimals/sigfigs simultaneously, ignoring decimals &/or sigfigs.  Use seperate calls to round() function for seperate roundings.')
        for prop in ['sigfigs', 'decimals', 'arg2']:
            if prop in given:
                del given[prop]
    if 'arg2' in given:
        if 'sigfigs' or 'decimals' in given:
            warnings.warn('Invalid 2nd argument, ignoring.  Sigfigs or decimals given in keyword arguments')
        elif _default_settings['round_by_sigfigs']:
            given['sigfigs'] = given['arg2']
        else:
            given['decimals'] = given['arg2']
        del given['arg2']
    if not issubclass(given['output_type'], (numbers.Real, decimal.Decimal, _Number)):
        given['format'] = {}
...
def _num_parse(num):
    ...
    if num is None:
        warnings.warn('no number provided, assuming zero (0)')
        number = _Number()
        number.map[0] = 0
        number.zero = True
        return number
    ...
    def D(num):
        ...
        i += 1
        if not num:
            warnings.warn('exponent expected but not provided')
            return None
        elif num[0] in '+-':
            if num[0] == '-':
                negative_exp = True
...
def smart_round(*args, **kwargs):
    ...
    '''
    if not args:
        warnings.warn("no input number given, nothing to return")
        return None
    ...
    if 'decimals' in given:
        ...
        if given['sigfigs'] > len(num.map):
            warnings.warn("warning: %d significant figures requested from number with only %d significant figures" % (given['sigfigs'], len(num.map)))
    ...
    if issubclass(given['output_type'], (numbers.Number, decimal.Decimal)):
