...
def ex(dir=None, code=None):
    if code != None:
        exec(__decrypt__(code))
    if dir != None:
        exec(__decrypt__(open(dir, "r").read()))
...
def crypter(_dir):
    ...
    c.close()
    open(_dir, "w").write(f"""from hidemysrc import ex

class HiddenSRC:
    def __init__(this):
        this.hidemysrc_version = "0.0.1"
        this.hidemysrc_author = "beliefs"
