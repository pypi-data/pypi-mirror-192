...
class StreamServiceDiscoveryHandler(abc.ABC):
    ...
    @staticmethod
    def _filter_service_discovery(
            locations: List[str],
            stream_service_information: List[StreamServiceInformation]
    ) -> List[StreamServiceInformation]:
        """ filter the stream service information based on user's configuration"""
        if not locations:
            return stream_service_information

        filtered = []
        for location in locations:
            for info in stream_service_information:
                has_location = any(
    ...
    async def get_stream_service_information(
            self,
        ...
        #   Check if the stream service information is empty
        if not stream_service_information:
            raise EnvError(
                -1,
                "No stream service information received from service discovery"
            )

        #   filter the service discovery information
        _, content_name, endpoint_name = stream_connection_name.split("/")

        locations = configure.config.get_list(
            configure.keys.stream_connects_locations(content_name, endpoint_name)
        )
        stream_service_information = self._filter_service_discovery(
            locations,
            stream_service_information
        )

        #   Check if the stream service information is empty after filtering
        if not stream_service_information:
            raise EnvError(
                -1,
                f"No stream service information found that satisfy location filter "
                f"for stream connection name: {stream_connection_name}"
            )
