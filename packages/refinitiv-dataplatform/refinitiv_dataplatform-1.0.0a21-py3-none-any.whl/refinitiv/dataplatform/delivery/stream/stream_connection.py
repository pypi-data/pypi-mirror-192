...
class StreamConnection(threading.Thread, LogReporter, abc.ABC):
    ...
    def run(self):
        ...
        while not self.is_closing and self._do_reconnect:
            ...
            if "desktop" in self._session.name:
                user_id = os.getenv("REFINITIV_AAA_USER_ID")
                if user_id:
                    cookies = f"user-uuid={user_id}"
            ...
            self.debug(
                f"Run websocket with {str(_proxy_config)} "
                f"and {_no_proxy} http_no_proxy with {self._websocket.header} headers "
                f"and {self._websocket.cookie} cookies"
            )
            ...
            if not self.is_closing and self._do_reconnect:
                ...
                if (
                    ...
                    # cancel the ready and make sure the stream is disconnected
                    #   cancel the ready
