class Calculator:
    ...
    def add(self):
        ...
        result = self.memory_value + self.n
        print(f'\033[1m{self.memory_value} + {self.n} = \033[36m{result}\033[0m'.center(85))
        return result
    ...
    def divide(self):
        ...
        if self.n != 0:
            result = self.memory_value / self.n
            print(f'|{self.memory_value} / {self.n} = \033[36m{result}\033[0m'.center(85))
            return result
        else:
    ...
    def n_root(self):
        ...
        self.n = self.validate_arguments(self.n)
        result = self.memory_value ** (1 / self.n)
        print(f'\033[1m{self.memory_value} ^ (1/{self.n}) = \033[36m{result}\033[0m'.center(85))
        return result
    ...
    while True:
        ...
        if menu in ['+', '-', '*', '/', 'e', 'r', '0', '1', '9']:
            ...
            # Call for class functions
                ...
                memory_num = Calculator(memory_num, num).add()
            elif menu == '-':
                memory_num = Calculator(memory_num, num).subtract()
            elif menu == '*':
                memory_num = Calculator(memory_num, num).multiply()
            elif menu == '/':
                memory_num = Calculator(memory_num, num).divide()
            elif menu == 'e':
                memory_num = Calculator(memory_num, num).expon()
            elif menu == 'r':
                memory_num = Calculator(memory_num, num).n_root()
