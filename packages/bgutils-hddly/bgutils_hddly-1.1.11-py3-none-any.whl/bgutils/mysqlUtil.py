...
class mysqlUtil:
    ...
    #添加上传文件记录
    def sftp_file_ins(self,filename,url,stud,desc,fsize,duration,ftype):
        try:
            df = pd.DataFrame(columns=['filename','url','stud','desc','fsize','duration','ftype'])
            # print(df)
            row1=pd.DataFrame({'filename':filename,'url':url,'stud':stud,'desc':desc,'fsize':fsize,'duration':duration,'ftype':ftype}, index=[1])
            # print(row1)
