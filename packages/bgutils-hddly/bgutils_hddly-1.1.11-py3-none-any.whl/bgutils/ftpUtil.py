...
class ftpUtil:
    ...
    def putfile_project(self, local_path, prj_id, stud_id, no_file,desc):
        # # 要传输文件的路径
        # filepath = "./myname.jpg"
        # # 上传后的传输文件的文件名
        ...
        try:
            ...
            no = no_file.zfill(8)
            remote_file = prj + "_" + stud + "_" + no + ftype
        ...
        try:
            ...
            # item['stud'] = stud
            self.__mutil.sftp_file_ins(local_path, self.__url_head +prj +'/'+ remote_file, stud,desc,fsize,duration,ftype)
        except Exception as ex:
