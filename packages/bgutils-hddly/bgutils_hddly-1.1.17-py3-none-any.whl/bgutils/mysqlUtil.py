...
class mysqlUtil:
    ...
    def get_file_list(self, ftype):
        if ftype:
            ...
            ftype = ".mp4"
            'select id,url,fdesc,fsize,duration,ftype,uptime,pid from sftp_files where ftype="{}" order by uptime desc limit 100'.format(
                ftype)
    ...
    def get_files_bypid(self, pid):
        if pid:
            pid = str(pid).lower()
            sql = 'select id,url,fdesc,fsize,duration,ftype,uptime,pid from sftp_files where pid="{}" order by uptime desc limit 100'.format(
                pid)
        else:
            pid = "p023101".lower()
            'select id,url,fdesc,fsize,duration,ftype,uptime,pid from sftp_files where pid="{}" order by uptime desc limit 100'.format(
                pid)
        connection = self.engine.connect()
        df = pd.read_sql(sql,  con=connection)
        results = json.loads(df.to_json(orient='records'))
