...
class DatasetCreator(object):
    ...
    def clf_slicer_all_present(self):
        if self.settings['bout_setting']:
            self.df = bout_aggregator(data=self.df, clfs=self.clf_names, feature_names=self.feature_names).reset_index(drop=True)
        else:
            clf_cnt = self.df[self.clf_names].sum(axis=1)
            idx_lst = list(clf_cnt.index[clf_cnt >= 1])
            self.df = self.df[self.df.index.isin(idx_lst)]
    ...
    def save(self):
        ...
        if not self.settings['bout_setting']:
            self.results['BOUT_AGGREGATION'] = False
            self.results['START_FRAME'] = self.df[['FRAME']]
            self.results['END_FRAME'] = None
            self.results['CLF'] = self.df[self.clf_names]
            self.results['CLF_PROBABILITY'] = self.df[self.clf_probability_cols]
        else:
            self.results['BOUT_AGGREGATION'] = True
        ...
        if not self.settings['bout_setting']:
            print(f'SIMBA COMPLETE: Dataset for unsupervised learning saved at {self.save_path}. The dataset contains {str(len(self.results["DATA"]))} frames (elapsed time {self.timer.elapsed_time_str}s)')
        else:
