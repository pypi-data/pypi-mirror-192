...
def HDBSCANTransform(embedding_model_path: str,
    ...
    if settings['save_format'] is 'csv':
        save_path = os.path.join(save_dir, f'transformed_{input["CLUSTERER"]["NAME"]}.csv')
        embedding_data.to_csv(save_path, index=False)

    elif settings['save_format'] is 'parquet':
        save_path = os.path.join(save_dir, f'transformed_{input["CLUSTERER"]["NAME"]}.parquet')
        embedding_data.to_parquet(save_path, index=False)
...
# embedding_path = '/Users/simon/Desktop/envs/troubleshooting/unsupervised/dr_models/magical_darwin.pickle'
# clusterer_model_path = '/Users/simon/Desktop/envs/troubleshooting/unsupervised/cluster_models/keen_driscoll.pickle'
