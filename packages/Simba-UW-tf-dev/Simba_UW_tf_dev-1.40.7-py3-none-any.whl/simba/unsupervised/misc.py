...
def bout_aggregator(data: pd.DataFrame, clfs: list, feature_names: list):
    ...
    for video in data['VIDEO'].unique():
        ...
        for clf in clfs:
            bouts = detect_bouts(data_df=video_df, target_lst=[clf], fps=-1).sort_values(by='Start_frame')[['Start_frame', 'End_frame']].values
...
#
# bout_aggregator(data=data, clfs=['Attack', 'Sniffing'], feature_names=feature_names)