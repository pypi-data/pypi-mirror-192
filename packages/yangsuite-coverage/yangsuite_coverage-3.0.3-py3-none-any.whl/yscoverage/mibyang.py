        ...
        try:
            if 'experimental' in curr_walk_file.lower():
                ...
                if not tokens or len(tokens) != 2:
                    ...
                    log.error('No OS name specified in walk file.')
                    raise ValueError('No OS name specified in walk file.')
                orig_os = tokens[0]
            else:
                tokens = [
                ...
                if not tokens:
                    log.error('Invalid walk file name.')
                    raise ValueError('Invalid walk file name.')
                orig_os = tokens[0]
        except ValueError:
