...
class FeatureView(unified_common.BaseTectonObject, internal_fco.Fco):
    ...
    @utils.requires_validation
    def _batch_materialization_schedule(self) -> pendulum.Duration:
        return self._feature_definition.batch_materialization_schedule

    @utils.requires_validation
    def _schedule_offset(self) -> pendulum.Duration:
        return self._feature_definition.max_source_data_delay
...
class MaterializedFeatureView(FeatureView):
    ...
    def get_historical_features(
        ...
        # TODO(jake): Port over get_historical_features() error checking.
        ...
        if start_time is not None and end_time is not None and start_time > end_time:
            raise errors.GET_HISTORICAL_FEATURES_START_TIME_GT_END_TIME
    ...
    def run(
        ...
        **mock_sources: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        :param start_time: The start time of the time window to materialize. If not set, defaults to `end_time` minus
            `batch_schedule`. If `end_time` is also not set, defaults to the start of the last complete materialization period.
        :param end_time: The end time of the time window to materialize. If not set, defaults to `start_time` plus
            `batch_schedule`. If `start_time` is also not set, defaults to the end of the last complete materialization period.
    ...
    def __init__(
        ...
        *,
        ...
        batch_compute: Optional[
            Union[
                feature_configs.DatabricksClusterConfig,
                feature_configs.EMRClusterConfig,
                feature_configs.DatabricksJsonClusterConfig,
                feature_configs.EMRJsonClusterConfig,
            ]
        ] = None,
...
class OnDemandFeatureView(FeatureView):
    ...
    sources: Tuple[Union["FeatureReference", declarative_data_source.RequestSource], ...] = attrs.field(
    ...
    def __init__(
        ...
        *,
        ...
        prevent_destroy: bool = False,
        mode: str,
        sources: List[Union[declarative_data_source.RequestSource, FeatureView, "FeatureReference"]],
        schema: List[types.Field],
        feature_view_function: Callable,
    ):
        from tecton.cli import common as cli_common
...
) -> Tuple[Union[FeatureReference, declarative_data_source.RequestSource], ...]:
    sources = []
...
def _source_to_pipeline_node(
    source: Union[
        ...
        declarative_data_source.RequestSource,
        FeatureView,
        FeatureReference,
    ],
    input_name: str,
    ...
    if isinstance(source, unified_data_source.DataSource):
        ...
        pipeline_node.data_source_node.CopyFrom(node)
    elif isinstance(source, declarative_data_source.RequestSource):
    ...
    return declarative_transformation.PipelineNodeWrapper(node_proto=pipeline_node)
...
) -> Dict[str, declarative_transformation.PipelineNodeWrapper]:
    kwargs = {}
...
def _build_pipeline_for_odfv(
    ...
    pipeline_function: Callable[..., declarative_transformation.PipelineNodeWrapper],
    sources: Sequence[Union[declarative_data_source.RequestSource, FeatureReference, FeatureView]],
) -> declarative_transformation.PipelineNodeWrapper:
...
def _build_materialized_feature_view_args(
    ...
    online: bool,
    offline: bool,
    offline_store: Union[feature_configs.ParquetConfig, feature_configs.DeltaConfig],
    online_store: Optional[Union[feature_configs.DynamoConfig, feature_configs.RedisConfig]],
    aggregation_interval: Optional[datetime.timedelta],
    aggregations: Optional[Sequence[declarative_feature_view.Aggregation]],
    ...
    monitoring = feature_configs.MonitoringConfig(monitor_freshness, expected_feature_freshness, alert_email)
