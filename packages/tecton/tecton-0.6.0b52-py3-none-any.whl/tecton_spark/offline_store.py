...
from tecton_core.offline_store import TimestampFormats
...
class ParquetWriter(OfflineStoreWriter):
    ...
    def append_dataframe(self, data_frame):
        if self._params.is_continuous:
            align_duration = core_time_utils.convert_timedelta_for_version(
                pendulum.duration(seconds=CONTINUOUS_PARTITION_SIZE_SECONDS), self._version
            )
...
class ParquetReader(OfflineStoreReader):
    def __init__(self, spark: SparkSession, fd: FeatureDefinition):
        ...
        if fd.is_continuous:
            self._partition_size = pendulum.Duration(seconds=CONTINUOUS_PARTITION_SIZE_SECONDS)
        else:
            self._partition_size = fd.min_scheduling_interval
    ...
    def read(self, time_limits: Optional[pendulum.Period]):
        ...
        if self._fd.is_continuous:
            partition_col_name = TIME_PARTITION
        else:
            partition_col_name = ANCHOR_TIME

        spark_df = spark_df.withColumn(partition_col_name, functions.col(partition_col_name).cast("long"))
        ...
        if time_limits and self._partition_size:
            ...
            partition_col = functions.col(partition_col_name)
...
class DeltaWriter(OfflineStoreWriter):
    ...
    def __init__(
        ...
        self._version = version
        self._spark = spark
        self._partition_size = delta_config.time_partition_size.ToTimedelta()
...
class DeltaReader(OfflineStoreReader):
    def __init__(self, spark: SparkSession, fd: FeatureDefinition):
        ...
        assert fd.materialization_enabled and fd.writes_to_offline_store
        self._path = fd.materialized_data_path
        self._partition_size = fd.offline_store_config.delta.time_partition_size.ToTimedelta()
