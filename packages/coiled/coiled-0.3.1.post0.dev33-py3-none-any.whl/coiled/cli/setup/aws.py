...
def do_quota_increases(quota_client, region: str, quotas: list):
    for quota in quotas:
        ...
        if desired_value and desired_value != quota["value"]:
            if not request_quota_increase(
                quota_client, region, quota, desired_value=desired_value
            ):
...
def request_quota_increase(
    ...
    try:
        ...
        request_case_id = quota_response["RequestedQuota"].get(
            "CaseId"
        )  # might not be in response
