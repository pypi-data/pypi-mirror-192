...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _create_software_environment(
        ...
        *,
        ...
        if (
            name is None
            and conda is not None
            and isinstance(conda, dict)
            and "name" in conda
        ):
            name = conda["name"]
        ...
        if new_build_backend and conda_env_name:
            raise TypeError(
                "The build backend does not support custom conda environment names"
            )
        if new_build_backend and (pip or conda) and container:
            raise TypeError(
                "The build backend does not support public software environments"
            )
                ...
                if alias.get("latest_spec"):
                    ...
                    if spec.get("latest_build"):
                        ...
                        if build["state"] == "built":
                            ...
                            info_url = (
                                self.server + f"/software/{alias['id']}"
                                f"/history?account={account}&specExpanded={alias['latest_spec']['id']}&tab=logs"
                            )
                            log_output.write(
                                f"Logs may be truncated, see {info_url} for full output"
                            )

                            if final_state == "error":
                                raise RuntimeError(
                                    "The software environment failed to build"
                                )
        ...
        if pip_extracted_conda is not None and "dependencies" in pip_extracted_conda:
            pip_extracted_conda["dependencies"] = sorted(
                set(pip_extracted_conda["dependencies"])
            )
    ...
    async def _tail_software_build_logs(
        ...
        page_token = None
        logs = await self._fetch_software_build_logs(
            senv_build_id, account=account, page_token=page_token
        )
        printed_logs = set()
    ...
    async def _fetch_software_build(
        self, senv_build_id: int, account: Optional[str] = None
    ):
        account = account or self.default_account
        resp = await self._do_request(
            "GET",
            self.server
            + f"/api/v2/software-environment/account/{account}/build/{senv_build_id}",
        )
    ...
    async def _fetch_software_build_logs(
        ...
        resp = await self._do_request(
            "GET",
            self.server
            + f"/api/v2/software-environment/account/{account}/build/logs/{senv_build_id}",
            params=params,
        )
        return await resp.json()
    ...
    @track_context
    async def _list_software_environments(
        self, account: Optional[str] = None, new_build_backend: bool = True
    ) -> dict:
        if new_build_backend:
            return await self._depaginate_v2(
                self._list_software_environments_page_v2, account=account
            )
        else:
    ...
    async def _list_software_environments_page_v2(
        self, offset: int, account: Optional[str] = None
    ) -> Tuple[dict, int]:
        account = account or self.default_account
        response = await self._do_request(
            "GET",
        ...
        response_json = await response.json()
        return {
            alias["name"]: alias for alias in response_json["items"]
        }, response_json["count"]
    ...
    def set_aws_credentials(
        self: CloudSyncAsync, aws_credentials: dict, account: Optional[str] = None
    ):
        return self._sync(
    ...
    def delete_software_environment(
        ...
        return self._sync(
            self._delete_software_environment, name, account, new_build_backend
        )
    ...
    async def _get_software_info(
        ...
        if new_build_backend:
            response = await self._do_request(
                "GET",
                self.server
                + f"/api/v2/software-environment/account/{account}/alias/name/{name}",
            )
            if response.status >= 400:
                await handle_api_exception(response)
            alias = await response.json()
            return await self._get_software_spec(
                alias["latest_spec"]["id"], account=account
            )
        else:
            response = await self._do_request(
                "GET",
    ...
    def list_core_usage(
        self: CloudSyncAsync, account: Optional[str] = None
...
def list_software_environments(account=None, new_build_backend: bool = True):
    with Cloud() as cloud:
        return cloud.list_software_environments(
            account=account, new_build_backend=new_build_backend
        )


@delete_docstring
def delete_software_environment(
    name, account: Optional[str] = None, new_build_backend: bool = True
):
    with Cloud() as cloud:
        return cloud.delete_software_environment(
            name=name, account=account, new_build_backend=new_build_backend
        )
