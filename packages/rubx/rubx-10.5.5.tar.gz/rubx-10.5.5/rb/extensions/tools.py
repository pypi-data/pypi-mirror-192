...
class Maker(object):
    ...
    def check_link(cls, link: str, client: object = None,
        ...
        if link.startswith('@') or re.search(r'rubika\.ir/\w{4,25}', link):

            link: str = link.replace('https://', '').replace('rubika.ir/', '')
            result: dict = client.getObjectByUsername(client, link.replace('@', ''))
            ...
            if 'joinc' in link:
                ...
                result = client.groupPreviewByJoinLink(client, link)
                ...
                result = client.channelPreviewByJoinLink(client, link)
