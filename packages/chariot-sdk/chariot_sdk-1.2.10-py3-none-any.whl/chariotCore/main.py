...
def cmdline():
    ...
    parser.add_argument("-g", "--generate",
                        help="生成插件，需要yaml或json类型的插件定义文件路径参数，建议每次改动插件定义文件时重新生成一次插件。注意，重新生成插件会覆盖不同组件的models.py校验文件。使用示例：chariot-plugin -g .\plugin.spec.yaml",
                        action="append")
    parser.add_argument("-ag", "--auto_generate",
                        help="自动生成高度模板化插件，需要特殊格式的插件定义文件路径，此方法一般自动调用。",
                        action="append")
    parser.add_argument("-y", "--yaml", help="在当前工作目录下生成一个yaml模板文件用于编写插件参数",
                        action="store_true")
    parser.add_argument("-r", "--run",
                        help="运行插件指定功能，需要json格式的数据参数。注意，此命令会尝试运行完成一个功能的完整流程，并且发送真实请求。使用示例：chariot-plugin -r tests\example_action.json",
                        action="append")
    parser.add_argument("-hp", "--http", help="启动api接口，插件以REST服务的形式对外提供服务，用于测试和使用接口",
                        action="store_true")
    parser.add_argument("-t", "--test",
                        help="测试插件指定功能的连接器(connection)部分，需要json格式的数据参数。注意，此命令可能会发送真实请求。使用示例：chariot-plugin -t tests\example_action.json",
                        action="append")
    parser.add_argument("-tb", "--tarball",
                        help="插件打包(此命令仅在Linux环境下可用)，生成在线包，用于在线环境的插件安装，打包后的文件较小",
                        action="store_true")
    parser.add_argument("-mki", "--mkimg",
                        help="制作成docker镜像(此命令仅在Linux环境下可用），生成离线包，用于离线环境的插件安装。注意，打包时需要联网，打包后的文件较大",
                        action="store_true")
