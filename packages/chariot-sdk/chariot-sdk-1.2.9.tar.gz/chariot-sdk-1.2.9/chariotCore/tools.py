    ...
    for module_key, params in module_data.items():
        try:
            ...
            if module == "actions":
                ...
                raise Exception(f"未知的模块：{module}")
        ...
        #   生成测试用的json文件
        generateTestFile(work_path, key_data, key_test_template)

    generateInitFile(module, key_path, init_list)
...
def generateTestFile(work_path, key_data, key_test_template):
    """
    #   生成测试用的json文件
    参数说明：
    work_path:str,    #   当前工作区绝对路径
    key_data:dict,     #   功能的各项参数
    key_test_template:str,  #   需要渲染的测试文件模板
    生成失败时抛出异常
    """

    module_key = key_data.get("name")
    ...
    try:
        #   渲染测试文件内容
        test = renderStrTemplate(key_data, key_test_template)
