...
def hand_B(data):
    ...
    results_x_iter = list(map(lambda a: (a['x']-point_relation['x']), points_top))
    results_x = list(filter(lambda a: a<0, results_x_iter))

    results_x_diff = list(map(lambda a: (a['x']), points_top))
    results_diff = [results_x_diff[3]-results_x_diff[2],results_x_diff[2]-results_x_diff[1], results_x_diff[1] - results_x_diff[0]]
    results_diff_filter = (list(filter(lambda x: x > (0.1), results_diff)))

    results_z_diff = dict(map(lambda a: (a['index'], a['y']), points_calculate))
    results_z_diff_major = (list(filter(lambda x: results_z_diff[x] == max(results_z_diff.values()), results_z_diff))[0])

    diff_top = list(map(lambda a: (a['y']), points_top))
    diff_between = [points_midd[1]['y'] - top for top in (diff_top)]
    results_diff_between = list(filter(lambda x: x < (0), diff_between))

    results_midd_iter = dict(map(lambda a: (a['index'], a['y']), points_calculate_midd))
    mid_axis_major = (list(filter(lambda x: results_midd_iter[x] == min(results_midd_iter.values()), results_midd_iter))[0])

    if len(results_x) == 0:
        if len(results_diff_filter) == 0:
            if results_z_diff_major == mid_axis_major:
                if len(results_diff_between) == 0:
                    if results_z_diff_major == "THUMB_TIP":
                        result['rule_match'] = True
    return result
