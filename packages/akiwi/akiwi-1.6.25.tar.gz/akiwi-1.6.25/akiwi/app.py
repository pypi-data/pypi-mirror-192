...
class Config:
    ...
    def setup(self):
        ...
        kiwi_sh_lines = open(kiwi_sh, "r").read().split("\n")
        kiwi_exe = sys.argv[0]
        dst_value = f"export KIWI_EXE='{kiwi_exe}'"
        if kiwi_sh_lines[0] != dst_value:
...
class Actions:
    ...
    def __register_lib(self, folder_name, lib_file, update_installed_lib=True):
        try:
            ...
            if PATHvalue != "":
                self.cfg.CURRENT_ENVARS["PATH"][folder_name] = PATHvalue

            LDLIBRARYvalue = attributes.get("LD_LIBRARY_PATH", "")
            if LDLIBRARYvalue != "":
                self.cfg.CURRENT_ENVARS["LD_LIBRARY_PATH"][folder_name] = LDLIBRARYvalue

            if update_installed_lib:
    ...
    def get(self, args : argparse.Namespace, addi_args):
        ...
        if not args.disable_run:
            if not self.__atomatic_run(proj, args): return False
            with ChangeCWD(proj):
                utils.vprint("Execute repvar")
                return self.app.run_with_command("repvar")
        else:
        ...
        if state == "Cached" and os.path.isdir(install_to) and (dname in self.cfg.INSTALLED_LIBS):
            if not args.force:
                ...
                if os.path.exists(lib_file):
                    self.__register_lib(dname, lib_file, False)
...
class Application:
    def __init__(self):
        ...
        c.add_argument("pkg", type=str, help="package name")
        c.add_argument("-update", action="store_true", help="Force update")
        c.add_argument("-save", type=str, help="Unzipped folder")
