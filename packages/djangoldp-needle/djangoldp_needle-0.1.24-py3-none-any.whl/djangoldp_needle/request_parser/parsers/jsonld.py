...
class JSONLD(AbstractParser):
    def parse(self, annotation_target: AnnotationTarget, target_url, bs_document, previous_parse_result):
        ...
        for jsonElement in jsonElements:
            if (jsonElement):
                value = json.loads("".join(jsonElement.contents))
                ...
                if value:
                    ...
                    if type and type in accepted_types:
                        ...
                        if jsonld_string:
                            ...
                            date = None
                            if (datePublished):
                                date = self.try_strptimeAll(datePublished)
                                if date is not None:
                            ...
                            if date is None:
                                ...
                                if (dateCreated):
                                    date = self.try_strptimeAll(dateCreated)
                                    if date is not None:
                                        annotation_target.publication_date = date.isoformat()
    ...
    def try_strptimeAll(self, dateString):
                                                            ...
                                                            if date is None and cutIndex < 0:
                                                                ...
                                                                if result:
                                                                    ...
                                                                    if result:
                                                                        dateString = result.group(1)
                                                                        if date is None:
                                                                            date = self.try_strptime(dateString, '%Y/%m')
                                                                            if date is None:
                                                                                date = self.try_strptime(dateString, '%Y-%m')
                                                                    else:
                                                                        p = re.compile(
                                                                        ...
                                                                        if result:
                                                                            dateString = result.group(1)
                                                                            date = self.try_strptime(dateString, '%Y')
        else:
                    ...
                    if date is None:
                        ...
                        if date is None:
                            ...
                            if date is None:
                                date = self.try_strptime(dateString, '%m/%d/%Y %H:%M:%S')
                                if date is None:
                                    date = self.try_strptime(dateString, '%d-%m-%Y')
                                    if date is None:
                                        date = self.try_strptime(dateString, '%m-%d-%Y')
                                        if date is None:
                                            date = self.try_strptime(dateString, '%d/%m/%Y')
                                            if date is None:
                                                date = self.try_strptime(dateString, '%m/%d/%Y')
        if date is not None:
            ...
            if cutIndex > 0:
                dateCut = dateStringSave[:cutIndex]
