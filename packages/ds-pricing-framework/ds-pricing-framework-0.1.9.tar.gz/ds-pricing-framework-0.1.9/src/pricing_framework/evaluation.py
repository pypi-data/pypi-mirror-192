...
class EvaluateFactor(object):
    ...
    def compare_loss_table(self):
        ...
        df = self.df
        proposed_variable = self.proposed_sort_by_var
    
        variable = df[proprosed_variable].unique()
        variable = variable.tolist()
        variable.sort()
        count = df.groupby([proprosed_variable])["policy_number"].count()
        claim = df.groupby([proprosed_variable])["claim_indicator"].sum()
        freq = claim/count

        il = df.groupby([proprosed_variable])["il"].sum()
        curr_ep = df.groupby([proprosed_variable])["curr_earned_premium"].sum()
        prop_ep = df.groupby([proprosed_variable])["prop_earned_premium"].sum()
        curr_lr = il/curr_ep
        prop_lr = il/prop_ep

        titles = [proprosed_variable, 'Policy Count', '# of Claims', 'Freq.', 'Current LR', 'Proposed LR']
        rows = list(zip(variable, count, claim, freq, curr_lr, prop_lr))

        return print(tabulate(rows, headers=titles, floatfmt=".4f"))
    
    def overall_impact(df):
        ...
        curr_prem = df2["curr_earned_premium"].sum()
        prop_prem = df2["prop_earned_premium"].sum()
        ...
        titles = ['% Policies Impacted', 'Total Premium Change', 'Average Impacted Price Change', 'Total Average Price Change']
        rows = (pct_impacted, pct_change_prem, pct_change_prem2, pct_change_prem)
