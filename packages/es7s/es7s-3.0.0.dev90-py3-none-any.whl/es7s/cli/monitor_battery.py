...
class BatteryMonitor(CoreMonitor[BatteryInfo, CoreMonitorConfig]):
    ...
    def _init_settings(self) -> CoreMonitorSettings:
        return CoreMonitorSettings[CoreMonitorConfig](
            ...
            alt_mode=True,
            config=CoreMonitorConfig("monitor.battery"),
...
class _BatteryRenderer(GenericRenderer):
    BORDER_LEFT_CHAR = "▕"
    BORDER_RIGHT_CHAR = "▏"
    ...
    def __init__(
        ...
        self._remaining_formatter = pt.DynamicBaseFormatter(
            [
                pt.CustomBaseUnit("sec", 60),
                pt.CustomBaseUnit("min", 60),
                pt.CustomBaseUnit("hr", 24, collapsible_after=10),
                pt.CustomBaseUnit("day", overflow_after=365),
            ],
    ...
    def format(self, binfo: BatteryInfo, alter_mode_enabled: bool) -> pt.Text:
        ...
        if alter_mode_enabled:
            return pt.Text(self._format_remaining(binfo), fmt.level_style)
