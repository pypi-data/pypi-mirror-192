...
EPILOG_COMMON_OPTIONS = EpilogPart(
    "Run 'es7s options' to see the details of common options '-v', '-q', '-c', '-C' and '--tmux'.",
    group="run",
)
...
class HelpStyles(Styles):
    ...
    TEXT_INLINE_CONFIG_VAR = pt.Style(underlined=True)            # <- <section.opt> |< >| config variable name            #
...
def _catch_and_log_and_exit(func: F) -> F:
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.exception(e)
            _exit(1, "Terminating due to exception")
...
def _exit(code: int = 0, msg: str = "Terminating"):
