...
class WeatherMonitor(CoreMonitor[WeatherInfo, _WeatherMonitorConfig]):
    ...
    def _format_data_impl(self, msg: SocketMessage[WeatherInfo]) -> pt.Text:
        ...
        wicon_origin = fields[0].strip().removesuffix("\ufe0f")
        ...
        return (
            ...
            + pt.Fragment(temp_str.rjust(4), Styles.TEXT_MAIN_VALUE)
            + pt.Fragment(temp_unit.ljust(3), Styles.TEXT_AUXILIARY)
        )
...
class WeatherDemoComposer(GenericDemoComposer):
    ...
    def _render_footer(self):
        self._print_footer((self._col_width + 1) * (WeatherIconSet.MAX_SET_ID + 2) - 1)
