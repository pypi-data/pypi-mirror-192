...
from ._base import _exit, CliCommand, GroupOption
from ..shared import (
    get_logger,
    get_stdout,
    ShutdownableThread,
...
from ..shared.io import make_dummy_io

DT = t.TypeVar("DT")
CT = t.TypeVar("CT", bound="CoreMonitorConfig")

SEPARATOR_MAP: t.Dict[str, str] = {
    "none": "",
    "space": " ",
    "space2": "  ",
    "space3": "   ",
...
class CoreMonitorConfig:
    ...
    debugging_markup: bool = field(default=False, init=False)
    separator_left: RT = field(default="", init=False)
    separator_right: RT = field(default="", init=False)
    ...
    def update_from_config(self):
        self.is_enabled = get_config().getboolean(self._config_section, "enable")

        debugging_markup_glob = get_config().getboolean("monitor", "debug", fallback=False)
        debugging_markup_monitor = get_config().getboolean(
            self._config_section, "debug", fallback=False
        )
        self.debugging_markup = debugging_markup_glob or debugging_markup_monitor

        self.separator_left = self._resolve_separator("left")
        self.separator_right = self._resolve_separator("right")

        # also update specific monitors' settings in subclass method override

    def drop_separators(self):
        # for --demo mode
        self.separator_left = self.separator_right = SEPARATOR_MAP.get("none")

    def _resolve_separator(self, side: str) -> pt.Fragment:
        key = get_config().get(self._config_section, f"separator-{side}", fallback="none")
        val = SEPARATOR_MAP.get(key, None)
        if val is None:
            get_logger().warning(f'Invalid separator key: "{key}"')
            val = ""
        if not self.debugging_markup:
            return pt.Fragment(val)
        left = side == "left"
        val = val.replace(" ", ">" if left else "<")
        return pt.Fragment(val, "#7280e2" if left else "#7e59a9")
...
class RatioStyleMap(deque[RatioStyle]):
    def __init__(self, values: t.Sequence[RatioStyle]):
        cur_max_ratio = 0.0
        for value in values:
            if value.ratio_upper_threshold <= cur_max_ratio:
                raise ValueError(f"Ratio style values must be a nondecreasing sequence: {values}.")
            cur_max_ratio = value.ratio_upper_threshold
            get_logger().debug(f"Registered new ratio -> style mapping: {value}")
        super().__init__(values, maxlen=len(values))

    def find(self, cur_ratio: float) -> pt.Style:
        for val in self:
...
class CoreMonitorState:
    ...
    def __repr__(self):
        parts = (
            ...
            f"msg={self.last_valid_msg.timestamp}" if self.last_valid_msg else "",
            f'data="{self.data_fmtd!r}"' if isinstance(self.data_fmtd, pt.IRenderable) else "",
            f"ratio={100*self.ratio:>.1f}%" if self.ratio else "",
        )
    ...
    def log(self):
        get_logger().debug(f"State: " + repr(self))
...
class CoreMonitor(ShutdownableThread, t.Generic[DT, CT], ABC):
    ...
    def __init__(self, ctx: click.Context, demo: bool, **kwargs):
        ...
        self._socket_client = SocketClient(
            ...
            self._setup.socket_receive_interval_sec,
        ...
        if demo:
            try:
                self._renderer.set_output_stream(make_dummy_io())
    ...
    def _init_renderer(self) -> IMonitorRenderer:
        renderer_t = self._setup.renderer or GenericRenderer
    ...
    def _run_ui_thread(self):
        ...
        self._socket_client_ready.wait(1.0)
    ...
    def _get_retry_on_error_delay_sec(self) -> float:
        ...
        #   25 sec on 3rd
        #   2 min on 4th
        #   exit on 5th, so that tmux can restart the monitor
    ...
    def _format_data(self, msg: SocketMessage[DT]):
        ...
        elif isinstance(data_fmtd, list):
    ...
    def _update(self):
        ...
        try:
            ...
            if (
                ...
                or not self._setup.cache_output
            ):
                logger.debug(f"Received {len(msg_raw)} bytes of data")
                logger.trace(msg_raw, label="Received data dump")
                logger.debug("Deserialized message: " + repr(msg))
            ...
            self._last_success_msg_timestamp = msg.timestamp
            self._add_timeout(self._setup.update_interval_sec)
            return
    ...
    def _increase_error_amount(self):
        ...
        if self._state.error_amount >= self.RETRY_ON_ERROR_MAX_ATTEMPTS:
            _exit(1, "Terminating due to max retry amount exceeded")
    ...
    def _update_from_config(self):
        ...
        is_enabled_newval = self._setup.config.is_enabled
        if not is_enabled_newval and self._state.is_enabled:
            get_logger().info(f"Disabling {self} due to config setting")
            get_logger().info(f"Pausing {self._socket_client} due to config setting")
            self._socket_client_pause.set()
        elif is_enabled_newval and not self._state.is_enabled:
            get_logger().info(f"Enabling {self} due to config setting")
            get_logger().info(f"Unpausing {self._socket_client} due to config setting")
            self._socket_client_pause.clear()
        self._state.is_enabled = is_enabled_newval
...
class IMonitorRenderer(ABC):
    ...
    @abstractmethod
    def set_output_stream(self, io: IoProxy):
        raise NotImplementedError
...
class GenericRenderer(IMonitorRenderer):
    def __init__(
        ...
        monitor_setup: CoreMonitorSettings,
        monitor_state: CoreMonitorState,
        output_alt_st: pt.Style = pt.Style(italic=True),
    ):
        self._output_width = output_width
        self._stdout: IoProxy = get_stdout()

        self._output_alt_st = output_alt_st
        self._output_network_comm = pt.Fragment("·", pt.Style(fg=get_config().get_theme_color()))

        self._output_init = self._make_fixed_width_output("...", Styles.TEXT_DISABLED)
        self._output_disabled = self._make_fixed_width_output("-"*(min(3, self._output_width-2)), Styles.TEXT_DISABLED)
        ...
        self._debug_override = pt.Style(bg="#a58dbe", underlined=True, blink=False).autopick_fg()
    ...
    def _apply_debugging_markup(self, output: RT) -> pt.Text:
        ...
        for frag in output._fragments:
            frag = pt.Fragment(frag.string.replace(" ", "␣"), frag.style)
    ...
    def update_primary(self) -> str:
        ...
        if self._msetup.alt_mode and self._mstate.is_alt_mode:
            result.append(pt.Fragment(fmt=self._output_alt_st, close_this=False))

        if sep_left := self._msetup.config.separator_left:
            result.append(sep_left)

        if self._msetup.network_comm_indic:
        ...
        if sep_right := self._msetup.config.separator_right:
            result.append(sep_right)
    ...
    def render_progress_bar(self, result_parts: list[RT], max_len: int) -> pt.Text:
        ...
        if self._mstate.ratio:
            overw_st.append(self._msetup.ratio_styles_map.find(self._mstate.ratio))
            fill_threshold = round(max_len * self._mstate.ratio)

        result = pt.Text()
        while len(copy.copy(result_parts)) > 0:
            result_part = result_parts.pop(0)
            # if not len(result_part):
            #    continue
            ...
            for frag in frags:
                # if not len(frag):
                #    continue
                colorized_st = pt.merge_styles(frag.style, overwrites=overw_st)
                ...
                if len(result) < fill_threshold <= len(result) + len(frag):
                    ...
                    if right_part:
                        result_parts.insert(0, pt.Fragment(right_part, frag.style))
...
class GenericDemoComposer(IDemoComposer):
    def __init__(self, monitor: CoreMonitor):
        ...
        self._monitor._setup.config.drop_separators()
    ...
    def _render_msg(self, msg: SocketMessage):
    ...
    def _print_horiz_sep(self, label: str, width: int, fill: str = " "):
        stdout = get_stdout()
        label = label.center(width, fill)
        st = pt.Style(self._table_st, underlined=True, overlined=True)
