...
def get_opt_set(G,vertex1,vertex2):
    ...
    causal_nodes = [item for item in G.descendants([vertex1]) if item in G.ancestors([vertex2])]
    ...
    forb_set = forb_set + [vertex1] #The forbidden set characterizes those covariates that may never be included into a valid adjustment set
...
def get_opt_set_from_set(G,node_set1,node_set2):
    ...
    causal_nodes = [item for item in G.descendants(node_set1) if item in G.ancestors(node_set2) ]
    ...
    for element in input_set:
        ...
        flag_new_set = G.d_separated(element,node_set2,condition_set) # check if d-separated 
        if flag_new_set:
            input_set = new_set
...
def get_min_set(G,input,vertex1, vertex2):
    ...
    for element in G.children([vertex1]):  #prepare for d-separation test, construct a SWIG
        G.delete_diedge(vertex1, element) 

    for element in input_set:
        new_set = [item for item in input_set if item not in element]
        is_d_separated = G.d_separated(vertex1,vertex2,new_set) # check if d-separated 
        if is_d_separated:
            is_minimal_set = 1
...
class OptAdjustment:
    ...
    def generate_opt_adjustment_set(self, vertex1, vertex2):
            ...
            if is_d_separated:
                logger.info("Tested vertices are independent so no set is needed to condition on")
                return []
