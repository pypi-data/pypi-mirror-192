...
class DatosCROM(TablasVC):
    ...
    def incidencias_resumen_status(self):
        return (self.incidencias_redux
            .set_index('ID')
            .query('Status == "CERRADA COMPLETA"')
            .groupby(['Nemo','Reason'],as_index=False)
            [['Origin']]
            .count()
            .rename(columns={'Origin':'Cantidad'})
            .pivot_table(
                index='Nemo',
                columns=['Reason'],
                values='Cantidad',
            ...
            .sort_values(by='CERRADA COMPLETA',ascending=False)
            .pipe(lambda adf: pd.concat([adf, adf.sum().to_frame(name='Total').T]))
            .merge(
                right = self.incidencias_redux[['Nemo','Owner']].drop_duplicates().set_index('Nemo'),
                left_index=True,
                right_index=True,
                how='left'
            )
            .loc[:,['Owner','ABIERTA','VISADA','CERRADA COMPLETA','FAILURE','LIMITATION','MAPRO','QNIGHT']]
        )
