...
class Page:
    ...
    def get(self):
        ...
        if "missing" in data:
            ...
            data.pop("revisions")
            data.update(rev)
            self.origin = self._cont = data["content"]
            self.info = data
            # print(self.cont)
    ...
    def get_revisions(self,
        ...
        end: time.struct_time | None= None,
        dir = "older", prop = []) -> list:
        """ 판 목록 반환
...
{self.cont}'''
    ...
    def last_line(self) -> str:
        return self.cont.splitlines().pop()
    ...
    def append(self, string: str, dup: bool = True) -> str:
        ...
        if dup is True or (self.cont.find(string) == -1 and dup is False):
    ...
    def save(self,
        ...
        Args:
            summary (str, optional): 편집 요약.
            nocreate (_type_, optional): 문서 생성 안함(오직 편집만)
            minor (_type_, optional): 사소한 편집 표시
            delay (float, optional): 딜레이
        ...
        if "edit" in json:
            if "nochange" in json["edit"]:
                ...
                if "contentmodel" in json["edit"]:
                    ...
                    # """if 'result' in json['edit']:
                    # print(json['edit']['result'])"""
