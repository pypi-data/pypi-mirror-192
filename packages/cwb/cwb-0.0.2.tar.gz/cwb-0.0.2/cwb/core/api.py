...
from .color_cmdline import cprint, COLORS, cstring
...
# --------------------
    def get(self, params: dict) -> dict:
        ...
        response = super().get(url=self.url, params=request)
    ...
    def post(self, data: dict, files= None):
        ...
        response = super().post(url=self.url, data=request, files=files)
    ...
    def logout(self):
        if self.login_status is False:
            ...
            if "error" in data:
                ...
                self.login_status = False
                print(cstring("로그아웃", COLORS.SUCCESS).string + ":" + self.username)
