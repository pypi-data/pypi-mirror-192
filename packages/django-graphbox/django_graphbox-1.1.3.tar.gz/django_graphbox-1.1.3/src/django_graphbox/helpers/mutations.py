        ...
        if valid:
            try:
                ...
                if valid_operation:
                    ...
                    user_metadata={}
                    for field in actual_user_instance._meta.get_fields():
                        if not field.is_relation:
                            user_metadata[field.name]=getattr(actual_user_instance, field.name)
                    request_metadata={
                            'ip':info.context.META.get('REMOTE_ADDR'),
                            'user_agent':info.context.META.get('HTTP_USER_AGENT'),
                            'method':info.context.META.get('REQUEST_METHOD'),
                            'path':info.context.META.get('PATH_INFO'),
                            'query_string':info.context.META.get('QUERY_STRING'),
                            'content_type':info.context.META.get('CONTENT_TYPE'),
                            'content_length':info.context.META.get('CONTENT_LENGTH'),
                        }
