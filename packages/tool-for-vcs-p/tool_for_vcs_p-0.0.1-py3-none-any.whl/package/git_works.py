...
import package.work_args as wa
import package.parse_works as pa
...
def work_init():
    ...
    pa.__add_profile(ca.GC_DEFAULT_PROFILE_NAME)
    pa.__save_cfg_file()


def work_show_repos():
    isValid, repo_paths = pa.get_check_repos_paths()
    if not isValid:
        print("Error: cannot load repos paths!" /
              "Please check profiles/<active_profile>/repos in " + str(ca.GC_VCS_P_CFG))
        return

    isValid, repo_names = pa.get_check_repos_names()
    if not isValid:
...
def work_status():
    ...
    isValid, repos_paths = pa.get_check_repos_paths()
    if not isValid:
        return

    repos_str = pa.__build_repos_str(repos_paths)
...
def work_pull():
    ...
    isValid, branch_name, _, repos_paths = pa.get_check_meta_info()
    if not isValid:
        return
...
def work_orep(repo):
    isValid, repos_names = pa.get_check_repos_names()
    if not isValid:
        return
    ...
    if repo not in repos_names:
        print("Bad repo name (string supposed): " + str(repo) + \
              ...
              "\nor use " + str(wa.GC_ARG_SHOW_REPOS) + " to show" + \
...
def work_add():
    ...
    isValid, repos_names = pa.get_check_repos_paths()
    if not isValid:
        return

    repos_str = pa.__build_repos_str(repos_names)
...
def work_commit():
    ...
    if ca.G_CFG["editor_commit"] == ca.GC_EDITOR_COMMIT_DEFAULT:
        pa.__commit_default_editor(commit_file_path)
    else:
        pa.__commit_alternative_editor(commit_file_path)
...
def check_main_branch():
    profile = pa.__get_profile()
    branch_name = profile["branch_name"]
    current_branch = OS.path.split(GIT.Repository('..').head)[1]
