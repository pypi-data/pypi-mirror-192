...
def get_cache_dir_path() -> Path:
    ...
    current_notebook = get_notebook_if_appropriate()

    cache_dir = None

    if working_environment is WorkingEnvironment.SCRIPT:
        cache_dir = Path(__main__.__file__).parent / CACHE_DIR_NAME
    if working_environment is working_environment.NOTEBOOK:
        if not current_notebook:
            raise RuntimeError
        cache_dir = Path(current_notebook.path).parent / CACHE_DIR_NAME
...
def get_notebook_if_appropriate() -> Optional[jupyter_notebook.Notebook]:
    working_environment = _get_working_environment()
