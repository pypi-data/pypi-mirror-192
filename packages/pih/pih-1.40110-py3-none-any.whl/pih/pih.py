...
from pih.const import CONST, FIELD_NAME_COLLECTION, FIELD_COLLECTION, FILE, PASSWORD, PATHS, USER_PROPERTY, WorkstationMessageMethodTypes, LogChannels, LogCommands, LogLevels, MarkType, PolibasePersonReviewQuestStep, ServiceCommands, ServiceRoles, Settings, MessageTypes, MessageStatus
...
class PIH:
    ...
    class ERROR:
        ...
        class POLIBASE:
            ...
            @staticmethod
            def get_not_found_error(title: str, value: str, start: str = "Пациент/Клиент") -> str:
    ...
    class UPDATER:
        ...
        def update_remote(host: str, show_output: bool = False) -> bool:
            remote_executor_command_list: List[str] = PIH.PSTOOLS.create_command_list_for_remote_process_executor(
                host, True)
            command_list: List[str] = remote_executor_command_list + \
    ...
    class SETTINGS:
        ...
        class POLIBASE:
            ...
            class VISIT:
                ...
                @staticmethod
                def get_offer_telegram_bot_url_text(person: PolibasePerson) -> str:
                    return  PIH.SETTINGS.get(Settings.POLIBASE_PERSON_PERSONLESS_TAKE_TELEGRAM_BOT_URL_TEXT) if person is None else str(PIH.SETTINGS.get(Settings.POLIBASE_PERSON_TAKE_TELEGRAM_BOT_URL_TEXT)).format(name=FullNameTool.to_given_name(person.FullName)) 

    class PSTOOLS:

        @staticmethod
        def create_command_list_for_remote_process_executor(command_list: List, host: str, login: str = None, password: str = None, interactive: bool = False, as_admin: bool = False) -> List[str]:
            host = "\\\\" + host
            ...
            result: List[str] = [ps_executor_path, "-nobanner", "-accepteula", host,
                                DataTool.triple_bool(interactive, "-d", "-i", "-h"), "-u", user, "-p", password] + command_list
            if as_admin:
                result.append("-s")
            return result
        ...
        def create_remote_process_executor_for_service_role(value: ServiceRoles, interactive: bool = False) -> List[str]:
            ...
            return PIH.PSTOOLS.create_command_list_for_remote_process_executor([CONST.PYTHON.EXECUTOR], PIH.SERVICE.get_host(value), service_role_value.login, service_role_value.password, interactive)
    ...
    class SERVICE:
        ...
        class ADMIN:
            ...
            def develope(service_role: ServiceRoles, port: int = None) -> None:
                ...
                service_role_description.keep_alive = False
    ...
    class DATA:
        ...
        class FORMAT:
            ...
            def telephone_number_for_whatsapp(value: str) -> str:
                return PIH.DATA.FORMAT.telephone_number(value, CONST.MESSAGE.WHATSAPP.TELEPHONE_NUMBER_PREFIX)
        ...
        class TELEPHONE_NUMBER:
            ...
            def all(active: bool = True) -> List[str]:
                ...
                return ResultTool.data_map(ResultTool.data_filter(PIH.RESULT.USER.by_name("*", active=active), filter_function), map_function).data
            ...
            @staticmethod
            def by_wappi_profile_id(value: CONST.MESSAGE.WHATSAPP.WAPPI.PROFILE_ID) -> str:
                map : dict = {
                    CONST.MESSAGE.WHATSAPP.WAPPI.PROFILE_ID.CALL_CENTRE: PIH.DATA.TELEPHONE_NUMBER.call_centre_administrator(),
                    CONST.MESSAGE.WHATSAPP.WAPPI.PROFILE_ID.IT: PIH.DATA.TELEPHONE_NUMBER.it_administrator(),
                    CONST.MESSAGE.WHATSAPP.WAPPI.PROFILE_ID.MARKETER: PIH.DATA.TELEPHONE_NUMBER.marketer(),
                }
    ...
    class RESULT:
        ...
        class MESSAGE:
            ...
            class DELAYED:
                ...
                def get(search_condition: MessageSearchCritery = None, take_to_work: bool = False) -> Result[List[DelayedMessageVO]]:
                    return DataTool.to_result(
                        RPC.call(ServiceCommands.search_delayed_messages, (search_condition, take_to_work)), DelayedMessageVO)
        ...
        class WORKSTATION:
            ...
            @staticmethod
            def by_internal_telephone_number(value: int) -> Result[UserWorkstation]:
                result: Result[List[UserWorkstation]] = PIH.RESULT.WORKSTATION.all_with_user()
                workstation_list: List[UserWorkstation] = result.data
                result_worksation: UserWorkstation = None
            ...
            @staticmethod
            def by_any(value: Any) -> Result[List[UserWorkstation]]:
        ...
        class POLIBASE:
            ...
            class NOTIFICATION:
                ...
                class CONFIRMATION:
                    ...
                    def by(recipient: str, sender: str) -> Result[PolibasePersonNotificationConfirmation]:
                        return DataTool.to_result(RPC.call(ServiceCommands.get_polibase_person_notification_confirmation, PolibasePersonNotificationConfirmation(recipient, sender)), PolibasePersonNotificationConfirmation)
            ...
            def person_by_telephone_number(value: str, test: bool = None) -> Result[List[PolibasePerson]]:
                value = PIH.DATA.FORMAT.telephone_number_for_whatsapp(value)
                ...
                if ResultTool.data_is_empty(result):
                    raise PIH.ERROR.POLIBASE.get_not_found_error("идентификационным номером", value)
                return result
            ...
            def person_creator_by_pin(value: int, test: bool = None) -> Result[PolibasePerson]:
                ...
                if ResultTool.data_is_empty(result):
                    raise PIH.ERROR.POLIBASE.get_not_found_error(
            ...
            def persons_by_full_name(value: str, test: bool = None) -> Result[List[PolibasePerson]]:
                ...
                if ResultTool.data_is_empty(result):
                    raise PIH.ERROR.POLIBASE.get_not_found_error("именем", value)
                return result
        ...
        class USER:
            ...
            def by_internal_telephone_number(value: int) -> Result[User]:
                workstation_list: list[UserWorkstation] = PIH.RESULT.WORKSTATION.all_with_user().data
                result_worksation: UserWorkstation = None
                for workstation in workstation_list:
                    if not DataTool.is_empty(workstation.description):
                        index: int = workstation.description.find(CONST.INTERNAL_TELEPHONE_NUMBER_PREFIX)
            ...
            @staticmethod
            def by_job_position(job_position: CONST.AD.JobPisitions) -> Result[List[User]]:
            ...
            def by_group(group: CONST.AD.Groups) -> Result[List[User]]:
                return DataTool.to_result(RPC.call(ServiceCommands.get_users_by_group, group.name), User)
    ...
    class CHECK:
                ...
                class CONFIRMATION:
                    ...
                    def exists(recipient: str, sender: str, state: int = None) -> bool:
                        result: Result[PolibasePersonNotificationConfirmation] = PIH.RESULT.POLIBASE.NOTIFICATION.CONFIRMATION.by(
                            recipient, sender)
                        return not ResultTool.data_is_empty(result) and (True if state is None else result.data.state == state)
        ...
        class WORKSTATION:
            ...
            @staticmethod
            def watchable(workstation: Workstation) -> bool:
                return PIH.CHECK.WORKSTATION.has_property(workstation, CONST.AD.WSProperies.Watchable)

            @staticmethod
            def poweroffable(workstation: Workstation) -> bool:
                return PIH.CHECK.WORKSTATION.has_property(workstation, CONST.AD.WSProperies.Poweroffable)

        @staticmethod
        def telephone_number(value: str, whatsapp_number: bool = False) -> bool:
            exp = ("" if whatsapp_number else r"^\+") + "[0-9]{11,13}$"
            return not DataTool.is_empty(value) and re.fullmatch(exp, value) is not None

        @staticmethod
        def telephone_number_for_whatsapp(value: str) -> bool:
    ...
    class LOG:
        ...
        class COMMAND:
            ...
            @staticmethod
            def polibase_person_visit_notification_was_registered(visit: PolibasePersonVisitDS, notification: PolibasePersonVisitNotificationVO) -> None:
    ...
    class MESSAGE:
        ...
        class WORKSTATION:
            ...
            @staticmethod
            def send_to_all_workstations(message: str) -> None:
                if PIH.session.authenticate() and PIH.CHECK.ACCESS.admin(True):
                    def filter_function(workstation: UserWorkstation) -> bool:
                        return workstation.accessable
                    def every_action(workstation: UserWorkstation) -> None:
                        print(workstation.samAccountName)
                    ResultTool.every(ResultTool.data_filter(PIH.RESULT.WORKSTATION.all_with_user(), filter_function), every_action)

            @staticmethod
            def send_to_user(user: User, message: str, method_type: WorkstationMessageMethodTypes = WorkstationMessageMethodTypes.REMOTE) -> bool:
                return PIH.MESSAGE.WORKSTATION.send_to_user_or_workstation(user.samAccountName, None, message, method_type)

            @staticmethod
            def send_to_workstation(value: Workstation, message: str, method_type: WorkstationMessageMethodTypes = WorkstationMessageMethodTypes.REMOTE) -> bool:
                return PIH.MESSAGE.WORKSTATION.send_by_workstation_name(value.name, message, method_type)

            @staticmethod
            def send_by_workstation_name(value: str, message: str, method_type: WorkstationMessageMethodTypes = WorkstationMessageMethodTypes.REMOTE) -> bool:
                user: User = None
                try:
                    user = PIH.RESULT.USER.by_workstation_name(value).data
                except NotFound:
                    pass
                return PIH.MESSAGE.WORKSTATION.send_to_user_or_workstation(user.samAccountName if user is not None else None, value, message, method_type)

            @staticmethod
            def send_to_user_or_workstation(user_login: str, workstation_name: str, message: str, method_type: WorkstationMessageMethodTypes = WorkstationMessageMethodTypes.REMOTE) -> bool:
                ...
                def internal_send_by_login_and_workstation_name(login: str, workstation_name: str) -> None:
                    if method_type == WorkstationMessageMethodTypes.LOCAL_PSTOOL_MSG:
                        PIH.PSTOOLS.run_command(PIH.PSTOOLS.create_command_list_for_remote_process_executor(
            ...
            def by_login(login: str, message: str, method_type: WorkstationMessageMethodTypes = WorkstationMessageMethodTypes.REMOTE) -> bool:
                return PIH.MESSAGE.WORKSTATION.send_to_user_or_workstation(login, None, message, method_type)
        ...
        class WHATSAPP:
            ...
            class WAPPI:
                ...
                def get_wappi_collection() -> dict:
                    PC = CONST.MESSAGE.WHATSAPP.WAPPI.PROFILE_ID
                    result: dict = PIH.MESSAGE.WHATSAPP.WAPPI.WAPPI_PROFILE_MAP or {
                        PC.IT: PIH.DATA.TELEPHONE_NUMBER.it_administrator(),
                        PC.CALL_CENTRE: PIH.DATA.TELEPHONE_NUMBER.call_centre_administrator(),
                        PC.MARKETER: PIH.DATA.TELEPHONE_NUMBER.marketer()
                    }
                    if PIH.MESSAGE.WHATSAPP.WAPPI.WAPPI_PROFILE_MAP is None:
                        PIH.MESSAGE.WHATSAPP.WAPPI.WAPPI_COLLECTION = result
                    return result

                @staticmethod
                def get_profile_id(telephone_number: str) -> CONST.MESSAGE.WHATSAPP.WAPPI.PROFILE_ID:
                    if PIH.CHECK.telephone_number_for_whatsapp(telephone_number):
                ...
                @staticmethod
                def message_list(telephone_number: str, profile_id: Any = None) -> List[WhatsAppMessage]:
        ...
        class DELAYED:
            ...
            @staticmethod
            def send(message: DelayedMessage, register: bool = True, high_priority: bool = True) -> bool:
                return DataTool.rpc_unrepresent(RPC.call(ServiceCommands.send_delayed_message, (PIH.ACTION.MESSAGE.DELAYED.prepeare_message(message), register, high_priority)))
                ...
                def prepeare_message(message: DelayedMessage) -> DelayedMessage:
                    ...
                    if  message.type == MessageTypes.WHATSAPP.value and not DataTool.is_empty(message.recipient):
                        if PIH.CHECK.telephone_number(message.recipient):
                            #+7 -> 7
                            message.recipient = PIH.DATA.FORMAT.telephone_number(message.recipient, CONST.MESSAGE.WHATSAPP.TELEPHONE_NUMBER_PREFIX)
                    return message
        ...
        class POLIBASE:
            ...
            class SETTINGS:
                ...
                def set_for_workstation(name: str, test: bool = False, show_result: bool = False) -> None:
                    if PIH.CHECK.WORKSTATION.exists(name):
                        ...
                        process_result: CompletedProcess = PIH.PSTOOLS.run_command(PIH.PSTOOLS.create_command_list_for_remote_process_executor(["powershell", PathTool.resolve(os.path.join(
            ...
            class NOTIFICATION:
                ...
                class CONFIRMATION:
                    ...
                    @staticmethod
                    def complete(recepient: str, sender: str, state: int = 1) -> bool:
                        return DataTool.rpc_unrepresent(RPC.call(ServiceCommands.complete_polibase_person_notification_confirmation, PolibasePersonNotificationConfirmation(recepient, sender, state)))
            ...
            class VISIT:
                ...
                @staticmethod
                def register(value: PolibasePersonVisitDS = None) -> bool:
