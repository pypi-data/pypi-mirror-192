...
def validar_padron(dire: DataFrame, padweb: DataFrame, ue: DataFrame, nivel: str = 'cod_mod') -> DataFrame:
    ...
    '''
    Validaciones del padrón de la intervención donde se verifican: el código local, el nombre del pliego, nombre de la unidad ejecutora, nombre de la ugel y nombre de la institución educativa considerando como dato de validación a los códigos modulares. Se usan como insumos de validación al Padrón web de Escale y a la Base UE UGEL del equipo Analítica de Datos
    ...
    ue: DataFrame
        Base UE UGEL del equpipo Analítica de Datos

    nivel: str, por defecto cod_mod
        Nivel de contratación de la intervención

    Ejm:
    ----

    >>> metas = ct.validar_padron(dire, padweb, ue, 'ugel')
    ...
    if nivel == 'cod_mod':
        ...
        padweb_i=padweb_a_p[['COD_MOD','CODOOII','CODLOCAL','ANEXO','CEN_EDU','D_NIV_MOD']]
        ...
        ue_i.rename(columns={'NOM_PLIEGO':'nom_pliego'},inplace=True)
        ue_i.rename(columns={'NOM_UE':'nom_ue'},inplace=True)
        ue_i.rename(columns={'UGEL':'ugel_dire'},inplace=True)
        ...
        dire_padweb_ue=pd.merge(dire, ue_i, on =['ugel_dire'], how ='left',indicator='merge_2')

        # Variables de interés

        bint=dire_padweb_ue[['nom_pliego_dire','nom_pliego','nom_ue_dire','nom_ue','ugel_dire']]
