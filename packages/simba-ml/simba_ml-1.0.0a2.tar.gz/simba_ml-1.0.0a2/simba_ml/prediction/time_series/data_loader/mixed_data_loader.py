...
from simba_ml.prediction.time_series.config.mixed_data_pipeline import data_config
...
class MixedDataLoader:
    ...
    def __init__(self, config: data_config.DataConfig) -> None:
        ...
        self.__list_of_train_validation_sets = [
            [] for _ in range(len(self.config.ratios))
        ]
    ...
    def load_data(self) -> Tuple[list[pd.DataFrame], list[pd.DataFrame]]:
        ...
        synthetic = (
            ...
            if self.config.simulated is None
                ...
                os.getcwd() + self.config.simulated
            )
        )
        observed = (
            []
            if self.config.real is None
            else preprocessing.read_dataframes_from_csvs(os.getcwd() + self.config.real)
        )
