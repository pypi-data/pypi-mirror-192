...
def spimx(word,model,words_list1,words_list2,Google):
    import re
    import nltk
...
def rasv(word,model,words_list1,words_list2):
    ...
    def get_answer(input_word, words_list1, words_list2):
        ...
        if similar_word:
            ...
            return "No text for this input found in dataset"
            #from googlesearch import search
            #import requests
            #from bs4 import BeautifulSoup
...
def model_load(modeltype,input, model,Google):
    global words_list1
    global words_list2
    global model_name
    input = input.lower()
    ...
    if modeltype == "spim":
        ...
        if Google == True:
           return spimx(input,model,words_list1,words_list2,True)
        else:
