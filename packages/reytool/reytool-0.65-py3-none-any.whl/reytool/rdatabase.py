...
class RConn(object):
    ...
    def execute(
        self,
        sql: str,
        parms: Optional[Union[List[Dict], Dict]] = None,
        database: Optional[str] = None,
        fill_field: bool = True,
        none_values: List = none_values,
        once: bool = None,
        ...
        **kw_parms: Any
        ...
        sql = text(sql)
        if report:
            result, report_runtime = runtime(conn.execute, sql, parms, _ret_report=True)
            rowcount = result.rowcount
            report = "%s\nRow Count: %d\n\n%s\n\n%s" % (report_runtime, rowcount, sql, parms)
            rprint(report, title="SQL", frame="half")
        else:
