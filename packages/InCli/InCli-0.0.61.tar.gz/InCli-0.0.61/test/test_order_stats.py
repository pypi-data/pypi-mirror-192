...
class Test_Order_stats(unittest.TestCase):
    ...
    def test_order(self):
        ...
        q = f"select fields(all) from orderitem where OrderId='{self.orderId}' limit 200"
        ...
        for x,order_item in enumerate(order_items['records']):
         #   attr_str = order_item['vlocity_cmt__AttributeSelectedValues__c']
            ...
            prod_attr = simplejson.loads(prod_attr_str)
            print(f"- Line {x+1} - Product:{product['Name']}  {product['ProductCode']}  Action:{order_item['vlocity_cmt__Action__c']}")
            for cat in prod_attr['records']:
                for patt  in cat['productAttributes']['records']:
                    count = len(patt['values']) if 'values' in patt and patt['inputType']=='dropdown' else 1
                    values = [value['value'] for value in patt['values']] if patt['inputType'] == 'dropdown' else []
                    val = ", ".join(values)
                    print(f"       Attribute: {patt['inputType']:<10} ({count:>2})  {val}")
                    if patt['inputType']=='dropdown':
                        a=1
        ...
        for y,fr in enumerate(frs['records']):
            ...
            for frl in frl_s:
                ...
                mu_str = frl['vlocity_cmt__AttributesMarkupData__c']
                mu = simplejson.loads(mu_str)
                at_str = frl['vlocity_cmt__JSONAttribute__c']
                at = simplejson.loads(at_str)
                print(f" - Line {y+1} {frl['Name']}  Product:{frl_prod['Name']}   Action:{frl['vlocity_cmt__Action__c']}")  
                ...
                for frl_att in at.keys():
                    ...
                    for frl_att_a in frl_att_as:
                        ...
                        if frl_att_a['attributeRunTimeInfo']['dataType']=='Picklist':
                            a=1
    ...
    def parse_decomposition_mapping(mapping,mappings):
    ...
    def get_product_decomposition(self,product):
        ...
        for record in res['records']:
            ...
            mappring_data = simplejson.loads(record['vlocity_cmt__MappingsData__c']) if mappring_data_str != None else None
            ...
            decomposition = {
                ...
                'source_Id':record['vlocity_cmt__SourceProductId__c'],
                'mappings':mappings
            }
