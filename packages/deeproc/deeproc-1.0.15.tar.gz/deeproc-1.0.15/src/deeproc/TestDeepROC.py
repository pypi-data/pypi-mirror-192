...
costs       = dict(cFP=1, cFN=3, cTP=0, cTN=0, costsAreRates=False)  # depends on the dataset
...
def testDeepROCfolds(descr, matlabFiles=[536, 581, 643], groupAxis='FPR',
                     groups=[[0.0, 1/3], [1/3, 2/3], [2/3, 1.0]],
                     costs=dict(cFP=1, cFN=1, cTP=0, cTN=0, costsAreRates=False),
                     showPlot = True):
    ...
    ######################
    print('\nDeepROC analysis for multiple folds:')
    aDeepROC = DeepROC()
    ...
    foldScores     = []
    foldLabels     = []
    # pretend that each result file is a different fold for the same model
    for fileNum in matlabFiles:
        fileName   = f'input/result{fileNum}.mat'  # a matlab file (or future: csv file) for input
        try:
            fileContent = sio.loadmat(fileName)  # handle any file not found errors naturally
            foldScores.append(fileContent[scoreVariable])
            foldLabels.append(fileContent[targetVariable])
        except:
            raise ValueError(f'File {fileName} is either not found or is not a matlab file')
        #endtry
    #endfor

    numFolds = len(matlabFiles)
    for i in range(0, numFolds):
        fpr_temp, tpr_temp, threshold_temp = roc_curve(foldLabels[i], foldScores[i])
        aDeepROC.set_fold(fpr=fpr_temp, tpr=tpr_temp, threshold=threshold_temp)
    #endfor

    labels = foldLabels[0].flatten()
    for i in range(1, numFolds):
        labels = labels + foldLabels[i].flatten()
    labels = list(labels)
...
#endif
testDeepROC(testNum, scores, labels, groupAxis=groupAxis, groups=groups,
            costs=costs, showPlot=True)
testDeepROCfolds(testNum, matlabFiles=myFolds, groupAxis=groupAxis, groups=groups,
                 costs=costs, showPlot=True)
