...
.. currentmodule:: ghostbusterV4.scan
...
def assume_role(role_arn):
    ...
    assumed_role_object = sts.assume_role(
        RoleArn=role_arn,
        RoleSessionName="ghostbusterV4-session"
    )
...
@click.option(
    ...
    help="Manually specify DNS records to check against. ghostbusterV4 will check these IPs after checking retrieved DNS "
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Skip printing the ASCII art when starting up ghostbusterV4.",
)
@click.option(
    "--profile",
    default="",
    help="Specify a specific AWS profile to run ghostbusterV4 on.",
)
@click.option(
    "--roles",
    required=False,
    type=click.Path(exists=True),
    help="Specify CSV filename with AWS account IDs to run ghostbusterV4 on. Each account must have ghostbusterV4 role "
         "assumable by ghostbusterV4 ec2/lambda/whatever is running ghostbusterV4. Role name: ghostbusterV4TargetAccountRole."
...
@click.option(
    ...
    help="Like --roles, but finds all organisation accounts automatically. The argument value should be ARN of a role "
         "with organizations:ListAccounts and organizations:DescribeAccount. Ec2/lambda/whatever is running ghostbusterV4"
         " must have permissions to assume the organisation lookup role."
)
@click.option(
    "--whitelist", default="", help="Comma delimited list of IPS to whitelist."
)
...
def aws(
    ...
    for account_id in account_ids:
        role_arn = "arn:aws:iam::{0}:role/ghostbusterV4TargetAccountRole".format(account_id)
        try:
    ...
    for record_set in dns_ec2_ips:
        for record in record_set["records"]:
            if(not check_whitelist(whitelist_ips,record)):
