...
class Statistics:
    ...
    def moving_avg(args:(list|np.ndarray),size:int = 2):
        r"""
    ...
    def standard_deviation(args:(list|np.ndarray)):
    ...
    def sample_variance(args:(list|np.ndarray)):
        mean = sum(args)/len(args)
        rep = []
        for i in range(0,len(args)):
            a = (args[i]-mean)**2
    ...
    def population_variance(args:(list|np.ndarray)):
        mean = sum(args)/len(args)
        rep = []
        for i in range(0,len(args)):
            a = (args[i]-mean)**2
...
class Matrix:
    ...
    def inverse_matrix(X:np.matrix):
        try:
