This is the VaultDB Python package
...
Next, copy over the python package related files, and install the package.
    ...
    mkdir -p $VAULTDB_PREFIX/src/vaultdb-pythonpkg
    tar --directory=$VAULTDB_PREFIX/src/vaultdb-pythonpkg -xzpf tools/pythonpkg/dist/vaultdb-${SETUPTOOLS_SCM_PRETEND_VERSION}.tar.gz
    pip3 install --prefix $VAULTDB_PREFIX -e $VAULTDB_PREFIX/src/vaultdb-pythonpkg/vaultdb-${SETUPTOOLS_SCM_PRETEND_VERSION}

## Development and Stubs

`*.pyi` stubs are generated with [Mypy's `stubgen`](https://mypy.readthedocs.io/en/stable/stubgen.html) and tweaked. These are important for autocomplete in many IDEs, as static-analysis based language servers can't introspect `vaultdb`'s binary module.
...
// def query_df(self, df: object, virtual_table_name: str, sql_query: str) -> VaultDBPyRelation: ...
pybind_opts.disable_function_signatures();
m.def("query_df", &VaultDBPyRelation::QueryDF,
      "query_df(self, df: pandas.DataFrame, virtual_table_name: str, sql_query: str) -> VaultDBPyRelation \n"
...
// def query_df(self, df: pandas.DataFrame, virtual_table_name: str, sql_query: str) -> VaultDBPyRelation: ...
```
...
# hint: git add -p; git checkout HEAD tools/pythonpkg/vaultdb-stubs)
