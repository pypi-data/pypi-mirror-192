...
void ParquetReader::InitializeSchema(const vector<string> &expected_names, const vector<LogicalType> &expected_types,
	...
	for (idx_t i = 0; i < column_ids.size(); i++) {
		...
		if (parquet_options.data_merge && column_ids[i] >= expected_names.size())
			continue;			
...
void ParquetReader::RearrangeChildReaders(unique_ptr<duckdb::ColumnReader> &root_reader, vector<column_t> &column_ids) {
	...
	for (idx_t col = 0; col < column_ids.size(); ++col) {
		if (IsRowIdColumnId(column_ids[col])){
...
bool ParquetReader::ScanInternal(ParquetReaderScanState &state, DataChunk &result) {
	...
	if (state.current_group < 0 || (int64_t)state.group_offset >= GetGroup(state).num_rows) {
		...
		for (idx_t out_col_idx = 0; out_col_idx < result.ColumnCount(); out_col_idx++) {
			...
			if (!column_reader)
				continue;
			to_scan_compressed_bytes += column_reader->TotalCompressedSize();
		}
		...
		if (state.prefetch_mode && state.group_offset != (idx_t)group.num_rows) {
			...
			if (!state.filters && scan_percentage > ParquetReaderPrefetchConfig::WHOLE_GROUP_PREFETCH_MINIMUM_SCAN) {
				...
				for (idx_t out_col_idx = 0; out_col_idx < result.ColumnCount(); out_col_idx++) {
					...
					column_reader->RegisterPrefetch(trans, !(lazy_fetch && !has_filter));
				}
	...
	if (state.filters) {
		...
		for (auto &filter_col : state.filters->filters) {
			...
			root_reader->GetChildReader(file_col_idx)->Read(result.size(), filter_mask, define_ptr, repeat_ptr, result.data[filter_col.first]);
		...
		for (idx_t out_col_idx = 0; out_col_idx < result.ColumnCount(); out_col_idx++) {
			...
			auto column_reader = root_reader->GetChildReader(file_col_idx);
			if (filter_mask.none()) {
				column_reader->Skip(result.size());
				continue;
			}
			// TODO handle ROWID here, too
			column_reader->Read(result.size(), filter_mask, define_ptr, repeat_ptr, result.data[out_col_idx]);
		}
		...
		for (idx_t out_col_idx = 0; out_col_idx < result.ColumnCount(); out_col_idx++) {
			...
			auto rows_read = root_reader->GetChildReader(file_col_idx)->Read(result.size(), filter_mask, define_ptr, repeat_ptr, result.data[out_col_idx]);
