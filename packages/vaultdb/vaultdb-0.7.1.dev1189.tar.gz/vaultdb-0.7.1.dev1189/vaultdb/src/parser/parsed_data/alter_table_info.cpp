...
unique_ptr<AlterInfo> AlterTableInfo::Deserialize(FieldReader &reader) {
	...
	case AlterTableType::ALTER_PARTITION_KEY:
		return AlterPartitionKeyInfo::Deserialize(reader, std::move(data));
...
unique_ptr<AlterInfo> AlterPartitionKeyInfo::Copy() const {
	...
	return make_unique_base<AlterInfo, AlterPartitionKeyInfo>(GetAlterEntryData(), copied_vect);
}

void AlterPartitionKeyInfo::SerializeAlterTable(FieldWriter &writer) const {
	writer.WriteList<string>(partition_key_columns);
}

unique_ptr<AlterInfo> AlterPartitionKeyInfo::Deserialize(FieldReader &reader, AlterEntryData data) {
	auto partition_keys = reader.ReadRequiredList<string>();
	return make_unique<AlterPartitionKeyInfo>(std::move(data), std::move(partition_keys));
}
//===--------------------------------------------------------------------===//
// AddColumnTagInfo
//===--------------------------------------------------------------------===//
...
unique_ptr<AlterInfo> AddColumnTagInfo::Copy() const {
	return make_unique_base<AlterInfo, AddColumnTagInfo>(GetAlterEntryData(), column_name, tag_name);
}

void AddColumnTagInfo::SerializeAlterTable(FieldWriter &writer) const {
	writer.WriteString(column_name);
	writer.WriteString(tag_name);
}

unique_ptr<AlterInfo> AddColumnTagInfo::Deserialize(FieldReader &reader, AlterEntryData data) {
	auto new_name = reader.ReadRequired<string>();
	auto new_tag_name = reader.ReadRequired<string>();
	return make_unique<AddColumnTagInfo>(std::move(data), new_name, new_tag_name);
}
...
unique_ptr<AlterInfo> RemoveColumnTagInfo::Copy() const {
	return make_unique_base<AlterInfo, RemoveColumnTagInfo>(GetAlterEntryData(), column_name, tag_name);
}

void RemoveColumnTagInfo::SerializeAlterTable(FieldWriter &writer) const {
	writer.WriteString(column_name);
	writer.WriteString(tag_name);
}

unique_ptr<AlterInfo> RemoveColumnTagInfo::Deserialize(FieldReader &reader, AlterEntryData data) {
	auto column_name = reader.ReadRequired<string>();
	auto tag_name = reader.ReadRequired<string>();
	return make_unique<RemoveColumnTagInfo>(std::move(data), column_name, tag_name);
}
