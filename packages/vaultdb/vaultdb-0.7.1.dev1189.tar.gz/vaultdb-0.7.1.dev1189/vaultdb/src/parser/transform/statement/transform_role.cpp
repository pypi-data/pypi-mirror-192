...
unique_ptr<CreateStatement> Transformer::TransformCreateRole(duckdb_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<duckdb_libpgquery::PGCreateRoleStmt *>(node);
	D_ASSERT(stmt);
	(void)stmt;
...
unique_ptr<AlterStatement> Transformer::TransformAlterRole(duckdb_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<duckdb_libpgquery::PGAlterRoleStmt *>(node);
	D_ASSERT(stmt);
	(void)stmt;
	...
	AlterEntryData data(INVALID_CATALOG, SECURITY_SCHEMA, stmt->role->relname, true);
	if (stmt->options & duckdb_libpgquery::PG_ROLEOPT_NOLOGIN) {
		auto info = make_unique<ModifyRoleFlagInfo>(AlterRoleType::LOGIN_CHANGE, std::move(data), false);
		result->info = std::move(info);
		return result;
	} else if (stmt->options & duckdb_libpgquery::PG_ROLEOPT_LOGIN) {
		auto info = make_unique<ModifyRoleFlagInfo>(AlterRoleType::LOGIN_CHANGE, std::move(data), true);
		result->info = std::move(info);
		return result;
	}

	if (stmt->options & duckdb_libpgquery::PG_ROLEOPT_SUPERUSER) {
		auto info = make_unique<ModifyRoleFlagInfo>(AlterRoleType::SUPERUSER_CHANGE, std::move(data), true);
		result->info = std::move(info);
		return result;
	} else if (stmt->options & duckdb_libpgquery::PG_ROLEOPT_NOSUPERUSER) {
		auto info = make_unique<ModifyRoleFlagInfo>(AlterRoleType::SUPERUSER_CHANGE, std::move(data), false);
		result->info = std::move(info);
		return result;
	}
