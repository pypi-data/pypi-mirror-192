...
unique_ptr<QueryNode> Transformer::TransformSelectNode(duckdb_libpgquery::PGSelectStmt *stmt) {
	...
	case duckdb_libpgquery::PG_SETOP_NONE: {
		...
		// group by
		TransformGroupBy(stmt->groupClause, *result); //Column Auth
		// having
		result->having = TransformExpression(stmt->havingClause); //Column Auth
		// qualify
		result->qualify = TransformExpression(stmt->qualifyClause); //Column Auth
		// sample
		result->sample = TransformSampleOptions(stmt->sampleOptions); //Column Auth
		break;
	}
