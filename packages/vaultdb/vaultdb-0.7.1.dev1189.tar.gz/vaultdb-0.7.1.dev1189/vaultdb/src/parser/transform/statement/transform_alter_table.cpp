...
unique_ptr<AlterStatement> Transformer::TransformAlter(duckdb_libpgquery::PGNode *node) {
	...
	for (auto c = stmt->cmds->head; c != nullptr; c = c->next) {
		...
		case duckdb_libpgquery::PG_AT_PartitionBy: {
			...
			result->info = make_unique<AlterPartitionKeyInfo>(std::move(data), partition_cols);
			break;
		}
		...
		case duckdb_libpgquery::PG_AT_AlterColumnAddTAG: {
			result->info = make_unique<AddColumnTagInfo>(std::move(data), command->name, command->tagname);
			break;
		}
		case duckdb_libpgquery::PG_AT_AlterColumnDeleteTAG: {
			result->info = make_unique<RemoveColumnTagInfo>(std::move(data), command->name, command->tagname);
			break;
		}
