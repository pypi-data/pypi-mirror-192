...
unique_ptr<AlterStatement> Transformer::TransformPrivilege(AlterRoleType type, duckdb_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<duckdb_libpgquery::PGGrantStmt *>(node);
	D_ASSERT(stmt);
	(void)stmt;
	...
	case duckdb_libpgquery::PG_OBJECT_TABLE:
		...
		// Authorize role
		if (context)
			context->authorizer->Authorize_table(DEFAULT_SCHEMA, stmt->resourcename->relname,  
	...
	case duckdb_libpgquery::PG_OBJECT_SCHEMA:
		...
		if (context)
			context->authorizer->Authorize_schema(stmt->resourcename->relname,  
					static_cast<duckdb_libpgquery::PGPrivilegeOption>(stmt->privilegeOptions), true);
		break;
	...
	case duckdb_libpgquery::PG_OBJECT_TAG:
		...
		// Authorize role
		if (context)
			context->authorizer->Authorize_schema(DEFAULT_SCHEMA,
					static_cast<duckdb_libpgquery::PGPrivilegeOption>(stmt->privilegeOptions), true);
	...
	auto info = make_unique<ModifyRolePrivilegeInfo>(type, std::move(data),
					catalogType, stmt->resourcename->relname, 
