...
Transformer::Transformer(ClientContext *context, idx_t max_expression_depth_p)
    : context(context), parent(nullptr), max_expression_depth(max_expression_depth_p), stack_depth(DConstants::INVALID_INDEX) {
}

Transformer::Transformer(ClientContext *context, Transformer *parent)
    : context(context), parent(parent), max_expression_depth(parent->max_expression_depth), stack_depth(DConstants::INVALID_INDEX) {
	if(parent)
...
unique_ptr<SQLStatement> Transformer::TransformStatementInternal(duckdb_libpgquery::PGNode *stmt) {
	// Just make sure that we do not do auth on wrong table
	...
	case duckdb_libpgquery::T_PGCreateRoleStmt:
		return TransformCreateRole(stmt);
	case duckdb_libpgquery::T_PGAlterRoleStmt:	
		return TransformAlterRole(stmt);
	case duckdb_libpgquery::T_PGCreateTagStmt:
		return TransformCreateTag(stmt);
	case duckdb_libpgquery::T_PGCreateFortressStmt:
		return TransformCreateFortress(stmt);
	case duckdb_libpgquery::T_PGAlterFortressStmt:
		return TransformAlterFortress(stmt);
	case duckdb_libpgquery::T_PGLockFortressStmt:
		return TransformLockFortress(stmt);
	case duckdb_libpgquery::T_PGUnLockFortressStmt:
		return TransformUnlockFortress(stmt);
	case duckdb_libpgquery::T_PGGrantStmt:
		return TransformPrivilege(AlterRoleType::GRANT_PRIVILEGES, stmt);
	case duckdb_libpgquery::T_PGRevokeStmt:	
		return TransformPrivilege(AlterRoleType::REVOKE_PRIVILEGES, stmt);
