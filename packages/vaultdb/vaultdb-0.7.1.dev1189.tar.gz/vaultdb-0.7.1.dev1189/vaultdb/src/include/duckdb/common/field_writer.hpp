...
public:
	...
	void WriteMap(const map<K, T> &elements) {
		AddField();
		Write<uint32_t>(elements.size());
		for (auto &element : elements) {
			Write<K>(element.first);
			Write<T>(element.second);
		}
	}
...
public:
	...
	map<K, T> ReadOptionalMap() {
		...
		auto result_count = source.Read<uint32_t>();
		for (idx_t i = 0; i < result_count; i++) {
			result[source.Read<K>()] = source.Read<T>();
		}
		return result;
	}
