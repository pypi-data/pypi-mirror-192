...
vector<column_t> LogicalGet::BuildMergeKeyColumnIds() {
...
void LogicalGet::Serialize(FieldWriter &writer) const {
	...
	//writer.WriteList<column_t>(merge_column_indexes); //!VaultDB: Add this and Parquet function serialization when ready to read data from remote vaultdb service
}
...
unique_ptr<LogicalOperator> LogicalGet::Deserialize(LogicalDeserializationState &state, FieldReader &reader) {
	...
	//auto merge_column_indexes = reader.ReadOptionalList<column_t>();//!VaultDB: Add this and Parquet function serialization when ready to read data from remote vaultdb service
	...
	//result->merge_column_indexes = std::move(merge_column_indexes);
