...
static void BindExtraColumns(TableCatalogEntry &table, LogicalGet &get, LogicalProjection &proj, LogicalUpdate &update,
	...
	if (found_column_count > 0 && found_column_count != bound_columns.size()) {
		...
		for (auto &check_column_id : bound_columns) {
			...
			auto col_idx_it = std::find(get.column_ids.begin(), get.column_ids.end(), check_column_id.index);
			if ( col_idx_it == get.column_ids.end()){
				...
				auto col_idx = col_idx_it - get.column_ids.begin();				
				proj.expressions.push_back(make_unique<BoundColumnRefExpression>(
					column.Type(), ColumnBinding(get.table_index, col_idx)));
			}
...
static void BindUpdateConstraints(TableCatalogEntry &table, LogicalGet &get, LogicalProjection &proj, LogicalUpdate &update) {
	...
	if (update.update_is_del_and_insert || update.update_is_remote_data_update) { // VAULTDB: Insert all instead of updating
		...
		// VAULTDB: Backup columns to update local data as is otherwise constraints do not work.
		if (!update.update_is_del_and_insert && update.update_is_remote_data_update){
			for (auto col_idx:update.columns){
				update.update_only_columns.push_back(col_idx);
			}
		}
		BindExtraColumns(table, get, proj, update, all_columns);
	}
...
BoundStatement Binder::Bind(UpdateStatement &stmt) {
	...
	// VAULTDB: Insert all instead of updating
	update->update_is_remote_data_update = context.IsMergeEnabled();
