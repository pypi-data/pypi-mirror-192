...
BoundStatement Binder::Bind(CreateStatement &stmt) {
	...
	case CatalogType::ROLE_ENTRY: {
		result.plan = make_unique<LogicalCreate>(LogicalOperatorType::LOGICAL_CREATE_ROLE, std::move(stmt.info));
		break;
	}
	case CatalogType::TAG_ENTRY: {
		result.plan = make_unique<LogicalCreate>(LogicalOperatorType::LOGICAL_CREATE_TAG, std::move(stmt.info));
		break;
	}
	case CatalogType::FORTRESS_ENTRY: {
		result.plan = make_unique<LogicalCreate>(LogicalOperatorType::LOGICAL_CREATE_FORTRESS, std::move(stmt.info));
		break;
	}
