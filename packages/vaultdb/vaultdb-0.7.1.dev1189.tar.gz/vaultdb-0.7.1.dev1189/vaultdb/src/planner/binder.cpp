...
void Binder::AddSchemaName(string schema_name) {
	if (parent) {
		parent->AddSchemaName(std::move(schema_name));
		return;
	}
	schema_names.insert(std::move(schema_name));
}

const unordered_set<string> &Binder::GetSchemaNames() {
	if (parent) {
		return parent->GetSchemaNames();
	}
	return schema_names;
}
