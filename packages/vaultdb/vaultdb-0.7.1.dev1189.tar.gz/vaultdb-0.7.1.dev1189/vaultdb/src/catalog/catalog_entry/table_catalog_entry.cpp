...
void TableCatalogEntry::Serialize(Serializer &serializer) {
	...
	writer.WriteList<string>(fortress);	
...
unique_ptr<CreateTableInfo> TableCatalogEntry::Deserialize(Deserializer &source, ClientContext &context) {
	...
	auto fortress = reader.ReadOptionalList<string>();
	for (auto &i: fortress) {
        info->fortress.insert(i);
    }
	auto partition_key_columns = reader.ReadOptionalList<string>();
	for (auto &i: partition_key_columns) {
        info->partition_key_columns.push_back(i);
    }
	reader.Finalize();
	return info;
}
