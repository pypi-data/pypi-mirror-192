...
DuckTableEntry::DuckTableEntry(Catalog *catalog, SchemaCatalogEntry *schema, BoundCreateTableInfo *info,
	...
	if (!storage) {
		...
		if (primary_keys.size()>0){
			...
			vector<unique_ptr<Expression>> unbound_expressions;
			unbound_expressions.push_back(
				make_unique<BoundColumnRefExpression>("Primary_Key_Hash", LogicalType::HASH, ColumnBinding(0, column_ids.size())));
			column_ids.push_back(0);
			storage->info->tombstone = make_unique<ART>(column_ids, TableIOManager::Get(*storage), std::move(unbound_expressions), 
								IndexConstraintType::NONE, storage->db, true);
		}
	}
}
...
unique_ptr<CatalogEntry> DuckTableEntry::AlterEntry(ClientContext &context, AlterInfo *info) {
	...
	case AlterTableType::ADD_TAG: {
		auto add_info = (AddColumnTagInfo *)table_info;
		return AddTag(context, *add_info);
	}
	case AlterTableType::REMOVE_TAG: {
		auto remove_info = (RemoveColumnTagInfo *)table_info;
