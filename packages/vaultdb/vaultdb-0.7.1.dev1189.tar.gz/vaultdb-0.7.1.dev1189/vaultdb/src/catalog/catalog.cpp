...
// Role
//===--------------------------------------------------------------------===//
CatalogEntry *Catalog::CreateRole(CatalogTransaction transaction, CreateRoleInfo *info) {
	auto schema = GetSchema(transaction, SECURITY_SCHEMA);
	return CreateRole(transaction, schema, info);
}

CatalogEntry *Catalog::CreateRole(CatalogTransaction transaction, SchemaCatalogEntry *schema, CreateRoleInfo *info) {
	return schema->CreateRole(transaction, info);
}

CatalogEntry *Catalog::CreateRole(ClientContext &context, CreateRoleInfo *info) {
...
// TAG
//===--------------------------------------------------------------------===//
CatalogEntry *Catalog::CreateTag(CatalogTransaction transaction, CreateTagInfo *info) {
	auto schema = GetSchema(transaction, SECURITY_SCHEMA);
	return CreateTag(transaction, schema, info);
}

CatalogEntry *Catalog::CreateTag(CatalogTransaction transaction, SchemaCatalogEntry *schema, CreateTagInfo *info) {
	return schema->CreateTag(transaction, info);
}

CatalogEntry *Catalog::CreateTag(ClientContext &context, CreateTagInfo *info) {
...
// Fortress
//===--------------------------------------------------------------------===//
CatalogEntry *Catalog::CreateFortress(CatalogTransaction transaction, CreateFortressInfo *info) {
	auto schema = GetSchema(transaction, info->schema);
	return CreateFortress(transaction, schema, info);
}

CatalogEntry *Catalog::CreateFortress(CatalogTransaction transaction, SchemaCatalogEntry *schema, CreateFortressInfo *info) {
	return schema->CreateFortress(transaction, info);
}

CatalogEntry *Catalog::CreateFortress(ClientContext &context, CreateFortressInfo *info) {
...
void Catalog::InitializeSecuritySchema() {
	...
	auto vaultdbrole_result = LookupEntryInternal(data, CatalogType::ROLE_ENTRY, SECURITY_SCHEMA, ADMIN_ROLE);
	...
	auto pitag_result = LookupEntryInternal(data, CatalogType::TAG_ENTRY, SECURITY_SCHEMA, PI_TAG);
	if (!pitag_result.Found()) {
		...
		personal_info_tag.function = make_unique<ConstantExpression>(Value(99999999));
		personal_info_tag.on_conflict = OnCreateConflict::IGNORE_ON_CONFLICT;
		CreateTag(data, &personal_info_tag);
	}

	auto piitag_result = LookupEntryInternal(data, CatalogType::TAG_ENTRY, SECURITY_SCHEMA, PII_TAG);
	if (!piitag_result.Found()) {
		...
		personal_indetifiable_info_tag.function = make_unique<ConstantExpression>(Value(99999999));
