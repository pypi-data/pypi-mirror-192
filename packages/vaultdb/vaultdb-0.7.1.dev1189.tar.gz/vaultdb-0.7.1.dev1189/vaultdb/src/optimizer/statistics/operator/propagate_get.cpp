...
unique_ptr<NodeStatistics> StatisticsPropagator::PropagateStatistics(LogicalGet &get,
	...
	for (idx_t i = 0; i < get.column_ids.size(); i++) {
		...
		if (get.parquet_bind_data) {
			auto parquet_stats = get.parquet_function.statistics(context, get.parquet_bind_data.get(), get.column_ids[i]);
			if (stats && parquet_stats)
				stats->Merge(*parquet_stats);
		}
		if (stats) {
			ColumnBinding binding(get.table_index, i);
			auto entry = statistics_map.find(binding); //# We need to do something to keep remote and local stats separate
