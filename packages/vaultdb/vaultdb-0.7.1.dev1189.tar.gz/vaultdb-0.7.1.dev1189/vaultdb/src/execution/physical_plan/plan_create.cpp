...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalCreate &op) {
	...
	case LogicalOperatorType::LOGICAL_CREATE_ROLE:
		return make_unique<PhysicalCreateRole>(unique_ptr_cast<CreateInfo, CreateRoleInfo>(std::move(op.info)),
		                                           op.estimated_cardinality);
	case LogicalOperatorType::LOGICAL_CREATE_TAG:
		return make_unique<PhysicalCreateTag>(unique_ptr_cast<CreateInfo, CreateTagInfo>(std::move(op.info)),
		                                           op.estimated_cardinality);
	case LogicalOperatorType::LOGICAL_CREATE_FORTRESS:
		return make_unique<PhysicalCreateFortress>(unique_ptr_cast<CreateInfo, CreateFortressInfo>(std::move(op.info)),
		                                           op.estimated_cardinality);
