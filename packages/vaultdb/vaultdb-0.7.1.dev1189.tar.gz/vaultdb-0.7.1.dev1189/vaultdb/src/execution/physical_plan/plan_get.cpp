...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePhysicalTablePlan(LogicalGet &op) {
...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalGet &op) {
	...
	if (context.IsMergeEnabled() && table) {				
		auto left = CreatePhysicalTablePlan(op);
		...
		auto right = make_unique<PhysicalTableScan>(op.types, op.parquet_function, std::move(op.parquet_bind_data), op.returned_types,
		                                      op.column_ids, op.projection_ids, op.names, std::move(table_filters),
		                                      op.estimated_cardinality);
		D_ASSERT(left && right);
			
		vector<column_t> merge_key_column_ids = op.BuildMergeKeyColumnIds();
		return make_unique<PhysicalMerge>(op.types, table->GetStorage(), std::move(left), std::move(right), std::move(merge_key_column_ids), op.estimated_cardinality);
	} else
		return CreatePhysicalTablePlan(op);
}
} // namespace duckdb
