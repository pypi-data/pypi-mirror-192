#include "duckdb/execution/operator/join/physical_merge_hash.hpp"
...
unique_ptr<GlobalSinkState> PhysicalMerge::GetGlobalSinkState(ClientContext &context) const {
	...
	auto state = make_unique<MergeGlobalState>(context, table);
	return std::move(state);
}

unique_ptr<LocalSinkState> PhysicalMerge::GetLocalSinkState(ExecutionContext &context) const {
	auto result = make_unique<MergeLocalState>();
	result->build_chunk.Initialize(Allocator::Get(context.client), types);
	return std::move(result);
}

SinkResultType PhysicalMerge::Sink(ExecutionContext &context, GlobalSinkState &gstate_p, LocalSinkState &lstate_p,
                                   DataChunk &input) const {
	auto &lstate = (MergeLocalState &)lstate_p;
	
	input.Verify();
	lstate.build_chunk.Reset();
	lstate.build_chunk.SetCardinality(input);
	lstate.build_chunk.source = input.source;

	if (input.size()> 0) {
		for (idx_t i = 0; i < types.size(); i++) {
			lstate.build_chunk.data[i].Reference(input.data[i]);
		}
...
void PhysicalMerge::Combine(ExecutionContext &context, GlobalSinkState &gstate_p, LocalSinkState &lstate_p) const {
	auto &gstate = (MergeGlobalState &)gstate_p;
	auto &lstate = (MergeLocalState &)lstate_p;
...
SinkFinalizeType PhysicalMerge::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
...
// Operator
//===--------------------------------------------------------------------===//
class PhysicalMergeState : public OperatorState {
public:
	explicit PhysicalMergeState() {
	}
};

unique_ptr<OperatorState> PhysicalMerge::GetOperatorState(ExecutionContext &context) const {
	auto state = make_unique<PhysicalMergeState>();
	return std::move(state);
}

OperatorResultType PhysicalMerge::Execute(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
                                             GlobalOperatorState &gstate_p, OperatorState &state_p) const {
	//auto &state = (PhysicalMergeState &)state_p;
...
public:
	explicit HashMergeScanState(const PhysicalMerge &op) : op(op) {
	}
	const PhysicalMerge &op;
};

unique_ptr<GlobalSourceState> PhysicalMerge::GetGlobalSourceState(ClientContext &context) const {
	return make_unique<HashMergeScanState>(*this);
}

void PhysicalMerge::GetData(ExecutionContext &context, DataChunk &chunk, GlobalSourceState &gstate_p,
...
vector<const PhysicalOperator *> PhysicalMerge::GetSources() const {
	return {this};
}
