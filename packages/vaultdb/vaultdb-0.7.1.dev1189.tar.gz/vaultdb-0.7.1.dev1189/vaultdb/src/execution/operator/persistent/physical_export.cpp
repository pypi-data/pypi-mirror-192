...
void PhysicalExport::GetData(ExecutionContext &context, DataChunk &chunk, GlobalSourceState &gstate,
	...
	vector<CatalogEntry *> roles;
	vector<CatalogEntry *> tags;
	vector<CatalogEntry *> fortresses;
	...
	for (auto &schema : schema_list) {
		...
		schema->Scan(context.client, CatalogType::ROLE_ENTRY, [&](CatalogEntry *entry) {
			if (!entry->internal)
				roles.push_back(entry);
		});
		schema->Scan(context.client, CatalogType::TAG_ENTRY, [&](CatalogEntry *entry) {
			if (!entry->internal)
				tags.push_back(entry);
		});
		schema->Scan(context.client, CatalogType::FORTRESS_ENTRY,
		             [&](CatalogEntry *entry) { fortresses.push_back(entry); });
	...
	WriteCatalogEntries(ss, roles);
	WriteCatalogEntries(ss, tags);
	WriteCatalogEntries(ss, fortresses);
