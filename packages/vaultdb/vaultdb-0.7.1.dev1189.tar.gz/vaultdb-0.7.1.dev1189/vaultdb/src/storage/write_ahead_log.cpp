...
// ROLE
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateRole(RoleCatalogEntry *entry) {
	if (skip_writing) {
		return;
	}
	writer->Write<WALType>(WALType::CREATE_ROLE);
	entry->Serialize(*writer);
}

void WriteAheadLog::WriteDropRole(RoleCatalogEntry *entry) {
	if (skip_writing) {
		return;
	}
	writer->Write<WALType>(WALType::DROP_ROLE);
	writer->WriteString(entry->schema->name);
	writer->WriteString(entry->name);
}
...
// TAG
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateTag(TagCatalogEntry *entry) {
	if (skip_writing) {
		return;
	}
	writer->Write<WALType>(WALType::CREATE_TAG);
	entry->Serialize(*writer);
}

void WriteAheadLog::WriteDropTag(TagCatalogEntry *entry) {
	if (skip_writing) {
		return;
	}
	writer->Write<WALType>(WALType::DROP_TAG);
	writer->WriteString(entry->schema->name);
	writer->WriteString(entry->name);
}
...
// FORTRESS
//===--------------------------------------------------------------------===//
void WriteAheadLog::WriteCreateFortress(FortressCatalogEntry *entry) {
	if (skip_writing) {
		return;
	}
	writer->Write<WALType>(WALType::CREATE_FORTRESS);
	entry->Serialize(*writer);
}

void WriteAheadLog::WriteDropFortress(FortressCatalogEntry *entry) {
	if (skip_writing) {
		return;
	}
	writer->Write<WALType>(WALType::DROP_FORTRESS);
	writer->WriteString(entry->schema->name);
	writer->WriteString(entry->name);
}
