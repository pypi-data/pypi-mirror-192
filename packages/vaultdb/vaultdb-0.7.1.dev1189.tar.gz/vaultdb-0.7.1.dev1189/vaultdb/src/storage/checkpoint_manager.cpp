...
void CheckpointWriter::WriteSchema(SchemaCatalogEntry &schema) {
	...
	vector<RoleCatalogEntry *> roles;
	schema.Scan(CatalogType::ROLE_ENTRY, [&](CatalogEntry *entry) {
		if (entry->internal) {
			return;
		}
		roles.push_back((RoleCatalogEntry *)entry);
	});

	vector<TagCatalogEntry *> tags;
	schema.Scan(CatalogType::TAG_ENTRY, [&](CatalogEntry *entry) {
		if (entry->internal) {
			return;
		}
		tags.push_back((TagCatalogEntry *)entry);
	});

	vector<FortressCatalogEntry *> fortresses;
	schema.Scan(CatalogType::FORTRESS_ENTRY, [&](CatalogEntry *entry) {
		if (entry->internal) {
			return;
		}
		fortresses.push_back((FortressCatalogEntry *)entry);
	});
	...
	if (schema.name==SECURITY_SCHEMA){
		writer.WriteField<uint32_t>(roles.size());
		writer.WriteField<uint32_t>(tags.size());
		writer.WriteField<uint32_t>(fortresses.size());
	}
	writer.Finalize();
	
	if (schema.name==SECURITY_SCHEMA)
	{
		//Roles must be written ahead of Time
		for (auto &role : roles) {
			if (role->internal)
				continue;
			WriteRole(*role);
	...
	// write the auth catalog entries
	if (schema.name==SECURITY_SCHEMA)
	{
		for (auto &tag : tags) {
			if (tag->internal)
		...
		for (auto &fortress : fortresses) {
			if (fortress->internal)
				continue;;
			WriteFortress(*fortress);
		}
	}
}
...
void CheckpointReader::ReadSchema(ClientContext &context, MetaBlockReader &reader) {
	...
	uint32_t fortress_count = 0;
	if (info->schema==SECURITY_SCHEMA){
		role_count = field_reader.ReadRequired<uint32_t>();
		tag_count = field_reader.ReadRequired<uint32_t>();
		fortress_count = field_reader.ReadRequired<uint32_t>();
	}
	field_reader.Finalize();

	if (info->schema==SECURITY_SCHEMA)
	{
		//Roles must be read ahead of Time
		//TODO: Store Catalog Entries in separate file/Redis and read them when required
		for (uint32_t i = 0; i < role_count; i++) {
			ReadRole(context, reader);
		}
	}
	...
	if (info->schema==SECURITY_SCHEMA)
	{
		for (uint32_t i = 0; i < tag_count; i++) {
			ReadTag(context, reader);
		}

		for (uint32_t i = 0; i < fortress_count; i++) {
			ReadFortress(context, reader);
		}
	}
}
...
// Roles
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteRole(RoleCatalogEntry &seq) {
	seq.Serialize(GetMetaBlockWriter());
}

void CheckpointReader::ReadRole(ClientContext &context, MetaBlockReader &reader) {
	auto info = RoleCatalogEntry::Deserialize(reader);
	catalog.CreateRole(context, info.get());
}

//===--------------------------------------------------------------------===//
// Tags
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteTag(TagCatalogEntry &seq) {
	seq.Serialize(GetMetaBlockWriter());
}

void CheckpointReader::ReadTag(ClientContext &context, MetaBlockReader &reader) {
	auto info = TagCatalogEntry::Deserialize(reader);
	catalog.CreateTag(context, info.get());
}

//===--------------------------------------------------------------------===//
// Fortress
//===--------------------------------------------------------------------===//
void CheckpointWriter::WriteFortress(FortressCatalogEntry &fort) {
	fort.Serialize(GetMetaBlockWriter());
}

void CheckpointReader::ReadFortress(ClientContext &context, MetaBlockReader &reader) {
	auto info = FortressCatalogEntry::Deserialize(reader);
	catalog.CreateFortress(context, info.get());
}
