...
ClientContext::ClientContext(shared_ptr<DatabaseInstance> database, string role)
    : db(std::move(database)), interrupted(false), client_data(make_unique<ClientData>(*this)), transaction(*this),
	  authorizer(make_unique<Authorizer>(*this, role)) {
}
...
bool ClientContext::IsMergeEnabled() {
	...
	return config.enable_data_inheritance && !db_config.options.data_inheritance_path.empty();
}
...
unordered_set<string> ClientContext::GetSchemaNames(const string &query) {
	auto lock = LockContext();
