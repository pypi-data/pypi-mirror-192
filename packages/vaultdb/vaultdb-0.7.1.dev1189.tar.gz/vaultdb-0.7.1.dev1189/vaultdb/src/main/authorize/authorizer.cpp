...
Authorizer::Authorizer(ClientContext &context_p, string role)
    : context(context_p), role(role) {
}
...
void Authorizer::Role(const string &changed_role) {
	...
	for (auto table:constrained_tables) {
		...
		auto table_entry = Catalog::GetEntry<TableCatalogEntry>(context, INVALID_CATALOG, schema_name, table_name, true);
	...
	if (changed_role.empty()) {
		...
		if (active_role==INTERNAL_ROLE) {
			current_role = make_unique<CreateRoleInfo>();
			current_role->superuser = true;
			current_role->login = false;
			current_role->schema = SECURITY_SCHEMA;
			current_role->name = INTERNAL_ROLE;
		} else{
			auto role_entry = Catalog::GetEntry<RoleCatalogEntry>(context, INVALID_CATALOG, SECURITY_SCHEMA, active_role, true);
			if (role_entry) {
...
unique_ptr<ParsedExpression> Authorizer::AddFortressExpression(TableRef &tableref, unique_ptr<ParsedExpression> where_clause, duckdb_libpgquery::PGPrivilegeOption privilege_option) {
	...
	case TableReferenceType::BASE_TABLE: {
		...
		auto table_entry = Catalog::GetEntry<TableCatalogEntry>(context, INVALID_CATALOG, basetable.schema_name, basetable.table_name, true);
		...
		for (auto privilege : table_entry->fortress) {
			auto fortress_entry = Catalog::GetEntry<FortressCatalogEntry>(context, INVALID_CATALOG, SECURITY_SCHEMA, privilege, true);
			...
			if (it != GetCurrentRole()->privileges.end() && it->second.size() > 0) {
				...
				if (priv != privileges.end() && priv->second->privileges >= 1 && priv->second->privileges & privilege_option)
					continue; // No Condition is required, If user is allowed
			}
			exp->alias = alias;
			if (add_or) {
				result = make_unique<ConjunctionExpression>(conjuctiontype, std::move(result), std::move(exp));
				result->alias = alias;
			} else
				result = std::move(exp);
			add_or = true;
		...
		if (where_clause){
			if (result)
				return make_unique<ConjunctionExpression>(ExpressionType::CONJUNCTION_AND, std::move(result), std::move(where_clause));
			else
				return std::move(where_clause);
		}
		else
...
unique_ptr<ParsedExpression> Authorizer::GetFortressExpression(string schema_name, string table_name, duckdb_libpgquery::PGPrivilegeOption privilege_option) {
	unique_ptr<ParsedExpression> result;
...
void Authorizer::BuildFortressConstraints(string schema_name, string table_name, duckdb_libpgquery::PGPrivilegeOption privilege_option) {	
...
void Authorizer::Authorize_Columns(TableRef &tableref, vector<string> select_list) {
	...
	case TableReferenceType::BASE_TABLE: {
		...
		auto unauthorized_columns = GetUnauthorizedColumns(basetable);
		auto table_entry =
		    Catalog::GetEntry<TableCatalogEntry>(context, INVALID_CATALOG, basetable.schema_name, basetable.table_name, true);
		if (new_transaction)
			context.transaction.Commit();
