...
const vector<string> ExtensionHelper::PathComponents() {
	return vector<string> {".vaultdb", "extensions", GetVersionDirectoryName(), DuckDB::Platform()};
}
...
void ExtensionHelper::InstallExtension(ClientContext &context, const string &extension, bool force_install) {
	...
	string default_endpoint = "http://vaultdb-hosted-content.s3-website.us-east-2.amazonaws.com/extensions";
