...
public:
	...
	bool IsPartitionKey(idx_t col) override {
		auto result = std::find(entry->partition_key_columns.begin(),
						entry->partition_key_columns.end(),
...
unique_ptr<ColumnHelper> ColumnHelper::Create(CatalogEntry *entry) {
	...
	default:
		throw NotImplementedException("Unsupported catalog type for vaultdb_columns");
	}
}
...
void DuckDBColumnsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("vaultdb_columns", {}, DuckDBColumnsFunction, DuckDBColumnsBind, DuckDBColumnsInit));
}
