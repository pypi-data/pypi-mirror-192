...
void DuckDBFunctionsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	while (data.offset < data.entries.size() && count < STANDARD_VECTOR_SIZE) {
		...
		case CatalogType::PRAGMA_FUNCTION_ENTRY:
			...
			break;
		default:
			throw InternalException("FIXME: unrecognized function type in vaultdb_functions");
		}
		if (finished) {
...
void DuckDBFunctionsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(
	    TableFunction("vaultdb_functions", {}, DuckDBFunctionsFunction, DuckDBFunctionsBind, DuckDBFunctionsInit));
}
