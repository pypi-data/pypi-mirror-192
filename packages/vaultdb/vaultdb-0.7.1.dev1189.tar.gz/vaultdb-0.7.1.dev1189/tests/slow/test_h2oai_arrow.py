...
def group_by_q1(con):
    ...
    res = con.execute(
        "SELECT COUNT(*), sum(v1)::varchar AS v1 FROM ans").fetchall()
...
def group_by_q3(con):
    ...
    res = con.execute(
        "SELECT COUNT(*), sum(v1) AS v1, sum(v3) AS v3 FROM ans;").fetchall()
...
def group_by_q4(con):
    ...
    res = con.execute(
        "SELECT COUNT(*), sum(v1) AS v1, sum(v2) AS v2, sum(v3) AS v3 FROM ans").fetchall()
    assert res[0][0] == 96
...
def group_by_q6(con):
    ...
    res = con.execute(
        "SELECT COUNT(*), sum(median_v3) AS median_v3, sum(sd_v3) AS sd_v3 FROM ans").fetchall()
    assert res[0][0] == 9216
...
def group_by_q7(con):
    ...
    res = con.execute(
        "SELECT count(*), sum(range_v1_v2) AS range_v1_v2 FROM ans;").fetchall()
    assert res[0][0] == 95001
...
def group_by_q8(con):
    ...
    res = con.execute(
        "SELECT count(*), sum(largest2_v3) AS largest2_v3 FROM ans").fetchall()
...
def group_by_q10(con):
    ...
    res = con.execute(
        "SELECT sum(v3) AS v3, sum(count) AS count FROM ans;").fetchall()
...
def join_by_q1(con):
    ...
    res = con.execute(
        "SELECT COUNT(*), SUM(v1) AS v1, SUM(v2) AS v2 FROM ans;").fetchall()
...
class TestH2OAIArrow(object):
    @mark.parametrize('function', [
        ...
        group_by_q6,
        group_by_q7,
        # VAULTDB: Failing here will take time to figure out.
        # group_by_q8,
        # group_by_q9,
        # group_by_q10,
    ])
    @mark.parametrize('threads', [1, 4])
    @mark.usefixtures('groupby_data')
    def test_group_by(self, threads, function, groupby_data):
        groupby_data.execute(f"PRAGMA threads={threads}")

        function(groupby_data)
