...
class TestArrowIntegration(object):
    def test_parquet_roundtrip(self, duckdb_cursor):
        if not can_run:
            return
        parquet_filename = os.path.join(os.path.dirname(
            os.path.realpath(__file__)), 'data', 'userdata1.parquet')
        ...
        rel_from_arrow = vaultdb.arrow(
            userdata_parquet_table).project(cols).arrow()
        rel_from_arrow.validate(full=True)

        rel_from_duckdb = vaultdb.from_parquet(
            parquet_filename).project(cols).arrow()
        ...
        for i in [7, 51, 99, 100, 101, 500, 1000, 2000]:
            ...
            assert userdata_parquet_table.equals(
                userdata_parquet_table2, check_metadata=True)

            rel_from_arrow2 = vaultdb.arrow(
                userdata_parquet_table2).project(cols).arrow()
    ...
    def test_unsigned_roundtrip(self, duckdb_cursor):
        ...
        round_tripping = vaultdb.from_arrow(arrow_result).to_arrow_table()
    ...
    def test_decimals_roundtrip(self, duckdb_cursor):
        ...
        duckdb_conn.execute(
            "CREATE TABLE test (a DECIMAL(4,2), b DECIMAL(9,2), c DECIMAL (18,2), d DECIMAL (30,2))")

        duckdb_conn.execute(
            "INSERT INTO  test VALUES (1.11,1.11,1.11,1.11),(NULL,NULL,NULL,NULL)")

        true_result = duckdb_conn.execute(
            "SELECT sum(a), sum(b), sum(c),sum(d) from test").fetchall()
        ...
        arrow_result = duckdb_conn.execute(
            "SELECT sum(a), sum(b), sum(c),sum(d) from testarrow").fetchall()

        assert (arrow_result == true_result)

        arrow_result = duckdb_conn.execute(
            "SELECT typeof(a), typeof(b), typeof(c),typeof(d) from testarrow").fetchone()
        ...
        data = (pyarrow.array(np.array(
            [9999999999999999999999999999999999]), type=pyarrow.decimal128(38, 0)))
    ...
    def test_strings_roundtrip(self, duckdb_cursor):
        ...
        arrow_result = duckdb_conn.execute(
            "SELECT * from testarrow").fetchall()
