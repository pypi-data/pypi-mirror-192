...
class TestArrowUnregister(object):
    def test_arrow_unregister1(self, duckdb_cursor):
        ...
        connection = vaultdb.connect(":memory:")
        ...
        with pytest.raises(vaultdb.CatalogException, match='Table with name arrow_table does not exist'):
            connection.execute("SELECT * FROM arrow_table;").fetch_arrow_table()
        with pytest.raises(vaultdb.CatalogException, match='View with name arrow_table does not exist'):
