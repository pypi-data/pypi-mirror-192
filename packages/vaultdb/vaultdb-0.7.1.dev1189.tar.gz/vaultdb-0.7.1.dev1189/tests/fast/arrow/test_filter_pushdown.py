...
## VaultDB connection used in this test
...
class TestArrowFilterPushdown(object):
    ...
    def test_filter_pushdown_2145(self,duckdb_cursor):
        ...
        output_df = vaultdb.arrow(table).filter("date > '2019-01-01'").df()
        expected_df = vaultdb.from_parquet("data*.parquet").filter("date > '2019-01-01'").df()
