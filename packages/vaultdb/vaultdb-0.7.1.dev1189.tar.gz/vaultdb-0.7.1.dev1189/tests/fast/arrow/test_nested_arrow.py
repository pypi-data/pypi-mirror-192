...
def compare_results(query):
    true_answer = vaultdb.query(query).fetchall()
    from_arrow = vaultdb.from_arrow(vaultdb.query(query).arrow()).fetchall()
    assert true_answer == from_arrow


def arrow_to_pandas(query):
    return vaultdb.query(query).arrow().to_pandas()['a'].values.tolist()
...
class TestArrowNested(object):
    def test_lists_basic(self, duckdb_cursor):
        ...
        # Test Constant List
        query = vaultdb.query(
            "SELECT a from (select list_value(3,5,10) as a) as t").arrow()['a'].to_numpy()
        assert query[0][0] == 3
        assert query[0][1] == 5
        assert query[0][2] == 10

        # Empty List
        query = vaultdb.query("SELECT a from (select list_value() as a) as t").arrow()[
            'a'].to_numpy()
        assert len(query[0]) == 0

        # Test Constant List With Null
        query = vaultdb.query(
            "SELECT a from (select list_value(3,NULL) as a) as t").arrow()['a'].to_numpy()
        assert query[0][0] == 3
    ...
    def test_list_types(self, duckdb_cursor):
        ...
        # Large Lists
        data = pyarrow.array(
            [[1], None, [2]], type=pyarrow.large_list(pyarrow.int64()))
        arrow_table = pa.Table.from_arrays([data], ['a'])
        rel = vaultdb.from_arrow(arrow_table)
        res = rel.execute().fetchall()
        assert res == [([1],), (None,), ([2],)]

        # Fixed Size Lists
        data = pyarrow.array(
            [[1], None, [2]], type=pyarrow.list_(pyarrow.int64(), 1))
        arrow_table = pa.Table.from_arrays([data], ['a'])
        rel = vaultdb.from_arrow(arrow_table)
        res = rel.execute().fetchall()
        assert res == [([1],), (None,), ([2],)]
        ...
        data = [pyarrow.array([[1], None, [2]], type=pyarrow.list_(pyarrow.int64(), 1)), pyarrow.array([[1], None, [
            2]], type=pyarrow.large_list(pyarrow.int64())), pyarrow.array([[1, 2, 3], None, [2, 1]], type=pyarrow.list_(pyarrow.int64()))]
        ...
        struct = [pa.StructArray.from_arrays(
            data, ['fixed', 'large', 'normal'])]
        arrow_table = pa.Table.from_arrays(struct, ['a'])
        rel = vaultdb.from_arrow(arrow_table)
        res = rel.execute().fetchall()
        assert res == [({'fixed': [1], 'large': [1], 'normal': [1, 2, 3]},), ({
            'fixed': None, 'large': None, 'normal': None},), ({'fixed': [2], 'large': [2], 'normal': [2, 1]},)]
    ...
    def test_lists_roundtrip(self, duckdb_cursor):
        ...
        compare_results(
            "SELECT a from (select list_value(3,5,NULL) as a) as t")
        compare_results(
            "SELECT a from (select list_value(NULL,NULL,NULL) as a) as t")
        ...
        # Multiple Lists
        compare_results(
            "SELECT a from (SELECT LIST(i) as a FROM range(10000) tbl(i) group by i%10) as t")
        # Unique Constants
        compare_results(
            "SELECT a from (SELECT list_value(1) as a FROM range(10) tbl(i)) as t")
        # Nested Lists
        compare_results(
            "SELECT LIST(le) FROM (SELECT LIST(i) le from range(100) tbl(i) group by i%10) as t")
    ...
    def test_map_arrow_to_duckdb(self, duckdb_cursor):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match="Arrow map contains duplicate key, which isn't supported by VaultDB map type"):
            rel = vaultdb.from_arrow(arrow_table).fetchall()

    def test_map_arrow_to_pandas(self, duckdb_cursor):
        if not can_run:
            return
        assert arrow_to_pandas("SELECT a from (select MAP(LIST_VALUE(1, 2, 3, 4),LIST_VALUE(10, 9, 8, 7)) as a) as t") == [
            [(1, 10), (2, 9), (3, 8), (4, 7)]]
        assert arrow_to_pandas(
            "SELECT a from (select MAP(LIST_VALUE(),LIST_VALUE()) as a) as t") == [[]]
        assert arrow_to_pandas("SELECT a from (select MAP(LIST_VALUE('Jon Lajoie', 'Backstreet Boys', 'Tenacious D'),LIST_VALUE(10,9,10)) as a) as t") == [
            [('Jon Lajoie', 10), ('Backstreet Boys', 9), ('Tenacious D', 10)]]
        assert arrow_to_pandas("SELECT a from (select MAP(list_value(1), list_value(2)) from range(5) tbl(i)) tbl(a)") == [
            [(1, 2)], [(1, 2)], [(1, 2)], [(1, 2)], [(1, 2)]]
        assert arrow_to_pandas("SELECT MAP(LIST_VALUE({'i':1,'j':2},{'i':3,'j':4}),LIST_VALUE({'i':1,'j':2},{'i':3,'j':4})) as a") == [
