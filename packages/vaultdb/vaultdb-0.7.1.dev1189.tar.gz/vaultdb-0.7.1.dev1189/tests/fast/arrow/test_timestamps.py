...
class TestArrowTimestamps(object):
    def test_timestamp_types(self, duckdb_cursor):
        ...
        rel = vaultdb.from_arrow(arrow_table).arrow()
    ...
    def test_timestamp_overflow(self, duckdb_cursor):
        ...
        arrow_from_duck = vaultdb.from_arrow(arrow_table).arrow()
        ...
        with pytest.raises(vaultdb.ConversionException, match='Could not convert'):
            duck_rel = vaultdb.from_arrow(arrow_table)
