...
class TestArrowRecordBatchReader(object):
    ...
    def test_parallel_reader(self, duckdb_cursor):
        ...
        assert rel.filter("first_name=\'Jose\' and salary > 134708.82").aggregate(
            'count(*)').execute().fetchone()[0] == 12
        # The reader is already consumed so this should be 0
        assert rel.filter("first_name=\'Jose\' and salary > 134708.82").aggregate(
            'count(*)').execute().fetchone()[0] == 0
    ...
    def test_parallel_reader_replacement_scans(self, duckdb_cursor):
        ...
        assert duckdb_conn.execute(
            "select count(*) from reader where first_name=\'Jose\' and salary > 134708.82").fetchone()[0] == 12
        assert duckdb_conn.execute(
            "select count(*) from reader where first_name=\'Jose\' and salary > 134708.82").fetchone()[0] == 0
    ...
    def test_parallel_reader_register(self, duckdb_cursor):
        ...
        assert duckdb_conn.execute(
            "select count(*) from bla where first_name=\'Jose\' and salary > 134708.82").fetchone()[0] == 12
        assert duckdb_conn.execute(
            "select count(*) from bla where first_name=\'Jose\' and salary > 134708.82").fetchone()[0] == 0
