...
class TestNonDefaultConn(object):
    ...
    def test_values(self, duckdb_cursor):
        ...
        vaultdb.values([1],conn).insert_into("t")
    ...
    def test_query(self, duckdb_cursor):
        ...
        assert vaultdb.query("select count(*) from t",connection=conn).execute().fetchall()[0] ==  (1,)
        assert vaultdb.from_query("select count(*) from t",connection=conn).execute().fetchall()[0] ==  (1,)
    ...
    def test_from_csv(self, duckdb_cursor):
        ...
        rel = vaultdb.from_csv_auto(temp_file_name, conn)
    ...
    def test_from_parquet(self, duckdb_cursor):
        ...
        rel = vaultdb.from_parquet(temp_file_name, connection=conn)
        assert rel.query('t_2','select count(*) from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,)
    ...
    def test_from_df(self, duckdb_cursor):
        ...
        conn.execute("insert into t values (1)")
        test_df = pd.DataFrame.from_dict({"i":[1, 2, 3, 4]})
        rel = vaultdb.df(test_df, connection=conn)
        assert rel.query('t_2','select count(*) from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,)
        rel = vaultdb.from_df(test_df, connection=conn)
        assert rel.query('t_2','select count(*) from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,)
    ...
    def test_from_arrow(self, duckdb_cursor):
        ...
        rel = vaultdb.from_arrow(test_arrow, connection=conn)
        assert rel.query('t_2','select count(*) from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,)
        rel = vaultdb.arrow(test_arrow, connection=conn)
        assert rel.query('t_2','select count(*) from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,)
    ...
    def test_filter_df(self, duckdb_cursor):
        ...
        test_df = pd.DataFrame.from_dict({"i":[1, 2, 3, 4]})
        rel = vaultdb.filter(test_df,"i < 2", connection=conn)
        assert rel.query('t_2','select count(*) from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,)
    ...
    def test_project_df(self, duckdb_cursor):
        ...
        rel = vaultdb.project(test_df,"i", connection=conn)
    ...
    def test_agg_df(self, duckdb_cursor):
        ...
        conn.execute("insert into t values (1), (4)")
        test_df = pd.DataFrame.from_dict({"i":[1, 2, 3, 4],"j":[1, 2, 3, 4]})
        rel = vaultdb.aggregate(test_df,"count(*) as i", connection=conn)
    ...
    def test_distinct_df(self, duckdb_cursor):
        ...
        rel = vaultdb.distinct(test_df, connection=conn)
        assert rel.query('t_2','select * from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,1) 
    ...
    def test_limit_df(self, duckdb_cursor):
        ...
        conn.execute("insert into t values (1),(4)")
        test_df = pd.DataFrame.from_dict({"i":[1, 2, 3, 4]})
        rel = vaultdb.limit(test_df,1, connection=conn)
        assert rel.query('t_2','select * from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,1) 
    ...
    def test_query_df(self, duckdb_cursor):
        ...
        conn.execute("insert into t values (1),(4)")
        test_df = pd.DataFrame.from_dict({"i":[1, 2, 3, 4]})
        rel = vaultdb.query_df(test_df,'t_2','select * from t inner join t_2 on (a = i)', connection=conn)
    ...
    def test_query_order(self, duckdb_cursor):
        ...
        conn.execute("insert into t values (1),(4)")
        test_df = pd.DataFrame.from_dict({"i":[1, 2, 3, 4]})
        rel = vaultdb.order(test_df,'i', connection=conn)
        assert rel.query('t_2','select * from t inner join t_2 on (a = i)').fetchall()[0] ==  (1,1) 
