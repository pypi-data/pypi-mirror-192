...
class TestDuckMultithread(object):
    ...
    def test_execute(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,execute_query)
        duck_threads.multithread_test()

    def test_execute_many(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,execute_many_query)
        duck_threads.multithread_test()

    def test_fetchone(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,fetchone_query)
        duck_threads.multithread_test()

    def test_fetchall(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,fetchall_query)
        duck_threads.multithread_test()

    def test_close(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,conn_close)
        duck_threads.multithread_test()

    def test_fetchnp(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,fetchnp_query)
        duck_threads.multithread_test()

    def test_fetchdf(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,fetchdf_query)
        duck_threads.multithread_test()

    def test_fetchdfchunk(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,fetchdf_chunk_query)
        duck_threads.multithread_test()

    def test_fetcharrow(self, duckdb_cursor):
        if not can_run:
            return
        duck_threads = VaultDBThreaded(10,fetch_arrow_query)
        duck_threads.multithread_test()

    def test_fetch_record_batch(self, duckdb_cursor):
        if not can_run:
            return
        duck_threads = VaultDBThreaded(10,fetch_record_batch_query)
        duck_threads.multithread_test()

    def test_transaction(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,transaction_query)
        duck_threads.multithread_test()

    def test_df_append(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,df_append)
        duck_threads.multithread_test()

    def test_df_register(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,df_register)
        duck_threads.multithread_test()

    def test_df_unregister(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,df_unregister)
        duck_threads.multithread_test()

    def test_arrow_register_unregister(self, duckdb_cursor):
        if not can_run:
            return
        duck_threads = VaultDBThreaded(10,arrow_register_unregister)
        duck_threads.multithread_test()

    def test_table(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,table)
        duck_threads.multithread_test()

    def test_view(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,view)
        duck_threads.multithread_test()

    def test_values(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,values)
        duck_threads.multithread_test()
    
    def test_from_query(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,from_query)
        duck_threads.multithread_test()

    def test_from_DF(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,from_df)
        duck_threads.multithread_test() 

    def test_from_arrow(self, duckdb_cursor):
        if not can_run:
            return
        duck_threads = VaultDBThreaded(10,from_arrow)
        duck_threads.multithread_test()
 
    def test_from_csv_auto(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,from_csv_auto)
        duck_threads.multithread_test()

    def test_from_parquet(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,from_parquet)
        duck_threads.multithread_test()

    def test_description(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,description)
        duck_threads.multithread_test()

    def test_cursor(self, duckdb_cursor):
        duck_threads = VaultDBThreaded(10,cursor)
