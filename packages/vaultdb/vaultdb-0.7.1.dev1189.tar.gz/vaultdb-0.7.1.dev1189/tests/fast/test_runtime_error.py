...
closed = lambda: pytest.raises(vaultdb.ConnectionException, match='Connection has already been closed')
no_result_set = lambda: pytest.raises(vaultdb.InvalidInputException, match='No open result set')

class TestRuntimeError(object):
    def test_fetch_error(self):
        con = vaultdb.connect()
        con.execute("create table tbl as select 'hello' i")
        with pytest.raises(vaultdb.ConversionException):
    ...
    def test_register_error(self):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match='Python Object str not suitable to be registered as a view'):
    ...
    def test_arrow_fetch_table_error(self):
        ...
        res.close()
        with pytest.raises(vaultdb.InvalidInputException, match='There is no query result'):
    ...
    def test_arrow_record_batch_reader_error(self):
        ...
        res = arrow_relation.execute()
        res.close()
        with pytest.raises(vaultdb.ProgrammingError, match='There is no query result'):
    ...
    def test_relation_fetchall_error(self):
        ...
        del df_in
        with pytest.raises(vaultdb.ProgrammingError, match='Table with name df_in does not exist'):
            rel.fetchall()
    ...
    def test_relation_query_error(self):
        ...
        rel = conn.query("select * from x")
        del df_in
        with pytest.raises(vaultdb.CatalogException, match='Table with name df_in does not exist'):
    ...
    def test_conn_prepared_statement_error(self):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match='Prepared statement needs 2 parameters, 1 given'):
