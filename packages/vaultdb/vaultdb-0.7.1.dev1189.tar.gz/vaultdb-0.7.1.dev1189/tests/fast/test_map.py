...
class TestMap(object):
    def test_map(self, duckdb_cursor):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match='Expected 1 columns from UDF, got 2'):
            print(testrel.map(evil1).df())

        with pytest.raises(vaultdb.InvalidInputException, match='UDF column type mismatch'):
            print(testrel.map(evil2).df())

        with pytest.raises(vaultdb.InvalidInputException, match='UDF column name mismatch'):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match='UDF returned more than 2048 rows, which is not allowed.'):
            testrel.map(return_big_dataframe).df()

        empty_rel.map(return_dataframe).df().equals(pd.DataFrame({'A' : []}))

        with pytest.raises(vaultdb.InvalidInputException, match='No return value from Python function'):
            testrel.map(return_none).df()

        with pytest.raises(vaultdb.InvalidInputException, match='Need a DataFrame with at least one column'):
