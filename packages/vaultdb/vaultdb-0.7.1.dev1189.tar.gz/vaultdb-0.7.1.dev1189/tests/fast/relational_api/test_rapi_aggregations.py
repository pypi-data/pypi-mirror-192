...
def aggregation_generic(aggregation_function,assertion_answers):
    ...
    if len(assertion_answers) < 3:
        ...
        with pytest.raises(vaultdb.BinderException, match='No function matches the given name'):
    ...
    # Check broken
    with pytest.raises(vaultdb.BinderException, match='Referenced column "nonexistant" not found'):
...
class TestRAPIAggregations(object):
    ...
    def test_value_counts(self, duckdb_cursor, table):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match='Only one column is accepted'):
