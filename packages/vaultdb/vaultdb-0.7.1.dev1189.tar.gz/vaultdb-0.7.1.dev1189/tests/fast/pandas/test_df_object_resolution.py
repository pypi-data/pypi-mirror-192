...
standard_vector_size = vaultdb.__standard_vector_size__
...
class TestResolveObjectColumns(object):
    ...
    def test_integers(self):
        ...
        df_out = vaultdb.query_df(df_in, "data", "SELECT * FROM data").df()
        print(df_out)
    ...
    def test_struct_correct(self):
        ...
        duckdb_col = vaultdb.query("SELECT {a: 1, b: 3, c: 3, d: 7} as '0'").df()
        converted_col = vaultdb.query_df(df, "data", "SELECT * FROM data").df()
    ...
    def test_map_fallback_different_keys(self):
        ...
        converted_df = vaultdb.query_df(x, "x", "SELECT * FROM x").df()
        y = pd.DataFrame(
            [
        ...
        equal_df = vaultdb.query_df(y, "y", "SELECT * FROM y").df()
    ...
    def test_map_fallback_value_upgrade(self):
        ...
        converted_df = vaultdb.query_df(x, "df", "SELECT * FROM df").df()
        equal_df = vaultdb.query_df(y, "df", "SELECT * FROM df").df()
        pd.testing.assert_frame_equal(converted_df, equal_df)
    ...
    def test_map_correct(self):
        ...
        converted_col = vaultdb.query_df(x, "x", "select * from x as 'a'").df()
        vaultdb.query("""
            CREATE TABLE tmp(
        ...
        for _ in range(5):
            ...
            """)
        duckdb_col = vaultdb.query("select a from tmp AS '0'").df()
    ...
    def test_map_value_upgrade(self):
        ...
        x.rename(columns = {0 : 'a'}, inplace = True)
        converted_col = vaultdb.query_df(x, "x", "select * from x").df()
        vaultdb.query("""
            CREATE TABLE tmp2(
        ...
        for _ in range(4):
            ...
            """)
        duckdb_col = vaultdb.query("select a from tmp2 AS '0'").df()
        print(duckdb_col.columns)
        print(converted_col.columns)
        pd.testing.assert_frame_equal(converted_col, duckdb_col)
    ...
    def test_map_duplicate(self):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match="Dict->Map conversion failed because 'key' list contains duplicates"):
            converted_col = vaultdb.query_df(x, "x", "select * from x").df()
    ...
    def test_map_nullkey(self):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match="Dict->Map conversion failed because 'key' list contains None"):
            converted_col = vaultdb.query_df(x, "x", "select * from x").df()
    ...
    def test_map_nullkeylist(self):
        ...
        converted_col = vaultdb.query_df(x, "x", "select * from x").df()
        duckdb_col = vaultdb.query("SELECT {key: NULL, value: NULL} as '0'").df()
        pd.testing.assert_frame_equal(duckdb_col, converted_col)
    ...
    def test_struct_key_conversion(self):
        ...
        duckdb_col = vaultdb.query("select {'5':1, '-25':3, '32':3, '32456':7} as '0'").df()
        converted_col = vaultdb.query_df(x, "tbl", "select * from tbl").df()
        vaultdb.query("drop view if exists tbl")
        pd.testing.assert_frame_equal(duckdb_col, converted_col)
    ...
    def test_list_correct(self):
        ...
        duckdb_col = vaultdb.query("select [[5], [34], [-245]] as '0'").df()
        converted_col = vaultdb.query_df(x, "tbl", "select * from tbl").df()
        vaultdb.query("drop view if exists tbl")
        pd.testing.assert_frame_equal(duckdb_col, converted_col)
    ...
    def test_list_contains_null(self):
        ...
        duckdb_col = vaultdb.query("select [[5], NULL, [-245]] as '0'").df()
        converted_col = vaultdb.query_df(x, "tbl", "select * from tbl").df()
        vaultdb.query("drop view if exists tbl")
        pd.testing.assert_frame_equal(duckdb_col, converted_col)
    ...
    def test_list_starts_with_null(self):
        ...
        duckdb_col = vaultdb.query("select [NULL, [5], [-245]] as '0'").df()
        converted_col = vaultdb.query_df(x, "tbl", "select * from tbl").df()
        vaultdb.query("drop view if exists tbl")
        pd.testing.assert_frame_equal(duckdb_col, converted_col)
    ...
    def test_list_column_value_upgrade(self):
        ...
        """)
        duckdb_col = vaultdb.query("select a from tmp3 AS '0'").df()
        print(duckdb_col.columns)
        print(converted_col.columns)
        pd.testing.assert_frame_equal(converted_col, duckdb_col)
    ...
    def test_numpy_datetime(self):
        ...
        res = vaultdb.query_df(x, "x", "select distinct * from x").df()
    ...
    def test_numpy_datetime_int_internally(self):
        ...
        x = pd.DataFrame({'dates': pd.Series(data=data, dtype='object')})
        with pytest.raises(vaultdb.ConversionException, match=re.escape("Conversion Error: Unimplemented type for cast (BIGINT -> TIMESTAMP)")):
            rel = vaultdb.query_df(x, "x", "create table dates as select dates::TIMESTAMP WITHOUT TIME ZONE from x")
    ...
    def test_fallthrough_object_conversion(self):
        ...
        duckdb_col = vaultdb.query_df(x, "x", "select * from x").df()
        df_expected_res = pd.DataFrame({'0': pd.Series(['4','2','0'])})
        pd.testing.assert_frame_equal(duckdb_col, df_expected_res)

    def test_numeric_decimal(self):
        duckdb_conn = vaultdb.connect()

        # VaultDB uses DECIMAL where possible, so all the 'float' types here are actually DECIMAL
        reference_query = """
        ...
        reference = vaultdb.query("select * from tbl", connection=duckdb_conn).fetchall()
        conversion = vaultdb.query_df(x, "x", "select * from x").fetchall()
    ...
    def test_multiple_chunks_aggregate(self):
        ...
        res = vaultdb.query("select * from dates", connection=conn).df()
        date_df = res.copy()
        ...
        expected_res = vaultdb.query('select avg(epoch(i)), min(epoch(i)), max(epoch(i)) from dates;', connection=conn).fetchall()
        actual_res = vaultdb.query_df(date_df, 'x', 'select avg(epoch(i)), min(epoch(i)), max(epoch(i)) from x').fetchall()
    ...
    def test_mixed_object_types(self):
        ...
        res = vaultdb.query_df(x, "x", "select * from x").df()
    ...
    def test_analyze_sample_too_small(self):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match="Failed to cast value: Unimplemented type for cast"):
            res = vaultdb.query_df(x, "x", "select * from x").df()
    ...
    def test_numeric_decimal_zero_fractional(self):
        ...
        duckdb_conn.execute(reference_query)
        reference = vaultdb.query("select * from tbl", connection=duckdb_conn).fetchall()
        conversion = vaultdb.query_df(decimals, "x", "select * from x").fetchall()
