...
def check_equal(df, reference_query):
	...
	res = vaultdb.query('SELECT * FROM tbl', connection=duckdb_conn).fetchall()
	df_res = vaultdb.query('SELECT * FROM tbl', connection=duckdb_conn).df()
	out = vaultdb.query_df(df, 'x', "SELECT * FROM x").fetchall()
	assert(res == out)
