...
class TestPandasUnregister(object):
    def test_pandas_unregister1(self, duckdb_cursor):
        ...
        with pytest.raises(vaultdb.CatalogException, match='Table with name dataframe does not exist'):
            connection.execute("SELECT * FROM dataframe;").fetchdf()
        with pytest.raises(vaultdb.CatalogException, match='View with name dataframe does not exist'):
