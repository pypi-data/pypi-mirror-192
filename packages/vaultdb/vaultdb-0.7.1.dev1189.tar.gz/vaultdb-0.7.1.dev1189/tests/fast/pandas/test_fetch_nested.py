...
def compare_results(query, list_values=[]):
    df_duck = vaultdb.query(query).df()
    counter = 0
...
class TestFetchNested(object):
    ...
    def test_map_df(self,duckdb_cursor):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match="Map keys have to be unique"):
        ...
        with pytest.raises(vaultdb.InvalidInputException, match="Map keys can not be NULL"):
    ...
    def test_nested_mix(self,duckdb_cursor):
        ...
        df_duck = vaultdb.query("SELECT col0 as a FROM (VALUES ({'i':1,'j':2}), (NULL), ({'i':1,'j':2}), (NULL))").df()
        duck_values = df_duck['a']
        ...
        df_duck = vaultdb.query("SELECT col0 as a FROM (VALUES (MAP(LIST_VALUE(1,2),LIST_VALUE(3,4))),(NULL), (MAP(LIST_VALUE(1,2),LIST_VALUE(3,4))), (NULL))").df()
        duck_values = df_duck['a']
