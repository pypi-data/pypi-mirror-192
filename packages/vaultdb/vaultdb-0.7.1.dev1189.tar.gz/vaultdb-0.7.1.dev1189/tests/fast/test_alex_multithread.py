...
def tmp_database(tmp_path_factory):
    database = tmp_path_factory.mktemp(
        "databases", numbered=True) / "tmp.duckdb"
    return str(database)
...
def insert_from_cursor(duckdb_con):
    ...
    duckdb_cursor.execute(
        """INSERT INTO my_inserts VALUES (?)""", (thread_name,))
...
class TestPythonMultithreading(object):
    def test_multiple_cursors(self, duckdb_cursor):
        duckdb_con = vaultdb.connect()  # In Memory VaultDB
        duckdb_con.execute(
            """CREATE OR REPLACE TABLE my_inserts (thread_name varchar)""")
        ...
        assert duckdb_con.execute("""SELECT * FROM my_inserts order by thread_name""").fetchall() == [
            ('my_thread_0',), ('my_thread_1',), ('my_thread_2',)]
    ...
    def test_multiple_cursors_persisted(self, tmp_database):
        duckdb_con = vaultdb.connect(tmp_database)
        duckdb_con.execute(
            """CREATE OR REPLACE TABLE my_inserts (thread_name varchar)""")
