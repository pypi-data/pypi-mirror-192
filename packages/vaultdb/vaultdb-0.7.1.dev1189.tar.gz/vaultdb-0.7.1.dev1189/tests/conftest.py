...
def require():
    def _require(extension_name, db_name=''):
        ...
        # VAULTDB_PYTHON_TEST_EXTENSION_PATH can be used to add a path for the extension test to search for extensions
        if 'VAULTDB_PYTHON_TEST_EXTENSION_PATH' in os.environ:
            env_extension_path = os.getenv('VAULTDB_PYTHON_TEST_EXTENSION_PATH');
        ...
        for path in extension_paths_found:
            ...
            if (path.endswith(extension_name+".duckdb_extension")):
                conn = vaultdb.connect(db_name, config={'allow_unsigned_extensions' : 'true'})
...
def duckdb_cursor_autocommit(request, tmp_path):
    ...
    connection = vaultdb.connect(test_dbfarm)
