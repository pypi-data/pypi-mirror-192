...
def check_create_table(category):
    ...
    df_out = vaultdb.query_df(df_in, "data", "SELECT * FROM data")
    df_out = df_out.df()
    ...
    res = conn.execute(
        "SELECT t1.x FROM t1 inner join t2 on (t1.x = t2.x) order by t1.x").fetchall()
    assert res == conn.execute("SELECT x FROM t1 order by t1.x").fetchall()

    res = conn.execute(
        "SELECT t1.x FROM t1 inner join t2 on (t1.x = t2.y) order by t1.x").fetchall()
    correct_res = conn.execute("SELECT x FROM t1 order by x").fetchall()
    assert res == correct_res

    # Run equal ENUM comparison
    res = conn.execute(
        "SELECT t1.x FROM t1,t2 where t1.x = t2.x order by t1.x").fetchall()
    assert res == correct_res

    # Run different ENUM comparison
    conn.execute("CREATE TABLE t3 AS SELECT * FROM df_in_diff")
    res = conn.execute(
        "SELECT t1.x FROM t1,t3 where t3.k = t1.x order by t1.x").fetchall()
    assert res == correct_res
