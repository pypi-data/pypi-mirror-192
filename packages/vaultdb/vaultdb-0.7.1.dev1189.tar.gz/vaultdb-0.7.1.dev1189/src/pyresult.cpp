...
unique_ptr<DataChunk> VaultDBPyResult::FetchNext(QueryResult &result) {
...
unique_ptr<DataChunk> VaultDBPyResult::FetchNextRaw(QueryResult &result) {
	if (!result_closed && result.type == QueryResultType::STREAM_RESULT && !((StreamQueryResult &)result).IsOpen()) {
		result_closed = true;
		return nullptr;
	}
...
py::object VaultDBPyResult::Fetchone() {
	{
...
py::list VaultDBPyResult::Fetchmany(idx_t size) {
	py::list res;
...
py::list VaultDBPyResult::Fetchall() {
	py::list res;
	while (true) {
		auto fres = Fetchone();
		if (fres.is_none()) {
...
py::dict VaultDBPyResult::FetchNumpy() {
	return FetchNumpyInternal();
}

void VaultDBPyResult::FillNumpy(py::dict &res, idx_t col_idx, NumpyResultConversion &conversion, const char *name) {
...
py::dict VaultDBPyResult::FetchNumpyInternal(bool stream, idx_t vectors_per_chunk) {
	if (!result) {
		throw InvalidInputException("result closed");
	}
...
void VaultDBPyResult::ChangeToTZType(DataFrame &df) {
...
void VaultDBPyResult::ChangeDateToDatetime(DataFrame &df) {
	for (idx_t i = 0; i < result->ColumnCount(); i++) {
...
DataFrame VaultDBPyResult::FrameFromNumpy(bool date_as_object, const py::handle &o) {
...
DataFrame VaultDBPyResult::FetchDF(bool date_as_object) {
	timezone_config = QueryResult::GetConfigTimezone(*result);
	return FrameFromNumpy(date_as_object, FetchNumpyInternal());
}

DataFrame VaultDBPyResult::FetchDFChunk(idx_t num_of_vectors, bool date_as_object) {
...
bool VaultDBPyResult::FetchArrowChunk(QueryResult *result, py::list &batches, idx_t chunk_size) {
	ArrowArray data;
...
py::object VaultDBPyResult::FetchAllArrowChunks(idx_t chunk_size) {
	if (!result) {
		throw InvalidInputException("result closed");
	}
	auto pyarrow_lib_module = py::module::import("pyarrow").attr("lib");
...
duckdb::pyarrow::Table VaultDBPyResult::FetchArrowTable(idx_t chunk_size) {
	if (!result) {
...
duckdb::pyarrow::RecordBatchReader VaultDBPyResult::FetchRecordBatchReader(idx_t chunk_size) {
	if (!result) {
		throw InvalidInputException("There is no query result");
	}
	py::gil_scoped_acquire acquire;
...
py::list VaultDBPyResult::Description() {
...
void VaultDBPyResult::Close() {
	result = nullptr;
}

bool VaultDBPyResult::IsClosed() const {
