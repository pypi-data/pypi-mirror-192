...
@click.option(
    "--application-fqn", type=click.STRING, help="application fqn", required=True
)
@click.option(
    "--version",
    type=click.INT,
...
def build_logs_command(
    ...
    """
    Get build log tails for application fqn and deployment version
    """
    output_hook = RichOutputCallBack()
    tfs_client = ServiceFoundryServiceClient()

    if version:
        deployment_fqn = f"{application_fqn}:{version}"
    else:
        ...
        deployment_fqn = f"{application_fqn}:{application_info.lastVersion}"
