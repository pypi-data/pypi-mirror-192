...
class SimplePricerEngine:
    ...
    def accumulator(
        ...
        if accumulator_type in [AccumulatorType.LinearAcc, AccumulatorType.FixedAccBarrier]:
        ...
        # LinearAcc
        if pricing_method == PricingMethod.AS and accumulator_type == AccumulatorType.LinearAcc:
            assert barrier_price is not None, "barrier price is required for LinearAcc"
            param['barrier_price'] = barrier_price
            return LinearAcc(param)

        # FixedAcc
        elif pricing_method == PricingMethod.AS and accumulator_type == AccumulatorType.FixedAcc:
            assert payoff is not None, "payoff is required for FixedAcc"
            param['payoff'] = payoff
            return FixedAcc(param)

        # FixedAccBarrier
        elif pricing_method == PricingMethod.AS and accumulator_type == AccumulatorType.FixedAccBarrier:
            assert payoff is not None and barrier_price is not None, "payoff & barrier price is required for FixedAccBarrier"
            param['payoff'] = payoff
            param['barrier_price'] = barrier_price
            return FixedAccBarrier(param)

        # FixedAccAko
        elif pricing_method == PricingMethod.AS and accumulator_type == AccumulatorType.FixedAccAko:
            assert payoff is not None and barrier_price is not None, "payoff & barrier price is required for FixedAccAko"
            param['payoff'] = payoff
            param['barrier_price'] = barrier_price
            param['obs_freq'] = obs_freq
    ...
    def touch(
        ...
        # OneTouch
        if option_type == OptionType.ONETOUCH:
        ...
        # DoubleOneTouch
        elif option_type == OptionType.DOUBLEONETOUCH:
